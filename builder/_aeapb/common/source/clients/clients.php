			...
			jQuery.ajax({
				...
				success: function(data) {
					jQuery.map( data, function( item ) {
						...
						whatsapp.prop('checked', (item.whatsapp == 0 ? false : true));
						phone2.val(item.phone2);
						whatsapp2.prop('checked', (item.whatsapp2 == 0 ? false : true));
						note.val(item.note);
...
<div class="base-app<?php echo ' base-list-'.($cfg['listFull'] ? 'full' : 'ajax')?> clearfix">
	...
	$addBtn = '
		<button type="button" class="base-icon-plus btn-add btn btn-sm btn-success hasTooltip" title="'.$tipText.'" onclick="'.$relAdd.'" data-toggle="modal" data-target="#modal-'.$APPTAG.'" data-backdrop="static" data-keyboard="false">
