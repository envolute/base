...
// LIST
	...
	$query = '
		SELECT SQL_CALC_FOUND_ROWS
			...
			'. $db->quoteName('T1.phone') .',
			'. $db->quoteName('T1.whatsapp') .',
			'. $db->quoteName('T1.phone2') .',
			'. $db->quoteName('T1.whatsapp2') .',
		...
		<table class="table table-striped table-hover table-condensed">
			<thead>
				<tr>
					...
					<th>'.$$SETOrder(JText::_('FIELD_LABEL_GENDER'), 'T1.gender', $APPTAG).'</th>
					<th class="text-center">'.$$SETOrder(JText::_('FIELD_LABEL_BIRTHDAY'), 'T1.birthday', $APPTAG).'</th>
					<th class="text-center">'.JText::_('FIELD_LABEL_PHONE').'</th>
					'.$adminView['head']['actions'].'
				</tr>
			</thead>
			<tbody>
	...
	foreach($res as $item) {
		...
		if($hasAdmin) :
			...
			$adminView['list']['actions'] = '
				<td class="text-center valign-center hidden-print">
		...
		$info = !empty($item->note) ? '<span class="base-icon-info-circled cursor-help text-info hasPopover" data-placement="top" data-content="'.$item->note.'"></span>' : '';
		$gender = $item->gender == 1 ? JText::_('TEXT_MALE') : JText::_('TEXT_FEMALE');
		$phone = '';
		if(!empty($item->phone)) :
			$phone .= $item->phone;
			if($item->whatsapp == 1) $phone .= ' <span class="base-icon-whatsapp text-success cursor-help hasTooltip" title="WhatsApp"></span>';
		endif;
		if(!empty($item->phone2)) :
			$phone .= ' / '.$item->phone2;
			if($item->whatsapp2 == 1) $phone .= ' <span class="base-icon-whatsapp text-success cursor-help hasTooltip" title="WhatsApp"></span>';
		endif;
		...
		$html .= '
			<tr id="'.$APPTAG.'-item-'.$item->id.'" class="'.$rowState.'">
				...
				<td>
					...
					'.$info.'<small class="font-featured text-live">'.$item->email.'</small>
				</td>
				<td class="valign-center">'.$gender.'</td>
				<td class="valign-center">'.baseHelper::dateFormat($item->birthday).'</td>
				<td class="valign-center">'.$phone.'</td>
				'.$adminView['list']['actions'].'
			</tr>
		';
	}
