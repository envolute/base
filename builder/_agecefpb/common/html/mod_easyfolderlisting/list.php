...
<?php if($total > 0) : ?>
	...
	<?php for ($i = 0; $i < $total; $i++) : ?>
		...
		{
			$dt = modEasyFolderListingHelper::formatDate($params, $rows[$i]['date']);
		}
		
		//show info
		if ($params->get('efl_size') == "yes" || $params->get('efl_date') == "yes")
		{
			$info = ' class="hasTooltip" title="'.$sz.' '.$dt.'" ';
		}
		...
		{
			echo '<a href="'.JURI::base().$folder.'/'.$fixedName.'.'.$rows[$i]['ext'].'"'.$info.'>';
		}
