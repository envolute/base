...
<div id="polldiv_<?php echo $poll->id;?>">
  ...
  <form action="<?php echo JRoute::_('index.php');?>" method="post" name="poll_vote_<?php echo $poll->id;?>" id="poll_vote_<?php echo $poll->id;?>">
    <div class="radio">
      <?php for ($i = 0, $n = count($options); $i < $n; $i ++) { ?>
      	<label for="mod_voteid<?php echo $options[$i]->id;?>" class="<?php echo $tabclass_arr[$tabcnt].$params->get('moduleclass_sfx'); ?>" style="display:block;">
  ...
  <?php  endforeach;
  			...
  			if($params->get('show_msg')) :
  				echo '<div class="alert alert-warning all-expand" id="mod_poll_messages_'.$poll->id.'" style="margin:5px 0;">'.JText::_($msg);
