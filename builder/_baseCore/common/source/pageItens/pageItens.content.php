	...
	if($item->id != null) : // verifica se existe
			...
			for($i = 0; $i < count($files[$item->id]); $i++) {
				if(!empty($files[$item->id][$i]->filename)) :
					...
					elseif($files[$item->id][$i]->index > 1) : // images
						$images[] = 'images/uploads/'.$APPNAME.'/'.$files[$item->id][$i]->filename;
					endif;
				endif;
			}
		endif;
		...
		elseif($item->contentType == 3) : // gallery
			...
			for($i = 0; $i < count($images); $i++) {
				...
				$itemContent .= '<div class="'.$grid.' bottom-space-xl"><a href="'.$images[$i].'" class="set-modal" data-modal-rel="pageContent" data-modal-title="'.$item->title.'">'.$img.'</a></div>';
			}
			$itemContent .= '</div>';
		endif;
		// LINK PARA LISTAGEM
		...
		if($hasAdmin) :
			$itemActions = '
				<div class="'.$APPNAME.'-item-actions top-expand-sm">
