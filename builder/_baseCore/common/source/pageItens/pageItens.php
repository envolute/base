...
jQuery(function() {
	...
	// métodos controladores do formulário
		...
		window.<?php echo $APPTAG?>_formReset = function() {
			...
			title.val('');
			description.val('');
			selectRadio(coverType, <?php echo $_SESSION[$RTAG.'CoverTypeDef']?>);
			videoEmbed.val('');
			date.val('');
			month.val('0').trigger("chosen:updated"); // select
			year.val('<?php echo date('Y')?>');
			selectRadio(contentType, <?php echo $_SESSION[$RTAG.'ContentTypeDef']?>);
			url.val('');
			...
			selectRadio(element_contentModal, '');
			selectRadio(element_modalHeader, '');
			...
			selectRadio(content_galleryCaption, 0);
			content_layout.val('');
		...
		<?php if($cfg['hasUpload']) :?>
			...
			window.<?php echo $APPTAG?>_setNewFile = function() {
				var fileField = '';
				fileField += '<div class="col-xs-6 col-sm-3">';
				fileField += '	<div class="form-group">';
				fileField += '		<span class="btn-group">';
				fileField += '			<button type="button" class="base-icon-search btn btn-default set-file-action"></button>';
				fileField += '		</span>';
				fileField += '		<input type="file" name="<?php echo $cfg['fileField']?>['+<?php echo $APPTAG?>IndexFile+']" id="<?php echo $APPTAG?>-<?php echo $cfg['fileField']?>'+<?php echo $APPTAG?>IndexFile+'" class="form-control field-image element-invisible" />';
				fileField += '	</div>';
				fileField += '</div>';
			...
			jQuery.ajax({
				...
				success: function(data) {
					jQuery.map( data, function( item ) {
						...
						selectRadio(coverType, item.coverType);
						...
						year.val(item.year);
						selectRadio(contentType, item.contentType);
						url.val(item.url);
						...
						selectRadio(element_contentModal, item.element_contentModal);
						selectRadio(element_modalHeader, item.element_modalHeader);
						...
						selectRadio(content_galleryCaption, item.content_galleryCaption);
...
<div class="base-app<?php echo ' base-list-'.($cfg['listFull'] ? 'full' : 'ajax')?> clearfix">
	...
	';
	$admBtn = (!$cfg['listFull'] && !empty($_SESSION[$RTAG.'UrlListAdmin'])) ? '<a href="'.$_SESSION[$RTAG.'UrlListAdmin'].'" class="base-icon-cog btn btn-sm btn-warning hasTooltip" title="'.JText::_('TEXT_MANAGE').'"></a>' : '';
	?>
	...
	if($cfg['itemContent']) :
		...
		$itemContent = require($APPNAME.'.content.php');
