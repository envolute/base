...
jQuery(function() {
	...
	var descPopup	= jQuery('#modal-<?php echo $APPTAG?>-description');
		// lista completa
			...
			if(typeof id !== "null" && typeof id !== "undefined" && id != 0) {
				if(parentFieldId != null) {
					...
					if(parentFieldGroup && <?php echo $_SESSION[$RTAG.'HideParentField']?> && parentFieldId.find('option[value="'+id+'"]').length) {
						...
						console.log('<?php echo $_SESSION[$RTAG.'HideParentField']?>');
					}
				}
				btnPrev.remove();
				btnNext.remove();
	...
	// ações & métodos controladores da listagem
		...
		// Set Btn status
		// habilita/desabilita os botões de ação se houver, ou não, checkboxes marcados na Listagem
		window.<?php echo $APPTAG?>_viewDesc = function(itemID, btn) {
			var view = jQuery(jQuery(btn).data('target'));
			var head = jQuery('#<?php echo $APPTAG?>-description-'+itemID).find('.description-header').text();
			var body = jQuery('#<?php echo $APPTAG?>-description-'+itemID).find('.description-content').html();
			view.find('.modal-title').text(head);
			view.find('.modal-body').html(body);
		};
		descPopup.on('hidden.bs.modal', function () {
					...
					jQuery.map( data, function( res ) {
						if(res.status == 4) {
							for(i = 0; i < res.ids.length; i++) {
								...
								if((res.state == 2 && e.hasClass('base-icon-check-empty')) || res.state == 0) {
										e.removeClass('base-icon-check-empty').addClass('base-icon-check text-success');
										<?php if($cfg['listFull']) echo 'e.parents("tr").addClass("success");'?>
									...
									e.removeClass('base-icon-check text-success').addClass('base-icon-check-empty');
									<?php if($cfg['listFull']) echo 'e.parents("tr").removeClass("success");'?>
...
<div class="base-app<?php echo ' base-list-'.($cfg['listFull'] ? 'full' : 'ajax')?> clearfix">
	...
	<div class="modal fade" id="modal-<?php echo $APPTAG?>-description" tabindex="-1" role="dialog" aria-labelledby="view-<?php echo $APPTAG?>Description">
