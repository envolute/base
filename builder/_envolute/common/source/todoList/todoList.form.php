...
$query = '
SELECT id, type, CONCAT("#", id, " - ", title) task FROM
	'. $db->quoteName('#__envolute_tasks') .'
WHERE status < 3 AND state = 1
ORDER BY id DESC';
$db->setQuery($query);
			...
			<div class="row">
						...
						<select name="task_id" id="<?php echo $APPTAG?>-task_id" class="form-control field-id auto-tab" data-target="<?php echo $APPTAG?>-contact_id">
							...
							<?php
								foreach ($tasks as $obj) {
									$tp = ($obj->type == 1) ? ' [template]' : '';
									echo '<option value="'.$obj->id.'">'.baseHelper::nameFormat($obj->task).$tp.'</option>';
								}
							?>
						</select>
						<input type="hidden" name="ctask_id" id="<?php echo $APPTAG?>-ctask_id" />
					</div>
				</div>
				<div class="col-sm-9">
					<div class="form-group field-required">
				...
				<div class="col-sm-12">
					<div class="form-group">
						...
						<textarea rows="6" name="description" id="<?php echo $APPTAG?>-description" class="field-html" data-editor-full="true"></textarea>
					</div>
				</div>
			</div>
		</fieldset>
