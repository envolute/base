		...
		<table class="table table-striped table-hover table-condensed">
			<thead>
				<tr>
					...
					<th>'.JText::_('FIELD_LABEL_ORDER').'</th>
					'.$adminView['head']['actions'].'
				</tr>
			</thead>
			<tbody>
	...
	foreach($res as $item) {
		...
		if($item->taskType == 0) :
			...
			<a href="#" onclick="'.$APPTAG.'_setState('.$item->id.')" id="'.$APPTAG.'-state-'.$item->id.'">
				<span class="'.($item->state == 0 ? 'base-icon-check text-success' : 'base-icon-check-empty').'"></span>
			</a>
			';
		else :
			$template = ' <span class="label label-warning">Template</span>';
			$check = '<span class="base-icon-check-empty text-muted" style="cursor:not-allowed;"></span>';
		endif;
		...
		$html .= '
			<tr id="'.$APPTAG.'-item-'.$item->id.'" class="'.$rowState.'">
				...
				<td>#'.$item->task_id.' - '.$item->task.$template.'</td>
				<td>'.$item->ordering.'</td>
				'.$adminView['list']['actions'].'
			</tr>
		';
