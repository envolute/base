...
header( 'content-type: application/json; charset=utf-8' );
  ...
  if($task != null) :
    ...
    // CUSTOM -> Set Order
    function setOrder($ID, $taskId, $ctaskId, $ord, $cord, $cfg) {
      if(!empty($ID) && $ID != 0 && !empty($taskId) && $taskId != 0) :
        // database connect
      	$db = JFactory::getDbo();
        // last item
        $query = 'SELECT MAX('.$db->quoteName('ordering').') FROM '. $db->quoteName($cfg['mainTable']) .' WHERE '.$db->quoteName('id').' != '.$ID.' AND '. $db->quoteName('task_id') .' = '. $taskId;
    		$db->setQuery($query);
        $max = $db->loadResult();
        $max = empty($max) ? 0 : $max;
        if((!empty($ord) && $ord != 0) && ($ord != $cord || $taskId != $ctaskId)) :
          // this item position
          $query = 'SELECT COUNT(*) FROM '. $db->quoteName($cfg['mainTable']) .' WHERE '.$db->quoteName('ordering').' = '.$ord.' AND  '. $db->quoteName('id') .' != '. $ID .' AND '. $db->quoteName('task_id') .' = '. $taskId;
      		$db->setQuery($query);
          $exist = $db->loadResult();
          if($ord <= $max) :
            // define a posição quando a ordem é definida no insert
            // altera todos a partir da nova ordem
            if($cord == 0 || $taskId != $ctaskId) :
              $query = '
          ...
          // se o item for maior do que o máx, seta o máximo e define a ordem
            ...
            WHERE '.
              $db->quoteName('id') .' = '. $ID;
        		$db->setQuery($query);
            $db->execute();
            if($cord < $max) setOrder($ID, $taskId, $ctaskId, $nord, $cord, $cfg);
            return true;
          endif;
        elseif(empty($ord) || $ord == 0) :
          ...
          if($ord != $cord && $cord <= $max) :// this item position
            ...
            $query = 'SELECT COUNT(*) FROM '. $db->quoteName($cfg['mainTable']) .' WHERE '.$db->quoteName('ordering').' = '.$cord.' AND  '. $db->quoteName('id') .' != '. $ID .' AND '. $db->quoteName('task_id') .' = '. $taskId;
            $db->setQuery($query);
            $cexist = $db->loadResult();
            if(!$cexist) setOrder($ID, $taskId, $taskId, $max + 1, $cord, $cfg);
          endif;
          return true;
        else :
          return false;
    ...
    function reOrder($ID, $taskId, $ord, $cfg) {
      if(!empty($ID) && $ID != 0) :
        // database connect
      	$db = JFactory::getDbo();
        // last item
      ...
      if($id) :
        ...
        // COUNT LIST IDS
          ...
          foreach($list as $item) {
  			...
  			elseif($task == 'save' && $id) :
  				...
  				$query .=
            $db->quoteName('task_id') .'='. $db->quote($request['task_id']) .','.
            ...
            // SET ORDER
            setOrder($id, $request['task_id'], $request['ctask_id'], $request['ordering'], $request['cordering'], $cfg);
            if($request['task_id'] != $request['ctask_id']) :
              // RE-ORDER
              reOrder($id, $request['ctask_id'], $request['cordering'], $cfg);
            endif;
          ...
          for($i = 0; $i < count($Ids); $i++) {
            // GET PARENT INFO
            $queryOrder = 'SELECT task_id, ordering FROM '. $db->quoteName($cfg['mainTable']) .' WHERE '. $db->quoteName('id') .' = '.$Ids[$i];
            $db->setQuery($queryOrder);
            $obj = $db->loadObject();
            // RE-ORDER
            reOrder($Ids[$i], $obj->task_id, $obj->ordering, $cfg);
          }
        ...
        // validation
        if($request['task_id'] != 0 && !empty($request['name'])) :
