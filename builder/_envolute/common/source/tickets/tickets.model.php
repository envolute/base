...
header( 'content-type: application/json; charset=utf-8' );
  ...
  if($task != null) :
    ...
    // app
  	...
  	$request['department_id']= $input->get('department_id', 0, 'int');
    ...
    $request['subject']      = $input->get('subject', '', 'string');
      ...
      // define a data de fechamento da solicitação
      $end_date = $request['status'] == 1 ? date('Y-m-d H:i:s') : '';
    // email alert
      ...
      if($id) :
        ...
        // COUNT LIST IDS
          ...
          foreach($list as $item) {
            ...
            $db->quoteName('department_id') 	.'='. $request['department_id'] .','.
        ...
        // validation
        if($request['project_id'] != 0 && !empty($request['subject'])) :

          // Prepare the insert query
    			$query  = '
          INSERT INTO '. $db->quoteName($cfg['mainTable']) .'('.
            $db->quoteName('project_id') .','.
            $db->quoteName('department_id') .','.
            $db->quoteName('type') .','.
            ...
            $mailAPP = 'suporte@envolute.com';

            if($request['emailAlert'] == 1) :
              // get project name
              $query = 'SELECT '. $db->quoteName('name') .' FROM '. $db->quoteName('#__envolute_projects').' WHERE id='.$request['project_id'];
              $db->setQuery($query);
              $project = $db->loadResult();
              // get department info
              $query = 'SELECT '. $db->quoteName('name') .', '. $db->quoteName('email') .' FROM '. $db->quoteName($cfg['mainTable'].'_departments').' WHERE id='.$request['department_id'];
              $db->setQuery($query);
              $department = $db->loadObject();
              $mailFrom = !empty($department->email) ? $department->email : $config->get('mailfrom');
              // get files
              $query = 'SELECT *, TO_BASE64('. $db->quoteName('filename') .') fn, TO_BASE64('. $db->quoteName('mimetype') .') mt FROM '. $db->quoteName($cfg['fileTable']) .' WHERE '. $db->quoteName('id_parent') .' = '. $id . ' ORDER BY '. $db->quoteName('index');
          		$db->setQuery($query);
              ...
              foreach ($attachs as $attach) {
                $msgFiles .= '<br /><a href="'.$_root.'get-file?fn='.base64_encode($attach->filename).'&mt='.base64_encode($attach->mimetype).'&tag='.base64_encode($APPTAG).'">'.$attach->filename.' (original: '.$attach->originalName.')</a>';
              }
              if(!empty($msgFiles)) $msgFiles = '<p><strong>'.JText::_('FIELD_LABEL_ATTACHMENT').'s</strong>: '.$msgFiles.'</p>';
              // get priority
              $warning = '';
          		switch ($request['priority']) {
          			case '0':
          				$ptext = JText::_('FIELD_LABEL_LOW');
          				break;
          			case '1':
          				$ptext = JText::_('FIELD_LABEL_MEDIUM');
          				break;
          			case '2':
          				$ptext = JText::_('FIELD_LABEL_HIGHT');
                  $warning = '['.JText::_('MSG_EMAIL_URGENCY').']';
          				break;
          			default:
          				$ptext = '';
          		}
              switch ($request['status']) {
          			case '0':
          				$stext = JText::_('FIELD_LABEL_OPENED');
          				break;
          			case '1':
          				$stext = JText::_('FIELD_LABEL_CLOSED');
          				break;
          			default:
          				$ptext = '';
          		}
              // mensagem default
              $emailSubject = '[Ticket ID: '.$id.']'.$warning.' '.$request['subject'];
              // Send email alert
              $msgAlert = JText::sprintf('MSG_EMAIL_NEW_TICKET', $user->name, $project, $request['subject'], baseHelper::nameFormat($department->name), $ptext, $stext, $request['description'].$msgFiles);
              $msgFooter = JText::sprintf('MSG_EMAIL_NEW_TICKET_FOOTER', $domain);
              $eBody = baseHelper::mailTemplateDefault($msgAlert, JText::_('MSG_EMAIL_TITLE'), $msgFooter, 'logo-news-team.png');
              $mailAlert = JFactory::getMailer();
              $mailAlert->sendMail($mailAPP, $user->name, $mailFrom, $emailSubject.'Admin', $eBody, true);
              // Send email confirmation
              $ticketUrl = $_root.'client-area/ticket.php?id='.$id;
              $msgReturn = JText::sprintf('MSG_EMAIL_NEW_TICKET_CONFIRM', $user->name, $request['subject'], baseHelper::nameFormat($department->name), $ptext, $stext, $ticketUrl, $ticketUrl);
              $eReturn = baseHelper::mailTemplateDefault($msgReturn, JText::_('MSG_EMAIL_TITLE'), '', 'logo-news-team.png');
              $mailConfirm = JFactory::getMailer();
              $mailConfirm->sendMail($mailFrom, $senderName, $user->email, $emailSubject, $eReturn, true);
            endif;
