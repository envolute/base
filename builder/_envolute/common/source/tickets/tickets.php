...
jQuery(function() {
	...
	var department_id	= jQuery('#<?php echo $APPTAG?>-department_id');
	...
	var priority			= mainForm.find('input[name=priority]:radio');
	var end_date			= jQuery('#<?php echo $APPTAG?>-end_date');
	var status				= mainForm.find('input[name=status]:radio');
	var emailAlert		= jQuery('#<?php echo $APPTAG?>-emailAlert');
	...
	// métodos controladores do formulário
		...
		window.<?php echo $APPTAG?>_formReset = function() {
			...
			department_id.val('0').trigger("chosen:updated"); // select
			selectRadio(type, 0);
			subject.val('');
			description.val('');
			selectRadio(priority, 0);
			selectRadio(status, 0);
				// hide status on the insert
				jQuery('#<?php echo $APPTAG?>-status-group').addClass('hide');
			emailAlert.prop('checked', false).prop('disabled', false);
				// show emailAlert on the insert
				jQuery('#<?php echo $APPTAG?>-emailAlert-group').removeClass('hide');
			...
			jQuery.ajax({
				...
				success: function(data) {
					jQuery.map( data, function( item ) {
						...
						department_id.val(item.department_id).trigger("chosen:updated"); // selects
						selectRadio(type, item.type); // radio
						subject.val(item.subject);
						description.val(item.description);
						selectRadio(priority, item.priority); // radio
						selectRadio(status, item.status); // radio
							// show status on the edit
							jQuery('#<?php echo $APPTAG?>-status-group').removeClass('hide');
						emailAlert.prop('checked', false).prop('disabled', true);
							// hide emailAlert on the insert
							jQuery('#<?php echo $APPTAG?>-emailAlert-group').addClass('hide');
