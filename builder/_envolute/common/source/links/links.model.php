  ...
  if($task != null) :
    ...
    // app
      ...
      if($id) :
        ...
        // validation
        if($request['category_id'] != 0 && !empty($request['url'])) :
          ...
          INSERT INTO '. $db->quoteName($cfg['mainTable']) .'('.
            ...
            $request['period'] .','.
    				...
    				$db->quote($request['currency']) .','.
    				$db->quote($request['price']) .','.
    				$db->quote($request['start_date']) .','.
    				$db->quote($request['user']) .','.
    				$db->quote($request['password']) .','.
