...
if(isset($_SERVER["HTTP_X_REQUESTED_WITH"]) AND strtolower($_SERVER["HTTP_X_REQUESTED_WITH"]) == "xmlhttprequest") :
	...
	$query .= ' ORDER BY '. $db->quoteName('T1.due_date') .' DESC, '. $db->quoteName('T2.name') .' ASC';
	try {
		...
		foreach($res as $item) {
			...
			$html .= '
				<li class="'.$rowState.'">
					...
					'.baseHelper::getMonthName($item->month).' / '.$item->year.' ('.baseHelper::dateFormat($item->due_date).')
					<div class="small text-muted font-featured">'.$item->description.'</div>
				</li>
			';
		}
		$html .= '</ul>';
