<?php
/* SISTEMA PARA CADASTRO DE REDES SOCIAIS
 * AUTOR: IVO JUNIOR
 * EM: 18/02/2016
*/
...
jQuery(function() {
	...
	var payPopup	= jQuery('#modal-<?php echo $APPTAG?>-pay');
		// lista completa
	...
	var type					= mainForm.find('input[name=type]:radio');
	...
	var create_boleto = mainForm.find('input[name=create_boleto]:radio');
	var description		= jQuery('#<?php echo $APPTAG?>-description');
	var description1	= jQuery('#<?php echo $APPTAG?>-description1');
	var description2	= jQuery('#<?php echo $APPTAG?>-description2');
	var description3	= jQuery('#<?php echo $APPTAG?>-description3');
	var description4	= jQuery('#<?php echo $APPTAG?>-description4');
	var hosting				= jQuery('#<?php echo $APPTAG?>-hosting');
	var discount			= jQuery('#<?php echo $APPTAG?>-discount');
	var discount_note	= jQuery('#<?php echo $APPTAG?>-discount_note');
	var tax						= jQuery('#<?php echo $APPTAG?>-tax');
	var tax_note			= jQuery('#<?php echo $APPTAG?>-tax_note');
	var assessment		= jQuery('#<?php echo $APPTAG?>-assessment');
	var assessment_note		= jQuery('#<?php echo $APPTAG?>-assessment_note');
	var email_subject = jQuery('#<?php echo $APPTAG?>-email_subject');
	var email_content = jQuery('#<?php echo $APPTAG?>-email_content');
	var email_subject_resend = jQuery('#<?php echo $APPTAG?>-email_subject_resend');
	var email_content_resend = jQuery('#<?php echo $APPTAG?>-email_content_resend');
	var url_boleto 		= jQuery('#<?php echo $APPTAG?>-url_boleto');
	var bankAccount_info	= jQuery('#<?php echo $APPTAG?>-bankAccount_info');
	var paid_date			= jQuery('#<?php echo $APPTAG?>-paid_date');
	...
	// métodos controladores do formulário
		...
		var firstField = ''; // campo que recebe o focus no carregamento
		...
		// On Modal Close -> Ações quando o modal é fechado
		payPopup.on('hidden.bs.modal', function () {
			jQuery('#form-<?php echo $APPTAG?>-pay').find('#<?php echo $APPTAG?>-pay_id').val(0);
		});
		...
		window.<?php echo $APPTAG?>_formReset = function() {
			...
			project_id.prop('disabled', false).trigger("chosen:updated"); // select
			selectRadio(type, 0);
			jQuery('#<?php echo $APPTAG?>-type-group').removeClass('element-invisible');
			jQuery('#<?php echo $APPTAG?>-price-group').addClass('hide');
			month.val('0').trigger("chosen:updated"); // select
			year.val('<?php echo date('Y')?>'); // select
			due_date.val('');
			selectRadio(create_boleto, 1);
			price.val('').prop('readonly', false);
			description.val('<?php echo JText::_('TEXT_DESCRIPTION'); ?>');
			description1.val('<?php echo JText::_('TEXT_LINE_1'); ?>');
			description2.val('');
			description3.val('');
			description4.val('');
			...
			assessment.val('');
			assessment_note.val('');
			email_subject.val('<?php echo JText::_('MSG_EMAIL_SEND_SUBJECT')?>');
			email_content.val('');
			email_subject_resend.val('<?php echo JText::_('MSG_EMAIL_RESEND_SUBJECT')?>');
			email_content_resend.val('');
			url_boleto.val('');
			bankAccount_info.val('<?php echo JText::_('MSG_BANK_ACCOUNT_INFO')?>');
			paid_date.val('');
			note.val('');

			// Reseta o status da Fatura
			jQuery('#<?php echo $APPTAG?>-status-paid, #<?php echo $APPTAG?>-status-paid').addClass('hide');
		...
		// CUSTOM -> carrega o form de registro de pagamento
		window.<?php echo $APPTAG?>_setPay = function(id) {
			if(typeof id !== "null" && typeof id !== "undefined" && id != "" && id != 0) {
				var formPay = jQuery('#form-<?php echo $APPTAG?>-pay');
	      formPay.find('#<?php echo $APPTAG?>-pay_id').val(id);
				formPay.find('#<?php echo $APPTAG?>-pay_date').val('<?php echo date('d/m/Y')?>');
			}
			return false;
		};
	...
	// métodos controladores das ações referente ao banco de dados e envio de arquivos
			...
			jQuery.ajax({
				...
				success: function(data) {
					jQuery.map( data, function( item ) {
						...
						// App Fields
						selectRadio(type, item.type);
						if(item.type == 1) {
							jQuery('#<?php echo $APPTAG?>-price-group').removeClass('hide');
							jQuery('#<?php echo $APPTAG?>-hosting-group').addClass('hide');
						}
						price.val(item.price);
						project_id.val(item.project_id).prop('disabled', true).trigger("chosen:updated"); // select
						month.val(item.month).trigger("chosen:updated"); // select
						year.val(item.year);
						due_date.val(dateFormat(item.due_date)); // DATE -> conversão de data
						selectRadio(create_boleto, item.create_boleto);
						description.val(item.description);
						description1.val(item.description1);
						description2.val(item.description2);
						description3.val(item.description3);
						description4.val(item.description4);
						hosting.prop('checked', (item.hosting == 0 ? false : true));
						discount.val(item.discount);
						discount_note.val(item.discount_note);
						tax.val(item.tax);
						tax_note.val(item.tax_note);
						assessment.val(item.assessment);
						assessment_note.val(item.assessment_note);
						email_subject.val(item.email_subject);
						email_content.val(item.email_content);
						email_subject_resend.val(item.email_subject_resend);
						email_content_resend.val(item.email_content_resend);
						url_boleto.val(item.url_boleto);
						bankAccount_info.val(item.bankAccount_info);
						paid_date.val(dateFormat(item.paid_date)); // DATE -> conversão de data
						note.val(item.note);

						// Caso a fatura tenha sido enviada, não altera mais o tipo e o valor
						if(item.sent == 1){
							jQuery('#<?php echo $APPTAG?>-type-group, #<?php echo $APPTAG?>-hosting-group').addClass('element-invisible');
							jQuery('#<?php echo $APPTAG?>-price-group').removeClass('hide');
							price.prop('readonly', true);
						}
						// Caso a fatura tenha sido paga, desabilita todo o form
						if(item.paid == 1) {
							mainForm.find('fieldset').prop('disabled', true);
						}
						// ------------------------
							...
							if(res.status > 0) { // se alguma ação for realizada
								...
								if(res.parentField != '' && res.parentFieldVal != '') {
									...
									if(res.parentFieldLabel != '' && res.parentFieldLabel != null) {
										// CUSTOM: add value to 'data-target'
										jQuery(res.parentField).append('<option value="'+res.parentFieldVal+'" data-target-value="'+res.parentFieldUrl+'">'+res.parentFieldLabel+'</option>'); // add valor à lista
						...
						if(res.status == 4) {
							for(i = 0; i < res.ids.length; i++) {
								...
								if((res.state == 2 && e.hasClass('base-icon-ok')) || res.state == 0) {
									...
									if(res.parentField != '' && res.parentFieldVal != '') {
										// CUSTOM: add value to 'data-target'
										jQuery(res.parentField).append('<option value="'+res.parentFieldVal+'" data-target-value="'+res.parentFieldUrl+'">'+res.parentFieldLabel+'</option>');
										jQuery(res.parentField).val(res.parentFieldVal).trigger("chosen:updated").change(); // atualiza o select
									}
								}
							}
		...
		// Send Invoice
		// envia cobrança da fatura
		window.<?php echo $APPTAG?>_sendInvoice = function(itemID, resend, reminder) {
			var msg = (resend) ? '<?php echo JText::_('MSG_RESENDCONFIRM'); ?>' : '<?php echo JText::_('MSG_SENDCONFIRM'); ?>';
			if(reminder) msg = '<?php echo JText::_('MSG_REMINDERCONFIRM'); ?>';
			if(!confirm(msg)) {
				return false;
			} else {
				var cod = '&id='+itemID;
				var st = '&st='+reminder;
				jQuery.ajax({
					url: "<?php echo JURI::root().'templates/base/source/'.$APPNAME.'/'.$APPNAME ?>.model.php?aTag=<?php echo $APPTAG?>&rTag=<?php echo $RTAG?>&task=sendInvoice"+cod+st,
					dataType: 'json',
					type: 'POST',
					cache: false,
					success: function(data){
						jQuery.map( data, function( res ) {
							if(res.status == 1) {
								if(reminder) {
									alert('<?php echo JText::_('MSG_REMINDER_SUCCESS'); ?>');
								} else {
									var eSent = list.find('#<?php echo $APPTAG?>-sent-'+res.id); // enviado
									var eSend = list.find('#<?php echo $APPTAG?>-send-'+res.id); // enviar
									eSend.removeClass('btn-success').addClass('btn-danger').prop('title', '<?php echo JText::_('TEXT_RESEND_INVOICE')?>');
									eSent.removeClass('base-icon-cancel text-danger').addClass('base-icon-ok text-success');
									setTips();
									console.log('enviado!');
								}
							} else {
		...
		// CUSTOM -> registra o pagamento
		window.<?php echo $APPTAG?>_pay = function() {
			var formPay = jQuery('#form-<?php echo $APPTAG?>-pay');
      var payID = formPay.find('#<?php echo $APPTAG?>-pay_id').val();
			dateConvert();
      var dateID = formPay.find('#<?php echo $APPTAG?>-pay_date').val();
      var sender = formPay.find('#<?php echo $APPTAG?>-pay_mail');
      if(dateID == '' || dateID == '0000-00-00') {
        alert('<?php echo JText::_('MSG_DATE_REQUIRED'); ?>');
        return false;
      }
			var cod = '&id='+payID;
			var dt = '&dt='+dateID;
			var st = '&st='+(sender.is(':checked') ? 1 : 0);
			jQuery.ajax({
				url: "<?php echo JURI::root().'templates/base/source/'.$APPNAME.'/'.$APPNAME ?>.model.php?aTag=<?php echo $APPTAG?>&rTag=<?php echo $RTAG?>&task=pay"+cod+dt+st,
				dataType: 'json',
				type: 'POST',
				cache: false,
				success: function(data){
					jQuery.map( data, function( res ) {
						if(res.status == 1) {
							var e = list.find('#<?php echo $APPTAG?>-paid-'+res.id);
							var eSent = list.find('#<?php echo $APPTAG?>-sent-'+res.id); // enviado
							var ePaid = list.find('#<?php echo $APPTAG?>-paid-'+res.id); // pago
							var eSend = list.find('#<?php echo $APPTAG?>-send-'+res.id); // enviar
							var eRemd = list.find('#<?php echo $APPTAG?>-reminder-'+res.id); // lembrete
							var eEdit = list.find('#<?php echo $APPTAG?>-edit-'+res.id); // edit
							ePaid.add(eSent).replaceWith('<span class="base-icon-ok text-success"></span>');
							eRemd.add(eEdit).add(eSend).remove(); // remove os botões 'lembrete', 'enviar' e 'editar'
							payPopup.modal('hide');
						} else {
							alert(res.msg);
						}
...
<div class="base-app<?php echo ' base-list-'.($cfg['listFull'] ? 'full' : 'ajax')?> clearfix">
	...
	<div class="modal fade" id="modal-<?php echo $APPTAG?>-pay" tabindex="-1" role="dialog" aria-labelledby="modal-<?php echo $APPTAG?>payLabel">
		<div class="modal-dialog modal-sm" role="document">
			<div class="modal-content">
				<?php require($APPTAG.'.pay.form.php'); ?>
			</div>
		</div>
	</div>
</div>
