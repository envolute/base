<?php
/* SISTEMA PARA CADASTRO DE TIPOS DE SERVIÇOS
 * AUTOR: IVO JUNIOR
 * EM: 18/02/2016
*/
...
jQuery(function() {
	...
	// APP FIELDS
	var parent_id			= jQuery('#<?php echo $APPTAG?>-parent_id');
	var cparent_id		= jQuery('#<?php echo $APPTAG?>-cparent_id');
	...
	// métodos controladores do formulário
		...
		window.<?php echo $APPTAG?>_formReset = function() {
			...
			parent_id.val('0').trigger("chosen:updated"); // select
			<?php echo $APPTAG?>_getParentList(0);
			cparent_id.val('0');
			name.val('');
			description.val('');
			price.val('<?php echo $cfg['priceDefault']?>');
			ordering.val('0');
			cordering.val('0');
	...
	// métodos controladores das ações referente ao banco de dados e envio de arquivos
		...
		window.<?php echo $APPTAG?>_getParentList = function(id) {
			<?php echo $APPTAG?>_formExecute(true, true, true); // inicia o loader
			jQuery.ajax({
				url: "<?php echo JURI::root().'templates/base/source/'.$APPNAME.'/'.$APPNAME ?>.model.php?task=pList",
				dataType: 'json',
				type: 'POST',
				cache: false,
					...
					jQuery.map( data, function( res, i ) {
						if(res.status == 1) {
							...
							if(i == 0) {
								...
								parent_id.append('<option value="0">- <?php echo JText::_('TEXT_SELECT'); ?> -</option>');
							}
							parent_id.append('<option value="'+res.id+'">'+res.name+'</option>');
						} else {
							if(!itemID) mainForm.find('.set-error').removeClass('hide').text(res.msg);
						}
						if(typeof id !== "null" && typeof id !== "undefined" && id != 0) parent_id.val(id);
						parent_id.trigger("chosen:updated"); // atualiza o select
					});
				},
			...
			jQuery.ajax({
				...
				success: function(data) {
					jQuery.map( data, function( item ) {
						...
						// App Fields
						<?php echo $APPTAG?>_getParentList(item.parent_id);
						cparent_id.val(item.parent_id);
						name.val(item.name);
							...
							if(res.status > 0) { // se alguma ação for realizada
								...
								if(res.parentField != '' && res.parentFieldVal != '') {
									...
									if(res.parentFieldLabel != '' && res.parentFieldLabel != null) {
										jQuery(res.parentField).append('<option value='+res.parentFieldVal+' data-parent-desc="'+res.parentFieldDesc+'" data-parent-price="'+res.parentFieldPrice+'" data-parent-pricefixed="'+res.parentFieldPriceFixed+'">'+res.parentFieldLabel+'</option>'); // add valor à lista
										jQuery(res.parentField).val(res.parentFieldVal).trigger("chosen:updated").change(); // atualiza o select
										// CUSTOM: set fields default
										jQuery('#services-description').val(res.parentFieldDesc);
										jQuery('#services-price').val(res.parentFieldPrice);
									} else {
										jQuery(res.parentField).trigger("chosen:updated");
										// CUSTOM: reset fields default
										jQuery('#services-description').val('');
										jQuery('#services-price').val('');
									}
									// set price format
									setPrice();
								}
						...
						if(res.status == 4) {
							for(i = 0; i < res.ids.length; i++) {
								...
								if((res.state == 2 && e.hasClass('base-icon-ok')) || res.state == 0) {
									...
									if(res.parentField != '' && res.parentFieldVal != '') {
										jQuery(res.parentField).append('<option value='+res.parentFieldVal+' data-parent-desc="'+res.parentFieldDesc+'" data-parent-price="'+res.parentFieldPrice+'" data-parent-pricefixed="'+res.parentFieldPriceFixed+'">'+res.parentFieldLabel+'</option>');
										jQuery(res.parentField).val(res.parentFieldVal).trigger("chosen:updated").change(); // atualiza o select
										// CUSTOM: set fields default
										jQuery('#services-description').val(res.parentFieldDesc);
										jQuery('#services-price').val(res.parentFieldPrice);
