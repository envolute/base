...
// service
$query = 'SELECT * FROM '. $db->quoteName('#__envolute_services') .' WHERE parent_id = 0 AND state = 1 ORDER BY name';
$db->setQuery($query);
	...
	<div class="modal-body">
		<fieldset>
			...
			<div class="row">
				<div class="col-sm-9">
					<div class="form-group">
						<label class="iconTip hasPopover" data-content="<?php echo JText::_('FIELD_LABEL_PARENT_DESC'); ?>"><?php echo JText::_('FIELD_LABEL_PARENT'); ?></label>
						<select name="parent_id" id="<?php echo $APPTAG?>-parent_id" class="form-control auto-tab" data-target="<?php echo $APPTAG?>-name">

						</select>
						<input type="hidden" name="cparent_id" id="<?php echo $APPTAG?>-cparent_id" />
					</div>
				</div>
				<div id="<?php echo $APPTAG?>-order-group" class="col-sm-3">
					<div class="form-group">
				...
				<div class="col-sm-12">
					<div class="form-group">
						...
						<textarea rows="2" name="description" id="<?php echo $APPTAG?>-description" class="form-control"></textarea>
					</div>
				</div>
			</div>
		</fieldset>
