...
// LIST
	...
	function listQuery($pID, $where, $orderList, $cfg) {
		// database connect
		$db = JFactory::getDbo();
		$sql = '
			...
			WHERE
				parent_id = '.$pID.' AND '.$where.$orderList;
		;
		return $sql;
	}

	$query = listQuery(0, $where, $orderList, $cfg);
		...
		<table class="table table-striped table-hover table-condensed">
			<thead>
				<tr>
					...
					<th>'.$$SETOrder(JText::_('FIELD_LABEL_ORDER'), 'T1.ordering', $APPTAG).'</th>
					'.$adminView['head']['actions'].'
				</tr>
			</thead>
			<tbody>
...
// LIST RESULT - Listagem de Resultados
// $item => Object
// $indicator => Sub-item indicator ' -'
// $cfg => Parametros de configuração
function listResult($item, $indicator, $hasAdmin, $APPTAG, $cfg) {
	...
	$row = '
		<tr id="'.$APPTAG.'-item-'.$item->id.'" class="'.$rowState.'">
			'.$adminView['list']['info'].'
			<td>'.$indicator.baseHelper::nameFormat($item->name).'</td>
			<td>'.baseHelper::priceFormat($item->price).'</td>
			<td>'.$indicator.$item->ordering.'</td>
			'.$adminView['list']['actions'].'
		</tr>
	';
	return $row;
...
if($num_rows) : // verifica se existe
	...
	foreach($res as $item) {
		...
		// listagem principal
		$html .= listResult($item, '', $hasAdmin, $APPTAG, $cfg);

		$query = listQuery($item->id, $where, $orderList, $cfg);
		$db->setQuery($query);
		$sub = $db->loadObjectList();
		foreach($sub as $subItem) {
			// listagem de sub-itens
			$html .= listResult($subItem, '<span class="display-inline-block right-space-sm">&raquo</span>', $hasAdmin, $APPTAG, $cfg);
		}
