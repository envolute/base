...
jQuery(function() {
	...
	var task_editId	= jQuery('#<?php echo $APPTAG?>-task_editId');
	...
	// métodos controladores do formulário
		...
		var firstField = task_id; // campo que recebe o focus no carregamento
		...
		popup.on('hidden.bs.modal', function () {
			...
			if(parentFieldId != null) {
				...
				price_hour.val('');
				billable.prop("checked", 0);
			}
			<?php
			// recarrega a listagem
			if($cfg['listFull']) echo $APPTAG.'_listReload(fReload, false);';
			?>
			// CUSTOM -> recarrega a listagem de tarefas
			// obs: apenas se for 'FULL'. A listagem 'AJAX' recarrega na função 'SAVE'
			if(taskList && taskReload) tasks_listReload(true);
		});
		...
		window.<?php echo $APPTAG?>_formReset = function() {
			...
			user_id.val('<?php echo $user->id?>').trigger("chosen:updated"); // select
			date.val('');
			start_hour.val('00:00:00').trigger("chosen:updated"); // select
			end_hour.val('00:00:00').trigger("chosen:updated"); // select
			time.val('00:00:00').trigger("chosen:updated"); // select
			total_time.val('');
			hours.val('');
			closeTask.prop("checked", 0);
			note.val('');
		...
		window.<?php echo $APPTAG?>_setParent = function(id, tList) {
			// define se a listagem de tarefas será recarregada...
			taskList = (typeof tList !== "null" && typeof tList !== "undefined") ? tList : null;
			if(typeof id !== "null" && typeof id !== "undefined" && id != 0) {
				if(parentFieldId != null) {
		...
		task_id.on('change', function() {
			price_hour.val(jQuery(this).find('option:selected').data('priceHour'));
			var fixed = jQuery(this).find('option:selected').data('priceFixed');
			var Tbill = jQuery(this).find('option:selected').data('billable');
			var bill = (fixed == 0 && Tbill == 1) ? 1 : 0;
			billable.prop("checked", bill);
			setActiveState();
		});
	...
	// métodos controladores das ações referente ao banco de dados e envio de arquivos
			...
			jQuery.ajax({
				...
				success: function(data) {
					jQuery.map( data, function( item ) {
						...
						// App Fields
						task_id.val(item.task_id).trigger("chosen:updated"); // selects
						task_editId.val(item.task_id);
						...
						start_hour.val(item.start_hour).trigger("chosen:updated"); // select
						end_hour.val(item.end_hour).trigger("chosen:updated"); // select
						time.val(item.time).trigger("chosen:updated"); // select
						...
						closeTask.prop("checked", (item.closeTask == 1 ? true : false));
						note.val(item.note);
					...
					success: function(data){
						...
						jQuery.map( data, function( res ) {
							if(res.status > 0) { // se alguma ação for realizada
								...
								?>
								// CUSTOM -> recarrega a listagem de tarefas
								// obs: apenas se for 'AJAX'. A listagem 'FULL' recarrega no fechamento da 'modal' do form
								if(taskList == false) tasks_listReload(taskList, false, false, tasksoCHL, tasksrNID, tasksrID);
								else taskReload = true;
		...
		window.<?php echo $APPTAG?>_invoice = function(add) {
	      ...
	      if(!confirm('<?php echo JText::_('MSG_INVOICED_CONFIRM'); ?>')) return false;
				...
				if(!confirm('<?php echo JText::_('MSG_REMOVE_INVOICE_CONFIRM'); ?>')) return false;
				var invID = 0;
			}
