...
header( 'content-type: application/json; charset=utf-8' );
  ...
  if($task != null) :
    ...
    // app
  	...
  	$request['role']       = $input->get('role', '', 'str');
      ...
      if($id) :
        ...
        // validation
        if(!empty($request['project_id']) && !empty($request['contact_id'])) :
          ...
          INSERT INTO '. $db->quoteName($cfg['mainTable']) .'('.
            ...
            $query = 'SELECT client_id FROM '. $db->quoteName('#__envolute_projects') .' WHERE '. $db->quoteName('id') .' = '.$request['project_id'];
            $db->setQuery($query);
            $client = $db->loadResult();
            // o contato pertence ao cliente
            $query = 'SELECT COUNT(*) FROM '. $db->quoteName('#__envolute_rel_clients_contacts') .' WHERE '. $db->quoteName('contact_id') .' = '.$request['contact_id'] .' AND '. $db->quoteName('client_id') .' = '.$client;
            $db->setQuery($query);
            $belong = $db->loadResult();
            // atribui o contato ao cliente, caso ainda não tenha sido atribuído...
            if(!$belong) :
              $query  = '
              INSERT INTO '. $db->quoteName('#__envolute_rel_clients_contacts') .'('.
            ...
            if($request['emailConfirm'] == 1) :
              ...
              FROM '. $db->quoteName($cfg['mainTable']) .' T1
                ...
                JOIN '. $db->quoteName("#__envolute_clients") .' T4
                ON T4.id = T3.client_id
              WHERE '. $db->quoteName('T1.project_id') .' = '. $request['project_id'];
          		$db->setQuery($query);
              ...
              if(!empty($contact->email)) :
                ...
                $subject = JText::sprintf('MSG_REGISTRATION_EMAIL_SUBJECT', baseHelper::nameFormat($contact->project));
                $mailFrom = $config->get('mailfrom');
                // CUSTOM -> prepare e-mail
                $main = $request['main'] == 1 ? ' '.JText::_('TEXT_MAIN') : '';
                $eBody = JText::sprintf('MSG_REGISTRATION_EMAIL_BODY', baseHelper::nameFormat($contact->name), $main, baseHelper::nameFormat($contact->project), baseHelper::nameFormat($contact->client), $domain, $domain, $request['emailInfo'], $sitename);
                $mailBody = baseHelper::mailTemplateDefault($eBody, JText::_('MSG_REGISTRATION_EMAIL_TITLE'));
      ...
      elseif($task == 'cList' && !empty($project) && $project != 0) :
        ...
        // get contacts list
        $query = '
        SELECT T1.id, IF(T2.project_id IS NULL, "", "ADDED") added, T1.name, T1.nickname
        FROM
          '. $db->quoteName('#__envolute_contacts') .' T1
          LEFT JOIN '. $db->quoteName($cfg['mainTable']) .' T2
          ON T2.contact_id = T1.id AND T2.project_id = '.$project.'
        WHERE
          (T1.type = 0 AND T1.state = 1) OR T2.project_id = '.$project.'
        ORDER BY T2.project_id, T1.name';
