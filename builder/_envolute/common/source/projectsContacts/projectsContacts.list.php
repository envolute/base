	...
	foreach($res as $item) {
		...
		$html .= '
			<tr id="'.$APPTAG.'-item-'.$item->id.'" class="'.$rowState.'">
				...
				<td class="text-center">'.($item->type == 1 ? JText::_('FIELD_LABEL_TYPE_TEAM') : JText::_('FIELD_LABEL_CLIENT')).'</td>
				<td class="text-center">'.($item->main == 1 ? '<span class="base-icon-star text-live"></span>' : '').'</td>
				<td>'.$item->role.'</td>
				'.$adminView['list']['actions'].'
			</tr>
		';
