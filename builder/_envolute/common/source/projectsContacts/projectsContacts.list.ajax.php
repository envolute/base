...
if(isset($_SERVER["HTTP_X_REQUESTED_WITH"]) AND strtolower($_SERVER["HTTP_X_REQUESTED_WITH"]) == "xmlhttprequest") :
	...
	$query .= ' ORDER BY '. $db->quoteName('T1.main') .' DESC, '. $db->quoteName('T3.name');
	try {
	...
	if($num_rows) : // verifica se existe
		...
		foreach($res as $item) {
			...
			$desc = !empty($item->role) ? '<br /><em class="small text-muted font-featured cursor-help hasTooltip" title="'.JText::_('FIELD_LABEL_ROLE').'">('.baseHelper::nameFormat($item->role).')</em> ' : '';
			...
			$html .= '
				<li class="'.$rowState.'">
					'.$main.baseHelper::nameFormat($item->contact).$desc.'
					<span class="btn-group pull-right">'.$btnEdit.$btnDelete.'</span>
				</li>
			';
		}
		$html .= '</ul>';
