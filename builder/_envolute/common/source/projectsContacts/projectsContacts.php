...
jQuery(function() {
	...
	// métodos controladores do formulário
		...
		var firstField = project_id; // campo que recebe o focus no carregamento
		...
		window.<?php echo $APPTAG?>_formReset = function() {
			...
			project_id.prop('disabled', false).trigger("chosen:updated");
						...
						if(res.status != 0) {
							...
							if(i == 0) {
								...
								if(res.status == 1) {
									...
									contact_id.append('<option value="0">- <?php echo JText::_('TEXT_ADD_PROJECTS_CONTACTS'); ?> -</option>');
									projectClientId = 0;
								}
							}
			...
			jQuery.ajax({
				...
				success: function(data) {
					jQuery.map( data, function( item ) {
						...
						<?php echo $APPTAG?>_getContactsList(item.project_id, item.contact_id);
