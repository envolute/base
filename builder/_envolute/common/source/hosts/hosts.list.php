...
// LIST
	...
	$query = '
		SELECT SQL_CALC_FOUND_ROWS
			...
			'. $db->quoteName('T3.price') .' pricePlan,
			'. $db->quoteName('T1.price') .',
			'. $db->quoteName('T1.billable') .',
			'. $db->quoteName('T1.username') .',
			'. $db->quoteName('T1.pass_def') .',
		...
		<table class="table table-striped table-hover table-condensed">
			<thead>
				<tr>
					...
					<th>'.$$SETOrder(JText::_('FIELD_LABEL_USERNAME'), 'T1.username', $APPTAG).'</th>
					<th>'.$$SETOrder(JText::_('FIELD_LABEL_PLAN'), 'T3.name', $APPTAG).'</th>
					<th>'.$$SETOrder(JText::_('FIELD_LABEL_SPACE'), 'T3.space', $APPTAG).'</th>
...
if($num_rows) : // verifica se existe
	...
	foreach($res as $item) {
		...
		// GET DOMAINS
		$query = 'SELECT '. $db->quoteName('domain') .' FROM '. $db->quoteName('#__envolute_domains') .' WHERE host_id = '.$item->id.' AND state = 1 ORDER BY main DESC, domain ASC';
		$db->setQuery($query);
		$domains = $db->loadColumn();
		$domain = count($domains) > 0 ? '<div class="small text-muted"><span class="cursor-help hasTooltip" title="'.JText::_('MSG_DOMAINS').'">'.implode(', ', $domains).'</span></div>' : '';
		...
		if(!empty($item->description)) :
			$desc = ' <span class="base-icon-info-circled hasTooltip" title="'.$item->description.'"></span>';
		endif;
		$spaceTotal += $item->space;
		if($item->billable) :
			if($item->price != '0.00' && !empty($item->price)) :
				$price = 'R$ <span style="text-decoration:line-through;" class="text-muted">'.baseHelper::priceFormat($item->pricePlan).'</span><div class="small">&raquo; <span class="cursor-help hasTooltip" title="'.JText::_('FIELD_LABEL_EXTRA_PRICE').'<br />'.JText::_('FIELD_LABEL_EXTRA_PRICE_DESC').'">'.baseHelper::priceFormat($item->price).'</span></div>';
				$priceTotal += $item->price;
			else :
				$price = baseHelper::priceFormat($item->pricePlan, false, 'R$ ');
				$priceTotal += $item->pricePlan;
			endif;
			$priceTotal = strval(number_format($priceTotal, 2));
		else :
			$price = '<span class="text-muted cursor-help hasTooltip" title="'.JText::_('MSG_NOT_BILLABLE').'">0,00</span>';
		endif;
		$rowState = $item->state == 0 ? 'danger' : '';
		$html .= '
			<tr id="'.$APPTAG.'-item-'.$item->id.'" class="'.$rowState.'">
				'.$adminView['list']['info'].'
				<td>'.baseHelper::nameFormat($item->project).$desc.$domain.'</td>
				<td>'.$item->username.'<div class="small text-muted font-featured"><span class="cursor-help hasTooltip" title="'.JText::_('FIELD_LABEL_PASS_DEFAULT_DESC').'">'.$item->pass_def.'</span></div></td>
				<td>'.$item->plan.'</td>
				<td>'.$item->space.'Gb</td>
				<td>'.$price.'</td>
				'.$adminView['list']['actions'].'
	...
	$html .= '
		<tr class="warning strong">
			<td colspan="5">Total</td>
			<td>'.$spaceTotal.'GB</td>
			<td colspan="3">R$ '.baseHelper::priceFormat($priceTotal).'</td>
		</tr>
	';
