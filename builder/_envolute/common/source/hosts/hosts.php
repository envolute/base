<?php
/* SISTEMA PARA CADASTRO DE HOSPEDAGENS
 * AUTOR: IVO JUNIOR
 * EM: 18/02/2016
*/
...
jQuery(function() {
	...
	var plan_id				= jQuery('#<?php echo $APPTAG?>-plan_id');
	...
	var pass_def			= jQuery('#<?php echo $APPTAG?>-pass_def');
	...
	// métodos controladores do formulário
		...
		window.<?php echo $APPTAG?>_formReset = function() {
			...
			plan_id.val('0').trigger("chosen:updated"); // select
			price.val('');
			...
			jQuery.ajax({
				...
				success: function(data) {
					jQuery.map( data, function( item ) {
						...
						plan_id.val(item.plan_id).trigger("chosen:updated"); // selects
						price.val(item.price);
						billable.prop("checked", (item.billable == 1 ? true : false));
						username.val(item.username);
						pass_def.val(item.pass_def);
