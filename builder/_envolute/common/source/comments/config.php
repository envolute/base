...
// crud's main table
  $cfg['mainTable']         = '#__envolute_comments';
...
// crud's relation actions
  ...
  if(!$ajaxRequest) :
    ...
    // comment APP term -> termo referente ao APP que recebe o comentário. Ex: 'No Chamado, Na tarefa'
    $_SESSION[$RTAG.'RelEmailAPPTerm'] = isset(${$APPTAG.'RelEmailAPPTerm'}) ? ${$APPTAG.'RelEmailAPPTerm'} : '';
    // relationed table APP -> tabela do APP relacionado. Ex: #__envolute_tasks, #__envolute_tickets
    $_SESSION[$RTAG.'RelTableAPP'] = isset(${$APPTAG.'RelTableAPP'}) ? ${$APPTAG.'RelTableAPP'} : '';
    // field for label -> Campo da tabela de relacionamento que será usado como referencia ao item (name, title, subject...)
    $_SESSION[$RTAG.'RelTableAPPLabel'] = isset(${$APPTAG.'RelTableAPPLabel'}) ? ${$APPTAG.'RelTableAPPLabel'} : '';
    // relationed table APP Contacts -> tabela de contatos do APP relacionado. Ex: #__envolute_contacts...
    $_SESSION[$RTAG.'TableAPPContacts'] = isset(${$APPTAG.'TableAPPContacts'}) ? ${$APPTAG.'TableAPPContacts'} : '';
    // relation table APP Contacts -> tabela de contatos do APP relacionado. Ex: #__envolute_rel_tasks_contacts...
    $_SESSION[$RTAG.'RelTableAPPContacts'] = isset(${$APPTAG.'RelTableAPPContacts'}) ? ${$APPTAG.'RelTableAPPContacts'} : '';
  endif;
