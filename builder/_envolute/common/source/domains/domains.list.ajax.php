	...
	if(!empty($rID) && $rID !== 0) :
		if(isset($_SESSION[$RTAG.'RelTable']) && !empty($_SESSION[$RTAG.'RelTable'])) :
			$query .= ' FROM '.
				...
				JOIN '. $db->quoteName($cfg['mainTable'].'_providers') .' T2
				ON '.$db->quoteName('T2.id') .' = T1.provider
			...
			$html .= '
				<li class="'.$rowState.'">
					...
					<small class="text-muted font-featured">
						<span class="cursor-help hasTooltip" title="'.JText::_('TEXT_DUE_DATE').'">'.baseHelper::dateFormat($item->date, 'd/m').'</span> - <span class="cursor-help hasTooltip" title="'.JText::_('FIELD_LABEL_PROVIDER').'">'.baseHelper::nameFormat($item->provider).'</span>
					</small>
				</li>
			';
		}
