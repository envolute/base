...
// LIST
	...
	$query = '
		...
		FROM
			...
			JOIN '. $db->quoteName('#__envolute_hosts') .' T2
			ON T2.id = T1.host_id
			LEFT JOIN '. $db->quoteName($cfg['mainTable'].'_providers') .' T3
			ON T3.id = T1.provider
		...
		<table class="table table-striped table-hover table-condensed">
			<thead>
				<tr>
					...
					<th>'.$$SETOrder(JText::_('FIELD_LABEL_HOST'), 'T4.name', $APPTAG).'</th>
	...
	foreach($res as $item) {
		...
		$html .= '
			<tr id="'.$APPTAG.'-item-'.$item->id.'" class="'.$rowState.'">
				'.$adminView['list']['info'].'
				<td>'.$main.$item->domain.'</td>
				<td>'.baseHelper::nameFormat($item->host).'</td>
				<td>'.baseHelper::dateFormat($item->date, 'd/m/Y', true, '-').'</td>
				<td>'.(!empty($item->provider) ? baseHelper::nameFormat($item->provider) : '-').'</td>
				'.$adminView['list']['actions'].'
			</tr>
		';
	}
