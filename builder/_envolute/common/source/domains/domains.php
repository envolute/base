<?php
/* SISTEMA PARA CADASTRO DE DOMÍNIOS
 * AUTOR: IVO JUNIOR
 * EM: 18/02/2016
*/
...
jQuery(function() {
	...
	// APP FIELDS
	var host_id 			= jQuery('#<?php echo $APPTAG?>-host_id');
	var domain				= jQuery('#<?php echo $APPTAG?>-domain');
	var date					= jQuery('#<?php echo $APPTAG?>-date');
	var provider			= jQuery('#<?php echo $APPTAG?>-provider');
	...
	// métodos controladores do formulário
		...
		var firstField = domain; // campo que recebe o focus no carregamento
		...
		window.<?php echo $APPTAG?>_formReset = function() {
			...
			domain.val('');
			date.val('');
			provider.val('').trigger("chosen:updated"); // select
		...
		window.<?php echo $APPTAG?>_editHost = function() {
			var itemID = host_id.val();
			if(itemID != '' && itemID != 0) hosts_loadEditFields(itemID, false, false);
				...
				success: function(data){
					...
					jQuery.map( data, function( res, i ) {
						if(res.status == 1) {
							// remove all options
							if(i == 0) {
								host_id.find('option').remove();
								// init new options list
								host_id.append('<option value="0">- <?php echo JText::_('TEXT_SELECT'); ?> -</option>');
							}
							host_id.append('<option value="'+res.id+'">'+res.name+'</option>');
						}
						if(typeof id !== "null" && typeof id !== "undefined" && id != 0) host_id.val(id);
						host_id.trigger("chosen:updated"); // atualiza o select
					});
				},
			...
			jQuery.ajax({
				...
				success: function(data) {
					jQuery.map( data, function( item ) {
						...
						// App Fields
						<?php echo $APPTAG?>_getParentList(item.host_id);
						domain.val(item.domain);
						date.val(dateFormat(item.date)); // DATE -> conversão de data
						provider.val(item.provider).trigger("chosen:updated"); // selects
