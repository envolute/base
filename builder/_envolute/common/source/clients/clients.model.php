...
header( 'content-type: application/json; charset=utf-8' );
  ...
  if($task != null) :
    ...
    // app
  	...
  	$request['proposal']     = $input->get('proposal', 0, 'int');
  	...
  	$request['email_payment'] = $input->get('email_payment', '', 'string');
    ...
    $request['date'] = !empty($request['date']) ? $request['date'] : date('Y-m-d');
  	$request['due_date']     = $input->get('due_date', 1, 'int');
      ...
      if($id) :
        ...
        // COUNT LIST IDS
          ...
          foreach($list as $item) {
              ...
              // App Fields
      				...
      				'email'         => $item->email,
      				'email_payment' => $item->email_payment,
            ...
            $db->quoteName('proposal') 	.'='. $request['proposal'] .','.
    				...
    				$db->quoteName('email_payment') 	.'='. $db->quote($request['email_payment']) .','.
            ...
            if(!empty($_SESSION[$RTAG.'RelTable']) && !empty($_SESSION[$RTAG.'AppNameId'])) :
              ...
              // isso é RECOMENDÁVEL sempre que houver um ou mais relacionamentos
                ...
                // $query = 'DELETE FROM '. $db->quoteName('#__envolute_projects') .' WHERE '. $db->quoteName('client_id') .' IN ('.$ids.')';
                // $db->setQuery($query);
      					// $db->execute();
            endif;
        ...
        if(!empty($request['name']) && !empty($request['email'])) :
          ...
          // Prepare the insert query
            ...
            $db->quoteName('type') .','.
            $db->quoteName('proposal') .','.
    				$db->quoteName('name') .','.
    				$db->quoteName('name_company') .','.
    				$db->quoteName('email') .','.
    				$db->quoteName('email_payment') .','.
            ...
            $db->quote($request['email_payment']) .','.
