		...
		<table class="table table-striped table-hover table-condensed">
			<thead>
				<tr>
					...
					<th width="90px">'.$$SETOrder(JText::_('FIELD_LABEL_SPACE'), 'T1.space', $APPTAG).'</th>
					<th width="80px">'.$$SETOrder(JText::_('FIELD_LABEL_PRICE'), 'T1.price', $APPTAG).'</th>
					'.$adminView['head']['actions'].'
				</tr>
			</thead>
			<tbody>
	...
	foreach($res as $item) {
		...
		$html .= '
			<tr id="'.$APPTAG.'-item-'.$item->id.'" class="'.$rowState.'">
				'.$adminView['list']['info'].'
				<td>
					'.$main.baseHelper::nameFormat($item->name).'
					<div class="text-xs text-muted font-featured">'.$item->description.'</div>
				</td>
				<td>'.$item->space.' Gb</td>
