...
// LIST
	...
	$query = '
		...
		FROM
			...
			ON T2.id = T1.task_id
			JOIN '. $db->quoteName('#__envolute_contacts') .' T3
	...
	<form id="form-list-'.$APPTAG.'" method="post">
		<table class="table table-striped table-hover table-condensed">
			<thead>
				<tr>
					'.$adminView['head']['info'].'
					<th>'.$$SETOrder(JText::_('FIELD_LABEL_TASK'), 'T2.title', $APPTAG).'</th>
...
if($num_rows) : // verifica se existe
	...
	foreach($res as $item) {
		...
		$task = '<span class="font-condensed cursor-help hasPopover" data-placement="top" data-content="<strong>'.JText::_('FIELD_LABEL_SERVICE').'</strong><div class=\'small\'>'.baseHelper::nameFormat($item->service).'</div><strong>'.JText::_('FIELD_LABEL_PROJECT').'</strong><div class=\'small\'>'.baseHelper::nameFormat($item->project).'</div>">'.baseHelper::nameFormat($item->title).'</span> ';
