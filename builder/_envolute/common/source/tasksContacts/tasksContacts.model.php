  ...
  if($task != null) :
    ...
    // app
      ...
      if($id) :
        ...
        // validation
        if(!empty($request['task_id']) && !empty($request['contact_id'])) :
          ...
          INSERT INTO '. $db->quoteName($cfg['mainTable']) .'('.
            ...
            if($request['emailConfirm'] == 1) :
              ...
              FROM '. $db->quoteName($cfg['mainTable']) .' T1
                ...
                JOIN '. $db->quoteName("#__envolute_tasks") .' T3
                ON T3.id = T1.task_id
                JOIN '. $db->quoteName("#__envolute_projects") .' T4
                ON T4.id = T3.project_id
                JOIN '. $db->quoteName("#__envolute_clients") .' T5
                ON T5.id = T4.client_id
              WHERE '. $db->quoteName('T1.task_id') .' = '. $request['task_id'];
          		$db->setQuery($query);
              ...
              if(!empty($contact->email)) :
                ...
                $subject = JText::sprintf('MSG_REGISTRATION_EMAIL_SUBJECT', '[#'.$contact->task_id.'] '.baseHelper::nameFormat($contact->task));
                $mailFrom = $config->get('mailfrom');
                // CUSTOM -> prepare e-mail
                $eBody = JText::sprintf('MSG_REGISTRATION_EMAIL_BODY', baseHelper::nameFormat($contact->name), '[#'.$contact->task_id.'] '.baseHelper::nameFormat($contact->task), baseHelper::nameFormat($contact->project), baseHelper::nameFormat($contact->client), $domain, $request['emailInfo'], $sitename);
      ...
      elseif($task == 'cList') :
        ...
        // get contacts list
        $query = '
        SELECT T1.id, IF(T2.task_id IS NULL, "", "ADDED") added, T1.name, T1.nickname
        FROM
          '. $db->quoteName('#__envolute_contacts') .' T1
          LEFT JOIN '. $db->quoteName($cfg['mainTable']) .' T2
          ON T2.contact_id = T1.id AND T2.task_id = '.$parent.'
        WHERE
          (T1.type = 0 AND T1.state = 1) OR T2.task_id = '.$parent.'
        ORDER BY T2.task_id, T1.name';
