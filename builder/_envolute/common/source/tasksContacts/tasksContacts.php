...
jQuery(function() {
	...
	// métodos controladores do formulário
		...
		window.<?php echo $APPTAG?>_formReset = function() {
			...
			task_id.prop('disabled', false).trigger("chosen:updated");
		...
		window.<?php echo $APPTAG?>_setParent = function(id) {
			if(typeof id !== "null" && typeof id !== "undefined" && id != 0) {
				...
				<?php echo $APPTAG?>_getContactsList(id);
				btnPrev.remove();
				btnNext.remove();
			}
		};
			...
			jQuery.ajax({
				...
				success: function(data) {
					jQuery.map( data, function( item ) {
						...
						// App Fields
						task_id.val(item.task_id).prop('disabled', true).trigger("chosen:updated"); // select;
						<?php echo $APPTAG?>_getContactsList(item.task_id, item.contact_id);
