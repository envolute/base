...
// LIST
	...
	$query = '
		...
		FROM
			...
			LEFT JOIN '. $db->quoteName('#__envolute_clients') .' T2
			ON T2.id = T1.client_id
			LEFT JOIN '. $db->quoteName($cfg['mainTable'].'_categories') .' T3
		...
		<table class="table table-striped table-hover table-condensed">
			<thead>
				<tr>
					...
					<th><span class="base-icon-calendar"></span> '.$$SETOrder(JText::_('FIELD_LABEL_START_DATE'), 'T1.start_date', $APPTAG).'</th>
					<th>'.$$SETOrder(JText::_('FIELD_LABEL_PORTFOLIO'), 'T1.portfolio', $APPTAG).'</th>
					'.$adminView['head']['actions'].'
				</tr>
			</thead>
			<tbody>
	...
	foreach($res as $item) {
		...
		if($cfg['hasUpload']) :
			...
			for($i = 0; $i < count($files[$item->id]); $i++) {
				if(!empty($files[$item->id][$i]->filename)) :
					// CUSTOM
					// separa as imagens [até o index 3]
					if($files[$item->id][$i]->index < 4) :
						$listImages = '/images/uploads/'.$APPNAME.'/'.$files[$item->id][$i]->filename;
					// dos anexos [a partir do index 4]
					else :
						$listFiles .= '
		...
		if($hasAdmin) :
			...
			$adminView['list']['actions'] = '
				...
				<td class="text-center">
					<a href="#" class="btn btn-xs btn-success" onclick="projectsContacts_setParent('.$item->id.')" data-toggle="modal" data-target="#modal-projectsContacts" data-backdrop="static" data-keyboard="false"><span class="base-icon-user-plus hasTooltip" title="'.JText::_('MSG_INSERT_CONTACT').'"></span></a>
					<a href="#" class="btn btn-xs btn-success" onclick="links_setParent('.$item->id.')" data-toggle="modal" data-target="#modal-links" data-backdrop="static" data-keyboard="false"><span class="base-icon-link hasTooltip" title="'.JText::_('MSG_INSERT_LINK').'"></span></a>
					<a href="#" class="btn btn-xs btn-success" onclick="hosts_setParent('.$item->id.')" data-toggle="modal" data-target="#modal-hosts" data-backdrop="static" data-keyboard="false"><span class="base-icon-database hasTooltip" title="'.JText::_('MSG_INSERT_HOST').'"></span></a>
					<a href="#" class="btn btn-xs btn-success" onclick="domains_setParent(0)" data-toggle="modal" data-target="#modal-domains" data-backdrop="static" data-keyboard="false"><span class="base-icon-globe hasTooltip" title="'.JText::_('MSG_INSERT_DOMAIN').'"></span></a>
		...
		$portfolio = '<span class="'.($item->portfolio == 1 ? 'base-icon-ok text-success' : 'base-icon-cancel text-danger').'"></span>';
		...
		$html .= '
			<tr id="'.$APPTAG.'-item-'.$item->id.'" class="'.$rowState.'">
				...
				<td>
					...
					'.$info.'
					<a href="#" class="base-icon-user text-muted hasTooltip" onclick="projectsContacts_listReload(false, false, false, true, \'project_id\', '.$item->id.')" data-toggle="modal" data-target="#modal-list-projectsContacts" title="'.JText::_('MSG_VIEW_CONTACTS').'"></a>
					<a href="#" class="base-icon-link text-muted hasTooltip" onclick="links_listReload(false, false, false, true, \'project_id\', '.$item->id.')" data-toggle="modal" data-target="#modal-list-links" title="'.JText::_('MSG_VIEW_LINKS').'"></a>
					<a href="#" class="base-icon-database text-muted hasTooltip" onclick="hosts_listReload(false, false, false, true, \'project_id\', '.$item->id.')" data-toggle="modal" data-target="#modal-list-hosts" title="'.JText::_('MSG_VIEW_HOSTS').'"></a>
					<a href="#" class="base-icon-megaphone text-muted hasTooltip" onclick="tickets_listReload(false, false, false, true, \'project_id\', '.$item->id.')" data-toggle="modal" data-target="#modal-list-tickets" title="'.JText::_('MSG_VIEW_TICKETS').'"></a>
					'.(!empty($listFiles) ? '<span class="left-space-xs right-space-xs set-border-left border-default"></span> '.$listFiles : '').'
				</td>
				<td>
					'.baseHelper::nameFormat($item->client).'
					<div><small class="text-muted font-featured cursor-help hasTooltip" title="'.JText::_('FIELD_LABEL_CATEGORY').'">'.baseHelper::nameFormat($item->category).'</small></div>
				</td>
				<td>'.baseHelper::dateFormat($item->start_date, 'd/m/Y', true, '-').'</td>
				<td>'.$portfolio.'</td>
				'.$adminView['list']['actions'].'
			</tr>
		';
