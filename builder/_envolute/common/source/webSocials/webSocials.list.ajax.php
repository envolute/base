...
if(isset($_SERVER["HTTP_X_REQUESTED_WITH"]) AND strtolower($_SERVER["HTTP_X_REQUESTED_WITH"]) == "xmlhttprequest") :
	...
	if($num_rows) : // verifica se existe
		...
		foreach($res as $item) {
			...
			$username = !empty($item->username) ? '<small class="text-live"> ('.$item->username.')</small>' : '';
			...
			$html .= '
				<li class="'.$rowState.'">
					...
					<div>'.baseHelper::nameFormat($item->name).$username.'</div>
					<a href="'.$item->page.'" target="_blank" class="small text-muted font-featured new-window">'.$item->page.'</a>
				</li>
			';
		}
		$html .= '</ul>';
