...
// EXTRAFIELDS
	...
	// load API
	$extrafields = JPATH_BASE.'/templates/base/source/extrafields_category.php';
	...
	if($this->showImage == 1) :
		...
		if(!is_null($img)) :
			...
			$image = '<div class="item-image obj-to-'.htmlspecialchars($imgfloat).'" style="width:'.$this->imgW.'px;">';
				...
				if($this->imgZoom) :
					$image .= '<a class="img-zoom set-modal" rel="image-preview" href="'.$JRoot.htmlspecialchars($img).'" data-modal-title="'.$this->escape($this->item->title).'">Zoom</a>';
				endif;
