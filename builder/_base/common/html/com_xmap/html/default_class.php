    ...
    */
    function printNode( &$node )
    {
        ...
        if ($this->canEdit) {
            ...
            echo ' <a href= "#" class="xmapexcl '.$class.' pull-right left-space-sm" rel="{uid:\''.$node->uid.'\',itemid:'.$node->id.'}">'.$img.'</a>';
        }
        $this->count++;
    ...
    */
    function changeLevel( $level ) {
        if ( $level > 0 ) {
            ...
            $this->_openList = "\n<ul class=\"level_".$this->level." list clearfix\">\n";
            $this->_closeItem = '';
    ...
    function startMenu(&$menu) {
        echo '<div class="xmap-menu-container">';
        if( $this->sitemap->params->get('columns') > 1)            // use columns
            echo '<div style="width:'.$this->_width.'%;">';
        if( $this->sitemap->params->get('show_menutitle'))         // show menu titles
            echo '<h5 class="menutitle"><span class="base-icon-menu"></span> '.$menu->name.'</h2>';
    }

    function endMenu(&$menu) {
        $sitemap=&$this->sitemap;
        $this->_closeItem='';
        if( $sitemap->params->get('columns')> 1) echo "</div>\n";
        echo '</div>';
    }
}