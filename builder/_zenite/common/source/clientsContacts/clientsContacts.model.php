    ...
    // app
      ...
      if($id) :
        ...
        // COUNT LIST IDS
          ...
          foreach ($regs as $reg) {
            // select projects_ids
            $query2 = 'SELECT '. $db->quoteName('id') .' FROM '. $db->quoteName('#__zenite_projects') .' WHERE '. $db->quoteName('client_id') .' = '.$reg->client_id;
            $db->setQuery($query2);
            $projs = $db->loadObjectList();
            foreach ($projs as $proj) {
              // select IDs of relation projects/contacts
              $query3 = 'SELECT '. $db->quoteName('id') .' FROM '. $db->quoteName('#__zenite_rel_projects_contacts') .' WHERE '. $db->quoteName('project_id') .' = '. $proj->id .' AND '. $db->quoteName('contact_id') .' = '.$reg->contact_id;
              $db->setQuery($query3);
              $rel = $db->loadResult();
            ...
            if(!empty($_SESSION[$RTAG.'RelTable']) && !empty($_SESSION[$RTAG.'AppNameId'])) :
              ...
              if(count($idProjContacts)) :
                $query = 'DELETE FROM '. $db->quoteName('#__zenite_rel_projects_contacts') .' WHERE '. $db->quoteName('id') .' IN('.implode(',', $idProjContacts).')';
                $db->setQuery($query);
      					$db->execute();
              endif;
            endif;
          ...
          INSERT INTO '. $db->quoteName($cfg['mainTable']) .'('.
            ...
            if($request['emailConfirm'] == 1) :
              ...
              FROM '. $db->quoteName($cfg['mainTable']) .' T1
                JOIN '. $db->quoteName("#__zenite_contacts") .' T2
      ...
      elseif($task == 'cList' && !empty($client) && $client != 0) :
        ...
        FROM
          '. $db->quoteName('#__zenite_contacts') .' T1
