...
if(!isset($pID) || empty($pID) || $pID == 0) :
	...
	if($num_rows) : // verifica se existe
		...
		foreach($res as $item) {
			...
			$age_type = $item->age_type ? ' <span class="base-icon-birthday text-live cursor-help hasTooltip" title="'.JText::_('FIELD_LABEL_AGE_TYPE_1_DESC').'"></span>' : '';
			...
			$html .= '
				<tr id="'.$APPTAG.'-item-'.$item->id.'" class="'.$rowState.'">
					...
					<td>'.$faixa.$age_type.'</td>
					<td>'.JText::_($gender).'</td>
