...
jQuery(function() {
	...
	var disability_id = jQuery('#<?php echo $APPTAG?>-disability_id');
	var distance 			= jQuery('#<?php echo $APPTAG?>-distance');
	var distance_unit	= mainForm.find('input[name=distance_unit]:radio'); // radio group
	var gender				= mainForm.find('input[name=gender]:radio'); // radio group
	var description		= jQuery('#<?php echo $APPTAG?>-description');
	var min_age 			= jQuery('#<?php echo $APPTAG?>-min_age');
	var max_age 			= jQuery('#<?php echo $APPTAG?>-max_age');
	var age_type			= mainForm.find('input[name=age_type]:radio'); // radio group
	var price 				= jQuery('#<?php echo $APPTAG?>-price');
	var limit					= jQuery('#<?php echo $APPTAG?>-limit');
	var link_map 			= jQuery('#<?php echo $APPTAG?>-link_map');
	...
	// métodos controladores do formulário
		...
		window.<?php echo $APPTAG?>_formReset = function() {
			...
			category_id.val('0').trigger("chosen:updated"); // select
			disability_id.val('0').trigger("chosen:updated"); // select
			distance.val('');
			selectRadio(distance_unit, 1); // radio
			selectRadio(gender, 0); // radio
			description.val('');
			min_age.val('');
			max_age.val('');
			selectRadio(age_type, 0); // radio
			price.val('');
			limit.val('');
			link_map.val('');
			...
			jQuery.ajax({
				...
				success: function(data) {
					jQuery.map( data, function( item ) {
						...
						// App Fields
						project_id.val(item.project_id).trigger("chosen:updated"); // select
						category_id.val(item.category_id).trigger("chosen:updated"); // select
						disability_id.val(item.disability_id).trigger("chosen:updated"); // select
						distance.val(item.distance);
						selectRadio(distance_unit, item.distance_unit); // radio
						selectRadio(gender, item.gender); // radio
						description.val(item.description);
						min_age.val(item.min_age);
						max_age.val(item.max_age);
						selectRadio(age_type, item.age_type); // radio
						price.val(item.price);
						limit.val(item.limit);
						link_map.val(item.link_map);
