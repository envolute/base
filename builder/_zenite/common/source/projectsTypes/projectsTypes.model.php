...
header( 'content-type: application/json; charset=utf-8' );
  ...
  if($task != null) :
    ...
    $request['project_id']   = $input->get('project_id', 0, 'int');
  	$request['category_id']  = $input->get('category_id', 0, 'int');
  	$request['disability_id']= $input->get('disability_id', 0, 'int');
  	$request['distance']     = $input->get('distance', '', 'string');
  	$request['distance_unit']  = $input->get('distance_unit', 1, 'int');
    $request['gender']       = $input->get('gender', 0, 'int');
  	$request['description']  = $input->get('description', '', 'string');
  	$request['min_age']      = $input->get('min_age', 0, 'int');
  	$request['max_age']      = $input->get('max_age', 0, 'int');
    $request['age_type']     = $input->get('age_type', 0, 'int');
  	$request['price']        = $input->get('price', '', 'float');
    $request['limit']        = $input->get('limit', 0, 'int');
  	$request['link_map']     = $input->get('link_map', '', 'string');
      ...
      if($id) :
        ...
        // COUNT LIST IDS
          ...
          foreach($list as $item) {
            ...
            $db->quoteName('category_id') 	.'='. $request['category_id'] .','.
            $db->quoteName('disability_id') 	.'='. $request['disability_id'] .','.
            $db->quoteName('distance') 	.'='. $db->quote($request['distance']) .','.
            $db->quoteName('distance_unit') 	.'='. $request['distance_unit'] .','.
            $db->quoteName('gender') 	.'='. $request['gender'] .','.
            $db->quoteName('description') 	.'='. $db->quote($request['description']) .','.
            $db->quoteName('min_age') 	.'='. $db->quote($request['min_age']) .','.
            $db->quoteName('max_age') 	.'='. $db->quote($request['max_age']) .','.
            $db->quoteName('age_type') 	.'='. $request['age_type'] .','.
            $db->quoteName('price') 	.'='. $db->quote($request['price']) .','.
            $db->quoteName('limit') 	.'='. $request['limit'] .','.
            $db->quoteName('link_map') 	.'='. $db->quote($request['link_map']) .','.
            ...
            if(!empty($_SESSION[$RTAG.'RelTable']) && !empty($_SESSION[$RTAG.'AppNameId'])) :
              ...
              $query = 'DELETE FROM '. $db->quoteName('#__zenite_rel_clients_socials') .' WHERE '. $db->quoteName('social_id') .' IN ('.$ids.')';
              $db->setQuery($query);
    					$db->execute();
              // CONTACTS -> remove os registros relacionados aos contatos
              $query = 'DELETE FROM '. $db->quoteName('#__zenite_rel_contacts_socials') .' WHERE '. $db->quoteName('social_id') .' IN ('.$ids.')';
              $db->setQuery($query);
    					$db->execute();
            endif;
        ...
        // validation
        if($request['project_id'] != 0 && $request['category_id'] != 0 && !empty($request['distance'])) :
          ...
          INSERT INTO '. $db->quoteName($cfg['mainTable']) .'('.
            ...
            $db->quoteName('disability_id') .','.
            $db->quoteName('distance') .','.
            $db->quoteName('distance_unit') .','.
            $db->quoteName('gender') .','.
            $db->quoteName('description') .','.
            $db->quoteName('min_age') .','.
            $db->quoteName('max_age') .','.
            $db->quoteName('age_type') .','.
            $db->quoteName('price') .','.
    				$db->quoteName('limit') .','.
            $db->quoteName('link_map') .','.
    				$db->quoteName('state') .','.
    				$db->quoteName('created_by')
    			.') VALUES ('.
            $request['project_id'] .','.
            $request['category_id'] .','.
            $request['disability_id'] .','.
            $db->quote($request['distance']) .','.
            $request['distance_unit'] .','.
            $request['gender'] .','.
            $db->quote($request['description']) .','.
            $db->quote($request['min_age']) .','.
            $db->quote($request['max_age']) .','.
            $request['age_type'] .','.
            $db->quote($request['price']) .','.
            $request['limit'] .','.
            $db->quote($request['link_map']) .','.
    				$request['state'] .','.
    				$user->id
    			.')';
