...
if(!isset($pID) || empty($pID) || $pID == 0) :
	...
	// LIST
		...
		$query = '
			SELECT SQL_CALC_FOUND_ROWS
				...
				'. $db->quoteName('T1.owner') .',
				'. $db->quoteName('T1.status') .',
				'. $db->quoteName('T1.status_date') .',
		...
		<form id="form-list-'.$APPTAG.'" method="post">
			<table class="table table-striped table-hover table-condensed">
				<thead>
					<tr>
						'.$adminView['head']['info'].'
						<th>'.JText::_('FIELD_LABEL_COD').'</th>
						<th class="hidden-print">'.JText::_('FIELD_LABEL_PRICE').'</th>
						<th>'.JText::_('FIELD_LABEL_OWNER').'</th>
						<th width="60" class="text-center hidden-print">'.$$SETOrder(JText::_('FIELD_LABEL_STATUS'), 'T1.status', $APPTAG).'</th>
						<th>'.$$SETOrder(JText::_('FIELD_LABEL_PROJECT'), 'T1.project_id', $APPTAG).'</th>
						<th class="hidden-print">'.$$SETOrder(JText::_('FIELD_LABEL_STATUS_DATE'), 'T1.status_date', $APPTAG).'</th>
						'.$adminView['head']['actions'].'
					</tr>
				</thead>
				<tbody>
	...
	if($num_rows) : // verifica se existe
		...
		foreach($res as $item) {
			...
			$status = $item->status ? '<span class="base-icon-ok text-success cursor-help hasTooltip" title="'.JText::_('TEXT_USED').'"></span>' : '<span class="base-icon-clock text-live cursor-help hasTooltip" title="'.JText::_('TEXT_WAITING').'"></span>';
			$owner = !empty($item->owner) ? baseHelper::nameFormat($item->owner) : '-';
			$rowState = $item->state == 0 ? 'danger' : '';
			$html .= '
				<tr id="'.$APPTAG.'-item-'.$item->id.'" class="'.$rowState.'">
					'.$adminView['list']['info'].'
					<td>'.$item->cod.'</td>
					<td class="hidden-print">'.baseHelper::priceFormat($item->price, false, '', true, '-').'</td>
					<td>'.$owner.'</td>
					<td class="text-center hidden-print">'.$status.'</td>
					<td>'.$filePath.baseHelper::nameFormat($item->project).' <small class="text-muted font-featured">('.baseHelper::dateFormat($item->projectDate).')</small></td>
					<td class="hidden-print">'.baseHelper::dateFormat($item->status_date, 'd/m/Y H:i:s', true, '-').'</td>
					'.$adminView['list']['actions'].'
				</tr>
			';
		}
