...
// filter params
	...
	// STATUS -> select
	$status	= $app->input->get('st', 2, 'int');
	if($status < 2) $where .= ' AND '.$db->quoteName('T1.status').' = '.$status;
	// PROJECTS
	...
	if(!empty($search)) :
		$where .= ' AND (';
		$where .= 'LOWER('.$db->quoteName('T1.cod').') LIKE LOWER("%'.$search.'%")';
		$where .= ' OR LOWER('.$db->quoteName('T1.owner').') LIKE LOWER("%'.$search.'%")';
		$where .= ')';
	endif;
	...
	<form id="filter-'.$APPTAG.'" class="hidden-print" method="get">
		<fieldset class="fieldset-embed filter '.((isset($_GET[$APPTAG.'_filter']) || $cfg['showFilter']) ? '' : 'closed').'">
			...
			<div class="row">
				...
				<div class="col-sm-2">
					<div class="form-group">
            <input type="text" name="fSearch" value="'.$search.'" class="form-control input-sm field-search width-full" placeholder="'.JText::_('FIELD_LABEL_COD').'" />
					</div>
				</div>
				<div class="col-sm-2">
					<div class="form-group">
						<select name="st" id="st" class="form-control input-sm set-filter">
							<option value="2">- '.JText::_('FIELD_LABEL_STATUS').' -</option>
							<option value="1"'.($status == 1 ? ' selected' : '').'>'.JText::_('TEXT_USED').'</option>
							<option value="0"'.($status == 0 ? ' selected' : '').'>'.JText::_('TEXT_WAITING').'</option>
						</select>
					</div>
				</div>
				<div class="col-sm-2">
