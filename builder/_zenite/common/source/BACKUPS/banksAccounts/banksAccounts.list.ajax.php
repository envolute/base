...
if(isset($_SERVER["HTTP_X_REQUESTED_WITH"]) AND strtolower($_SERVER["HTTP_X_REQUESTED_WITH"]) == "xmlhttprequest") :
	...
	if(!empty($rID) && $rID !== 0) :
		if(isset($_SESSION[$RTAG.'RelTable']) && !empty($_SESSION[$RTAG.'RelTable'])) :
			$query .= ' FROM '.
				...
				LEFT JOIN '. $db->quoteName('#__zenite_banks') .' T2
				ON T2.id = T1.bank_id
	...
	$query .= ' ORDER BY '. $db->quoteName('T1.id');
	try {
			...
			$html .= '
				<li class="'.$rowState.'">
					<span class="pull-right">
						...
						<a href="#" class="base-icon-pencil text-live hasTooltip" title="'.JText::_('TEXT_EDIT').'" onclick="'.$APPTAG.'_loadEditFields('.$item->id.', false, false)" id="'.$APPTAG.'-edit-'.$item->id.'"></a>
						<a href="#" class="base-icon-trash text-danger hasTooltip" title="'.JText::_('TEXT_DELETE').'" onclick="'.$APPTAG.'_del('.$item->id.', false)" id="'.$APPTAG.'-delete-'.$item->id.'"></a>
					</span>'.
					$note.baseHelper::nameFormat($item->bank).'<br />
					<small class="text-muted font-featured">
						<span class="cursor-help hasTooltip" title="'.JText::_('FIELD_LABEL_OPERATION').'">Op: '.$item->operation.'</span>
						<span class="left-space-sm cursor-help hasTooltip" title="'.JText::_('FIELD_LABEL_AGENCY').'">Ag: <span class="text-live">'.$item->agency.'</span></span>
						<span class="left-space-sm cursor-help hasTooltip" title="'.JText::_('FIELD_LABEL_ACCOUNT').'">Cc: <span class="text-live">'.$item->account.'</span></span>
					</small>
				</li>
			';
		}
