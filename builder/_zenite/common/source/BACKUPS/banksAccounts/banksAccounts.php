...
jQuery(function() {
	...
	// métodos controladores do formulário
		...
		var firstField = bank_id; // campo que recebe o focus no carregamento
		...
		window.<?php echo $APPTAG?>_formReset = function() {
			...
			bank_id.val('0').trigger("chosen:updated"); // select;
			agency.val('');
			account.val('');
			operation.val('');
			note.val('');
			...
			jQuery.ajax({
				...
				success: function(data) {
					jQuery.map( data, function( item ) {
						...
						// App Fields
						bank_id.val(item.bank_id).trigger("chosen:updated"); // select;
