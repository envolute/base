		...
		foreach($res as $item) {
			...
			$html .= '
				<li class="'.$rowState.'">
					...
					<span class="font-featured">'.$item->comment.'</span><br />
					'.(!empty($listFiles) ? '<small class="right-space-sm">'.$listFiles.'</small>' : '').'
					<span class="text-xs opacity_50"><span class="base-icon-calendar"></span> '.baseHelper::dateFormat($item->created_date, 'd/m/y H:i').' - <span class="base-icon-user hasTooltip" title="'.JText::_('TEXT_USER').'"></span> '.baseHelper::nameFormat($item->user).'</small>
				</li>
			';
		}
		$html .= '</ul>';
