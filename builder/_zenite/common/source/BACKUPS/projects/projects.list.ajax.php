...
if(isset($_SERVER["HTTP_X_REQUESTED_WITH"]) AND strtolower($_SERVER["HTTP_X_REQUESTED_WITH"]) == "xmlhttprequest") :
	...
	$query .= ' ORDER BY '. $db->quoteName('T2.name') .' ASC, '. $db->quoteName('T1.start_date') .' DESC';
	try {
			...
			$html .= '
				<li class="'.$rowState.'">
					<span class="btn-group pull-right">
						...
						<a href="#" class="base-icon-pencil text-live hasTooltip" title="'.JText::_('TEXT_EDIT').'" onclick="'.$APPTAG.'_loadEditFields('.$item->id.', false, false)"></a>
						<a href="#" class="base-icon-trash text-danger hasTooltip" title="'.JText::_('TEXT_DELETE').'" onclick="'.$APPTAG.'_del('.$item->id.', false)"></a>
					</span>
					<a href="#">'.baseHelper::nameFormat($item->name).'</a>
					<div class="small text-muted font-featured clearfix">'.
						'<span class="cursor-help hasTooltip" title="'.JText::_('FIELD_LABEL_CLIENT').'">'.baseHelper::nameFormat($item->client).'</span>
						 - <span class="cursor-help hasTooltip" title="'.JText::_('FIELD_LABEL_START_DATE').'">'.baseHelper::dateFormat($item->start_date, 'd/m/Y').'</span>
					</div>
				</li>
			';
		}
