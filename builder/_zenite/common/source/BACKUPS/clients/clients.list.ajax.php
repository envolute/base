...
if(isset($_SERVER["HTTP_X_REQUESTED_WITH"]) AND strtolower($_SERVER["HTTP_X_REQUESTED_WITH"]) == "xmlhttprequest") :
	...
	if(!empty($rID) && $rID !== 0) :
		if(isset($_SESSION[$RTAG.'RelTable']) && !empty($_SESSION[$RTAG.'RelTable'])) :
			...
			$query .= ' FROM '. $db->quoteName($cfg['mainTable']) .' T1 WHERE '. $db->quoteName($rNID) .' = '. $rID . ' AND T1.proposal = 0';
		endif;
	else :
	...
	if($num_rows) : // verifica se existe
		...
		foreach($res as $item) {
			...
			$img = !empty($listFiles) ? '<img class="img-responsive pull-left right-space" src="'.baseHelper::thumbnail($listFiles, 40, 40).'" />' : '';
			...
			$html .= '
				<li class="'.$rowState.'">
					<div class="clearfix">
						...
						<a href="#">'.baseHelper::nameFormat($item->name).'</a><br />
