...
jQuery(function() {
	...
	var proposal			= jQuery('#<?php echo $APPTAG?>-proposal');
	var name 					= jQuery('#<?php echo $APPTAG?>-name');
	var name_company	= jQuery('#<?php echo $APPTAG?>-name_company');
	...
	// métodos controladores do formulário
		...
		window.<?php echo $APPTAG?>_formReset = function() {
			...
			selectRadio(type, 1);
			proposal.prop('checked', false); // checkbox -> default: not checked
			name.val('');
			name_company.val('');
			email.val('');
			cmail.val('');
			...
			description.val('');
			date.val('');
			due_date.val('1').trigger("chosen:updated"); // select
			note.val('');
		...
		window.<?php echo $APPTAG?>_setType = function(e){
			if(e == 1) {
				...
				jQuery('#<?php echo $APPTAG?>-numDoc').removeClass('col-sm-3').addClass('col-sm-4');
			} else {
				setCPF(doc_number);
				doc_number.prev('label').text('CPF');
				jQuery('#<?php echo $APPTAG?>-company').hide();
				jQuery('#<?php echo $APPTAG?>-numDoc').removeClass('col-sm-4').addClass('col-sm-3');
			}
		}
			...
			jQuery.ajax({
				...
				success: function(data) {
					jQuery.map( data, function( item ) {
						...
						proposal.prop('checked', (item.proposal == 0 ? false : true));
						name.val(item.name);
						name_company.val(item.name_company);
						...
						due_date.val(item.due_date).trigger("chosen:updated"); // selects
						note.val(item.note);
					...
					success: function(data){
						...
						jQuery.map( data, function( res ) {
							if(res.status > 0) { // se alguma ação for realizada
								...
								// necessário após um procedimento ajax que envolve os elementos
								setFormDefinitions();
								// CUSTOM -> em caso de erro, mantém a formatação para o CPF ou CNPJ
								<?php echo $APPTAG?>_setType(mainForm.find('input[name=type]:radio:checked').val());
