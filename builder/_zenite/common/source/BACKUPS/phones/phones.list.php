...
// LIST
	...
	$query = '
		...
		FROM
			...
			LEFT JOIN '. $db->quoteName('#__zenite_rel_clients_phones') .' T2
			ON T2.phone_id = T1.id
			LEFT JOIN '. $db->quoteName('#__zenite_clients') .' T3
			ON T3.id = T2.client_id
			LEFT JOIN '. $db->quoteName('#__zenite_rel_contacts_phones') .' T4
...
if($num_rows) : // verifica se existe
	...
	foreach($res as $item) {
		...
		$main = $item->main == 1 ? '<span class="base-icon-star text-live"></span>' : '';
