...
if(isset($_SERVER["HTTP_X_REQUESTED_WITH"]) AND strtolower($_SERVER["HTTP_X_REQUESTED_WITH"]) == "xmlhttprequest") :
	...
	$query .= ' ORDER BY '. $db->quoteName('T2.name') .' ASC, '. $db->quoteName('T1.distance') .' ASC,'. $db->quoteName('T1.id') .' ASC';
	try {
	...
	if($num_rows) : // verifica se existe
		...
		foreach($res as $item) {
			...
			$ageGroup = ($item->min_age != 0 && $item->max_age != 0) ? ' - <small class="cursor-help hasTooltip" title="'.JText::_('TEXT_AGE_GROUP').'">'.$item->min_age.' '.JText::_('TEXT_TO').' '.($item->max_age == 0 ? '&infin;' : $item->max_age).' '.JText::_('TEXT_YEARS').'</small>' : '';
			$disability = $item->disability == 0 ? '' : ' ['.baseHelper::nameFormat($item->disability).']';
			...
			$html .= '
				<li class="'.$rowState.'">
					<div class="pull-right">
						...
						<a href="#" class="base-icon-pencil text-live hasTooltip" onclick="'.$APPTAG.'_loadEditFields('.$item->id.', false, false)" title="'.JText::_('TEXT_EDIT').'"></a>
						<a href="#" class="base-icon-trash text-danger hasTooltip" onclick="'.$APPTAG.'_del('.$item->id.', false)" title="'.JText::_('TEXT_DELETE').'"></a>
					</div>
					'.$item->distance.$distance_unit.$disability.'<br />
					<small class="text-muted font-featured">
						<span class="cursor-help hasTooltip" title="'.JText::_('FIELD_LABEL_CATEGORY').'">'.$item->category.'</span>'.$ageGroup.' - R$ '.baseHelper::priceFormat($item->price).'
					</small>
				</li>
			';
		}
