...
if(isset($_SERVER["HTTP_X_REQUESTED_WITH"]) AND strtolower($_SERVER["HTTP_X_REQUESTED_WITH"]) == "xmlhttprequest") :
	...
	// GET DATA
	$noReg = true;
	$query = 'SELECT T1.id, T2.name client, T3.name contact, T1.main, T1.department, T1.state';
	if(!empty($rID) && $rID !== 0) :
		if(isset($_SESSION[$RTAG.'RelTable']) && !empty($_SESSION[$RTAG.'RelTable'])) :
			$query .= ' FROM '.
				...
				JOIN '. $db->quoteName('#__zenite_contacts') .' T3
				ON '.$db->quoteName('T3.id') .' = T1.contact_id
			WHERE '.
	...
	$query .= ' ORDER BY '. $db->quoteName('T2.name') .', '. $db->quoteName('T3.name');
	try {
	...
	if($num_rows) : // verifica se existe
		...
		foreach($res as $item) {
			...
			$dept = !empty($item->department) ? '<em class="small text-muted font-featured cursor-help hasTooltip" title="'.JText::_('FIELD_LABEL_DEPARTMENT').'">('.baseHelper::nameFormat($item->department).')</em> ' : '';
			...
			$html .= '
				<li class="'.$rowState.'">
					<span class="btn-group pull-right">
						...
						<a href="#" onclick="'.$APPTAG.'_loadEditFields('.$item->id.', false, false)">
							<span class="base-icon-pencil text-live hasTooltip" title="'.JText::_('TEXT_EDIT').'"></span>
						</a>
						<a href="#" onclick="'.$APPTAG.'_del('.$item->id.', false)">
							<span class="base-icon-trash text-danger hasTooltip" title="'.JText::_('TEXT_DELETE_REL').'"></span>
						</a>
					</span>
					'.$main.baseHelper::nameFormat($item->contact).'<br />
					'.$dept.'
				</li>
			';
		}
