<?php
/* SISTEMA PARA CADASTRO DE CONTATOS DOS CLIENTES
 * AUTOR: IVO JUNIOR
 * EM: 18/02/2016
*/
...
jQuery(function() {
	...
	// métodos controladores do formulário
		...
		window.<?php echo $APPTAG?>_formReset = function() {
			...
			client_id.prop('disabled', false).trigger("chosen:updated");
			contact_id.add(contact_id.closest('.form-group').find('.base-icon-plus')).prop('disabled', false);
			contact_id.val('0').trigger("chosen:updated"); // select;
			main.prop('checked', false); // checkbox -> default: not checked
			department.val('');
			emailConfirm.prop('disabled', false);
			emailConfirm.prop('checked', false);
			emailInfo.val('').prop('disabled', true);
		...
		// new contacts list when change project
		client_id.on('change', function() {
			<?php echo $APPTAG?>_getContactsList(jQuery(this).val());
		});
	...
	// métodos controladores das ações referente ao banco de dados e envio de arquivos
		...
		// CUSTOM
		// Set Contacts List
		// seta a lista de contatos de acordo com o projeto selecionado
		window.<?php echo $APPTAG?>_getContactsList = function(itemID, id) {
			<?php echo $APPTAG?>_formExecute(true, true, true); // inicia o loader
			jQuery.ajax({
				url: "<?php echo JURI::root().'templates/base/source/'.$APPNAME.'/'.$APPNAME ?>.model.php?task=cList&pID="+itemID,
				dataType: 'json',
				type: 'POST',
				cache: false,
				success: function(data){
					...
					jQuery.map( data, function( res, i ) {
						if(res.status != 0) {
							// remove all options
							if(i == 0) {
								contact_id.find('option').remove();
								// init new options list
								if(res.status == 1) {
									contact_id.append('<option value="0">- <?php echo JText::_('TEXT_SELECT'); ?> -</option>');
									projectClientId = res.client;
								} else {
									contact_id.append('<option value="0">- <?php echo JText::_('TEXT_ADD_CLIENTS_CONTACTS'); ?> -</option>');
									projectClientId = 0;
								}
							}
							if(res.status == 1) contact_id.append('<option value="'+res.id+'">'+res.name+'</option>');
							if(typeof id !== "null" && typeof id !== "undefined" && id != 0) contact_id.val(id);
							contact_id.trigger("chosen:updated"); // atualiza o select
						} else {
							if(!itemID) mainForm.find('.set-error').removeClass('hide').text(res.msg);
						}
					});
					if(client_id.val() != '') {
						jQuery('#groupAdd-client-contact').removeClass('hide');
					} else {
						jQuery('#groupAdd-client-contact').addClass('hide');
					}
				},
			...
			jQuery.ajax({
				...
				success: function(data) {
					jQuery.map( data, function( item ) {
						...
						// App Fields
						client_id.val(item.client_id).prop('disabled', true).trigger("chosen:updated"); // select;
						<?php echo $APPTAG?>_getContactsList(item.client_id, item.contact_id);
						contact_id.add(contact_id.closest('.form-group').find('.base-icon-plus')).prop('disabled', true);
						main.prop('checked', (item.main == 0 ? false : true)); // checkbox -> default: not checked
