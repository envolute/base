		...
		<table class="table table-striped table-hover table-condensed">
			<thead>
				<tr>
					...
					<th>'.$$SETOrder(JText::_('FIELD_LABEL_CONTACT'), 'T3.name', $APPTAG).'</th>
					<th>'.$$SETOrder(JText::_('FIELD_LABEL_CLIENT'), 'T2.name', $APPTAG).'</th>
					<th class="text-center">'.$$SETOrder(JText::_('TEXT_MAIN'), 'T1.main', $APPTAG).'</th>
					<th>'.JText::_('FIELD_LABEL_DEPARTMENT').'</th>
	...
	foreach($res as $item) {
		...
		$html .= '
			<tr id="'.$APPTAG.'-item-'.$item->id.'" class="'.$rowState.'">
				...
				<td>'.baseHelper::nameFormat($item->contact).'</td>
				<td>'.baseHelper::nameFormat($item->client).'</td>
				<td class="text-center">'.($item->main == 1 ? '<span class="base-icon-star text-live cursor-help hasTooltip" title="'.JText::_('FIELD_LABEL_MAIN_DESC').'"></span>' : '').'</td>
				<td>'.baseHelper::nameFormat($item->department).'</td>
				...
				<td class="text-center">
					...
					<a href="#" class="btn btn-xs btn-danger" onclick="'.$APPTAG.'_del('.$item->id.', false)"><span class="base-icon-trash hasTooltip" title="'.JText::_('TEXT_DELETE_REL').'"></span></a>
				</td>
			</tr>
		';
	}
