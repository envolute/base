...
jQuery(function() {
	...
	// APP FIELDS
	var socialName_id = jQuery('#<?php echo $APPTAG?>-socialName_id');
	var page 					= jQuery('#<?php echo $APPTAG?>-page');
	...
	// métodos controladores do formulário
		...
		var firstField = socialName_id; // campo que recebe o focus no carregamento
		...
		window.<?php echo $APPTAG?>_formReset = function() {
			...
			socialName_id.val('0').trigger("chosen:updated"); // select
			page.val('');
			username.val('');
			...
			jQuery.ajax({
				...
				success: function(data) {
					jQuery.map( data, function( item ) {
						...
						// App Fields
						socialName_id.val(item.socialName_id).trigger("chosen:updated"); // select
						page.val(item.page);
