...
// LIST
	...
	$query = '
		...
		FROM
			...
			LEFT JOIN '. $db->quoteName('#__zenite_web_socials_names') .' T2
			ON T2.id = T1.socialName_id
			LEFT JOIN '. $db->quoteName('#__zenite_rel_clients_socials') .' T3
			ON T3.social_id = T1.id
			LEFT JOIN '. $db->quoteName('#__zenite_clients') .' T4
			ON T4.id = T3.client_id
			LEFT JOIN '. $db->quoteName('#__zenite_rel_contacts_socials') .' T5
			ON T5.social_id = T1.id
			LEFT JOIN '. $db->quoteName('#__zenite_contacts') .' T6
