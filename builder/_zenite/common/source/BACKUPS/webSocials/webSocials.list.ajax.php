...
if(isset($_SERVER["HTTP_X_REQUESTED_WITH"]) AND strtolower($_SERVER["HTTP_X_REQUESTED_WITH"]) == "xmlhttprequest") :
	...
	// GET DATA
	$noReg = true;
	$query = 'SELECT T1.id, T3.name, T1.page, T1.username, T1.state';
	if(!empty($rID) && $rID !== 0) :
		if(isset($_SESSION[$RTAG.'RelTable']) && !empty($_SESSION[$RTAG.'RelTable'])) :
			$query .= ' FROM '.
				...
				JOIN '. $db->quoteName($cfg['mainTable'].'_names') .' T3
				ON T3.id = T1.socialName_id
			WHERE '.
	...
	$query .= ' ORDER BY '. $db->quoteName('T1.id') .' ASC';
	try {
	...
	if($num_rows) : // verifica se existe
		...
		foreach($res as $item) {
			...
			$username = !empty($item->username) ? '<small class="text-muted font-featured"> ('.$item->username.')</small>' : '';
			...
			$html .= '
				<li class="'.$rowState.'">
					...
					<a href="'.$item->page.'" target="_blank" class="new-window">'.baseHelper::nameFormat($item->name).$username.'</a>
					<div class="text-xs text-muted font-featured">'.$item->page.'</div>
				</li>
			';
		}
		$html .= '</ul>';
