	...
	if($num_rows) : // verifica se existe
		...
		foreach($res as $item) {
			...
			$html .= '
				...
				$addressZip.baseHelper::nameFormat($item->address_district).$addressCity.$addressState.$addressCountry
				.'<div>
						'.$mapa.'
						...
						<a href="#" onclick="'.$APPTAG.'_loadEditFields('.$item->id.', false, false)" id="'.$APPTAG.'-edit-'.$item->id.'">
							<span class="base-icon-pencil text-live hasTooltip" title="'.JText::_('TEXT_EDIT').'"></span>
						</a>
						<a href="#" onclick="'.$APPTAG.'_del('.$item->id.', false)" id="'.$APPTAG.'-delete-'.$item->id.'">
							<span class="base-icon-trash text-danger hasTooltip" title="'.JText::_('TEXT_DELETE').'"></span>
						</a>
					</div>
				</li>
			';
