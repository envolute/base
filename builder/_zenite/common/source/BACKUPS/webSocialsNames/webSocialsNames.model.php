      ...
      if($id) :
        ...
        // COUNT LIST IDS
          ...
          foreach($list as $item) {
            ...
            // UPDATE FIELD
            $element = $elemVal = $elemLabel = $elemUrl = '';
            ...
            if(!empty($_SESSION[$RTAG.'RelTable']) && !empty($_SESSION[$RTAG.'AppNameId'])) :
              ...
              // isso é RECOMENDÁVEL sempre que houver um ou mais relacionamentos
                ...
                // $query = 'DELETE FROM '. $db->quoteName('#__zenite_clients_hosts') .' WHERE '. $db->quoteName('plan_id') .' IN ('.$ids.')';
                // $db->setQuery($query);
      					// $db->execute();
            endif;
        ...
        if(!empty($request['name']) && !empty($request['url'])) :
          ...
          INSERT INTO '. $db->quoteName($cfg['mainTable']) .'('.
            ...
            if(!empty($_SESSION[$RTAG.'FieldUpdated']) && !empty($_SESSION[$RTAG.'TableField'])) :
              $element = $_SESSION[$RTAG.'FieldUpdated'];
              $elemVal = $id;
              $query = 'SELECT '. $db->quoteName($_SESSION[$RTAG.'TableField']) .', '.$db->quoteName('url').' FROM '. $db->quoteName($cfg['mainTable']).' WHERE id='.$id.' AND state = 1';
              $db->setQuery($query);
              $elems = $db->loadObject();
              $elemLabel = $elems->name;
              $elemUrl = $elems->url;
