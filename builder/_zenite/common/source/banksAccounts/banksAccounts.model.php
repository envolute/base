        ...
        // COUNT LIST IDS
          ...
          foreach($list as $item) {
            ...
            if(!empty($_SESSION[$RTAG.'RelTable']) && !empty($_SESSION[$RTAG.'AppNameId'])) :
              ...
              $query = 'DELETE FROM '. $db->quoteName('#__zenite_rel_clients_addresses') .' WHERE '. $db->quoteName('address_id') .' IN ('.$ids.')';
              $db->setQuery($query);
    					$db->execute();
              // CONTACTS -> remove os registros relacionados aos contatos
              $query = 'DELETE FROM '. $db->quoteName('#__zenite_rel_contacts_addresses') .' WHERE '. $db->quoteName('address_id') .' IN ('.$ids.')';
              $db->setQuery($query);
    					$db->execute();
            endif;
