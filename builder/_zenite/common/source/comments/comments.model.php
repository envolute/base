          ...
          foreach($list as $item) {
            ...
            if(!empty($_SESSION[$RTAG.'RelTable']) && !empty($_SESSION[$RTAG.'AppNameId'])) :
              ...
              // isso é RECOMENDÁVEL sempre que houver um ou mais relacionamentos
                ...
                $query = 'DELETE FROM '. $db->quoteName('#__zenite_rel_tasks_comments') .' WHERE '. $db->quoteName('task_id') .' IN ('.$ids.')';
                $db->setQuery($query);
      					$db->execute();
                // TICKETS -> remove os registros relacionados aos contatos
                $query = 'DELETE FROM '. $db->quoteName('#__zenite_rel_tickets_comments') .' WHERE '. $db->quoteName('ticket_id') .' IN ('.$ids.')';
                $db->setQuery($query);
      					$db->execute();
            endif;
