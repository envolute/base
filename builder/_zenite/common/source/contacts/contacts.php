...
jQuery(function() {
	...
	// métodos controladores das ações referente ao banco de dados e envio de arquivos
      ...
      if(confirm(msg)) {
        <?php echo $APPTAG?>_formExecute(true, true, true); // inicia o loader
  			jQuery.ajax({
  				url: "<?php echo JURI::root().'templates/base/source/'.$APPNAME.'/'.$APPNAME ?>.model.php?aTag=<?php echo $APPTAG?>&rTag=<?php echo $RTAG?>&task=userBlock&lock="+lock+"&pid="+jQuery('#<?php echo $APPTAG?>-user_id').val(),
  				dataType: 'json',
  				type: 'POST',
  				cache: false,
  				success: function(data){
