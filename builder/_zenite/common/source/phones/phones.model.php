        ...
        // COUNT LIST IDS
          ...
          foreach($list as $item) {
            ...
            if(!empty($_SESSION[$RTAG.'RelTable']) && !empty($_SESSION[$RTAG.'AppNameId'])) :
              ...
              $query = 'DELETE FROM '. $db->quoteName('#__zenite_rel_clients_phones') .' WHERE '. $db->quoteName('phone_id') .' IN ('.$ids.')';
              $db->setQuery($query);
    					$db->execute();
              // CONTACTS -> remove os registros relacionados aos contatos
              $query = 'DELETE FROM '. $db->quoteName('#__zenite_rel_contacts_phones') .' WHERE '. $db->quoteName('phone_id') .' IN ('.$ids.')';
              $db->setQuery($query);
    					$db->execute();
            endif;
