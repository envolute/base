<?php
/* SISTEMA PARA CADASTRO DE PERFIS EM REDES SOCIAIS
 * AUTOR: IVO JUNIOR
 * EM: 18/02/2016
*/
...
jQuery(function() {
	...
	var projectType_id 		= jQuery('#<?php echo $APPTAG?>-projectType_id');
	...
	// métodos controladores do formulário
		...
		window.<?php echo $APPTAG?>_formReset = function() {
			...
			user_id.prop('disabled', false).trigger("chosen:updated"); // select
			project_id.prop('disabled', false).trigger("chosen:updated"); // select
			projectType_id.val('0').trigger("chosen:updated"); // select
			price.val('');
			discount.val('');
		...
		// CUSTOM -> new contacts list when change project
		project_id.on('change', function() {
			<?php echo $APPTAG?>_getTypesList(jQuery(this).val());
		});
	...
	// métodos controladores das ações referente ao banco de dados e envio de arquivos
		...
		// CUSTOM
		// Set Types List
		// seta a lista de tipos de acordo com o projeto selecionado
		window.<?php echo $APPTAG?>_getTypesList = function(itemID, id) {
				...
				success: function(data){
					...
					jQuery.map( data, function( res, i ) {
						if(res.status != 0) {
							// remove all options
							if(i == 0) {
								projectType_id.find('option').remove();
								// init new options list
								if(res.status == 1) {
									projectType_id.append('<option value="0">- <?php echo JText::_('TEXT_SELECT'); ?> -</option>');
								} else {
									projectType_id.append('<option value="0">- <?php echo JText::_('TEXT_NOT_TYPES'); ?> -</option>');
								}
							}
							if(res.status == 1) projectType_id.append('<option value="'+res.id+'">'+res.name+'</option>');
							if(typeof id !== "null" && typeof id !== "undefined" && id != 0) projectType_id.val(id);
							projectType_id.trigger("chosen:updated"); // atualiza o select
						} else {
			...
			jQuery.ajax({
				...
				success: function(data) {
					jQuery.map( data, function( item ) {
						...
						// App Fields
						user_id.val(item.user_id).prop('disabled', true).trigger("chosen:updated"); // select;
						project_id.val(item.project_id).prop('disabled', true).trigger("chosen:updated"); // select;
						<?php echo $APPTAG?>_getTypesList(item.project_id, item.projectType_id);
						price.val(item.price);
						discount.val(item.discount);
						selectRadio(status, item.status);
		...
		// CUSTOM -> Verifica os status de pagamento
        ...
        return false;
			...
			jQuery.ajax({
				...
				success: function(data) {
					jQuery.map( data, function( res ) {
						if(res.status == 1) {
							var path = '<?php echo JURI::root().'templates/base/source/registrations/registrations.getFile.php?fn='?>'+res.file;
							// console.log(path);
							location.href = path;
		...
		<div class="list-toolbar<?php echo ($cfg['staticToolbar'] ? '' : ' floating')?> hidden-print">
			...
			?>
				<?php if($hasAdmin) : ?>
					...
					<button type="button" onclick="<?php echo $APPTAG?>_userSync()" class="btn btn-sm btn-success hasTooltip" title="<?php echo JText::_('LABEL_SYNCRONIZE_DESC')?>">
						<span class="base-icon-arrows-cw"></span> <?php echo JText::_('LABEL_SYNCRONIZE')?>
					</button>
				<?php endif; ?>
