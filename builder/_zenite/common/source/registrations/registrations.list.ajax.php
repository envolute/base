...
if(isset($_SERVER["HTTP_X_REQUESTED_WITH"]) AND strtolower($_SERVER["HTTP_X_REQUESTED_WITH"]) == "xmlhttprequest") :
	...
	$query .= ' ORDER BY '. $db->quoteName('T2.date') .' DESC';
	try {
			...
			$html .= '
				<li class="'.$rowState.'">
					...
					<small class="text-muted font-featured">
						<span class="cursor-help hasTooltip" title="'.JText::_('FIELD_LABEL_PROJECT').'">'.$item->project.'</span> ('.baseHelper::dateFormat($item->date).')
					</small>
				</li>
			';
		}
