...
$query = 'SELECT * FROM '. $db->quoteName('#__users') .' WHERE block = 0 ORDER BY name ASC';
$db->setQuery($query);
$users = $db->loadObjectList();

$query = 'SELECT * FROM '. $db->quoteName('#__zenite_projects') .' WHERE state = 1 AND date > CURDATE() ORDER BY date DESC';
$db->setQuery($query);
	...
	<div class="modal-body">
		<fieldset>
			...
			<div class="row">
				<div class="col-sm-6">
					<div class="form-group field-required">
						<label><?php echo JText::_('FIELD_LABEL_USER'); ?></label>
						<div class="input-group">
							<select name="user_id" id="<?php echo $APPTAG?>-user_id" class="form-control field-id auto-tab" data-target="<?php echo $APPTAG?>-project_id">
				...
				<div class="col-sm-6">
					<div class="form-group field-required">
						...
						<select name="projectType_id" id="<?php echo $APPTAG?>-projectType_id" class="form-control field-id">
							<option value="0">- <?php echo JText::_('TEXT_SELECT_PROJECT'); ?> -</option>
						</select>
					</div>
				</div>
				<div class="col-sm-3">
				...
				<div class="col-sm-3">
					<div class="form-group">
						...
						<input type="text" name="discount" id="<?php echo $APPTAG?>-discount" class="form-control field-price" />
					</div>
				</div>
			</div>
			<hr class="hr-sm" />
				...
				<div class="col-sm-3">
					<div class="btn-group" data-toggle="buttons">
						<label class="btn btn-warning btn-active-success">
							...
							<input type="checkbox" name="status" id="<?php echo $APPTAG?>-status" value="2" />
							<?php echo JText::_('TEXT_CONFIRM'); ?>
						</label>
					</div>
				</div>
			</div>
