...
// LIST
		...
		<form id="form-list-'.$APPTAG.'" method="post">
			<table class="table table-striped table-hover table-condensed">
				<thead>
					<tr>
						'.$adminView['head']['info'].'
						<th>'.$$SETOrder('Atleta', 'T1.nome', $APPTAG).'</th>
						<th>'.$$SETOrder('Colocação', 'T1.colocacao', $APPTAG).'</th>
						<th>'.$$SETOrder('Colocação Faixa', 'T1.colocacao_faixa', $APPTAG).'</th>
						<th>'.$$SETOrder('Tempo', 'T1.tempo_liquido', $APPTAG).'</th>
						<th>'.$$SETOrder('Tempo Total', 'T1.tempo_bruto', $APPTAG).'</th>
						<th>'.$$SETOrder('Equipe', 'T1.equipe', $APPTAG).'</th>
					</tr>
				</thead>
				<tbody>
	';
	...
	if($num_rows) : // verifica se existe
		...
		foreach($res as $item) {
			...
			$rowState = ($item->state == 0 || $item->vencido == 1) ? 'danger' : '';
			$html .= '
				<tr id="'.$APPTAG.'-item-'.$item->id.'" class="'.$rowState.'">
					'.$adminView['list']['info'].'
					<td>'.$counter.'</td>
					<td>'.baseHelper::nameFormat($item->nome).'<br /><small class="text-muted font-featured">N&ordm;: '.$item->numero.' - Sexo: '.$item->sexo.' - Faixa: '.$item->faixa.'</small></td>
					<td>'.$item->colocacao.'</td>
					<td>'.$item->colocacao_faixa.'</td>
					<td>'.$item->tempo_liquido.'</td>
					<td>'.$item->tempo_bruto.'</td>
				</tr>
			';
		}
