...
// LIST
	...
	$query = '
		SELECT SQL_CALC_FOUND_ROWS
			...
			IF('.$db->quoteName('T1.date').' < NOW(), 1, 0) closed,
...
if($num_rows) : // verifica se existe
	...
	foreach($res as $item) {
		...
		$query = 'SELECT COUNT(*) FROM '. $db->quoteName('#__zenite_registrations') .' WHERE project_id = '.$item->id.' AND due_date > NOW() AND state = 1';
		$db->setQuery($query);
		$totalSubs = $db->loadResult();
		...
		$rowState = $item->state == 0 ? 'danger' : ($item->closed == 1 ? 'warning text-muted' : '');
		$html .= '
			<tr id="'.$APPTAG.'-item-'.$item->id.'" class="'.$rowState.'">
				'.$adminView['list']['info'].'
				<td>
					<a href="evento?p='.urlencode(base64_encode($item->id)).'">'.baseHelper::nameFormat($item->name).$info.'</a><br />
				...
				<td'.($item->closed == 1 ? ' class="text-danger"' : '').'>'.baseHelper::dateFormat($item->date, 'd/m/Y', true, '-').'</td>
				<td>
					<span class="text-'.($item->closed == 1 ? 'muted' : 'success').' cursor-help hasTooltip" title="'.JText::_('FIELD_LABEL_START_DATE').'">'.baseHelper::dateFormat($item->start_date, 'd/m/Y H:i', true, '-').'</span><br />
					<small class="base-icon-right-big font-featured text-'.($item->closed == 1 ? 'danger' : 'live').' cursor-help hasTooltip" title="'.JText::_('FIELD_LABEL_END_DATE_DESC').'"> '.baseHelper::dateFormat($item->end_date, 'd/m/Y H:i', true, '-').'</small>
				</td>
				<td>'.$item->limit.'</td>
				<td>'.$totalSubs.'</td>
				'.$adminView['list']['actions'].'
