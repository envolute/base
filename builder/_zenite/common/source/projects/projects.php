...
jQuery(function() {
	...
	var location			= jQuery('#<?php echo $APPTAG?>-location');
	var limit					= jQuery('#<?php echo $APPTAG?>-limit');
	var sizeShirts		= jQuery('#<?php echo $APPTAG?>-sizeShirts');
	var description		= jQuery('#<?php echo $APPTAG?>-description');
	var url_info			= jQuery('#<?php echo $APPTAG?>-url_info');
	var url_registration = jQuery('#<?php echo $APPTAG?>-url_registration');
	var rules_content = jQuery('#<?php echo $APPTAG?>-rules_content');
	var payment_card		= jQuery('#<?php echo $APPTAG?>-payment_card');
	var payment_boleto	= jQuery('#<?php echo $APPTAG?>-payment_boleto');
	var discount_coupon	= jQuery('#<?php echo $APPTAG?>-discount_coupon');
	var discount_age		= jQuery('#<?php echo $APPTAG?>-discount_age');
	var discount_deficient	= jQuery('#<?php echo $APPTAG?>-discount_deficient');
	var extra_field					= mainForm.find('input[name=extra_field]:radio');
	var extra_field_label		= jQuery('#<?php echo $APPTAG?>-extra_field_label');
	var extra_field_required		= jQuery('#<?php echo $APPTAG?>-extra_field_required');
	...
	// métodos controladores do formulário
		...
		var firstField = client_id; // campo que recebe o focus no carregamento
		...
		window.<?php echo $APPTAG?>_formReset = function() {
			...
			location.val('');
			limit.val('');
			sizeShirts.val('').trigger("chosen:updated"); // select
			description.val('');
			url_info.val('');
			url_registration.val('');
			rules_content.val('');
			payment_card.prop('checked', true);
			payment_boleto.prop('checked', true);
			discount_coupon.val('5');
			discount_age.val('50');
			discount_deficient.val('0');
			selectRadio(extra_field, 0);
			extra_field_label.val('');
			extra_field_required.prop('checked', false);
			note.val('');
			...
			jQuery.ajax({
				...
				success: function(data) {
					jQuery.map( data, function( item ) {
						...
						sizeShirts.val(item.sizeShirts).trigger("chosen:updated"); // select
						description.val(item.description);
						url_info.val(item.url_info);
						url_registration.val(item.url_registration);
						rules_content.val(item.rules_content);
						payment_card.prop('checked', (item.payment_card == 0 ? false : true));
						payment_boleto.prop('checked', (item.payment_boleto == 0 ? false : true));
						discount_coupon.val(item.discount_coupon);
						discount_age.val(item.discount_age);
						discount_deficient.val(item.discount_deficient);
						selectRadio(extra_field, item.extra_field);
						extra_field_label.val(item.extra_field_label);
						extra_field_required.prop('checked', (item.extra_field_required == 0 ? false : true));
						note.val(item.note);
