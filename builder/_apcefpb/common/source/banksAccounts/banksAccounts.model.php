  ...
  if($task != null) :
    ...
    $request['account']      = $input->get('account', '', 'string');
      ...
      if($id) :
        ...
        // COUNT LIST IDS
          ...
          foreach($list as $item) {
            ...
            if(!empty($_SESSION[$RTAG.'RelTable']) && !empty($_SESSION[$RTAG.'AppNameId'])) :
              ...
              $query = 'DELETE FROM '. $db->quoteName('#__apcefpb_rel_clients_banksAccounts') .' WHERE '. $db->quoteName('bankAccount_id') .' IN ('.$ids.')';
              $db->setQuery($query);
    					$db->execute();
              // PROVIDERS -> remove os registros relacionados aos prestadores
              $query = 'DELETE FROM '. $db->quoteName('#__apcefpb_rel_providers_banksAccounts') .' WHERE '. $db->quoteName('bankAccount_id') .' IN ('.$ids.')';
              $db->setQuery($query);
    					$db->execute();
              // EMPLOYEES -> remove os registros relacionados aos funcionários
              $query = 'DELETE FROM '. $db->quoteName('#__apcefpb_rel_employees_banksAccounts') .' WHERE '. $db->quoteName('bankAccount_id') .' IN ('.$ids.')';
              $db->setQuery($query);
    					$db->execute();
              // CONTACTS -> remove os registros relacionados aos contatos
              $query = 'DELETE FROM '. $db->quoteName('#__apcefpb_rel_contacts_banksAccounts') .' WHERE '. $db->quoteName('bankAccount_id') .' IN ('.$ids.')';
              $db->setQuery($query);
    					$db->execute();
            endif;
        ...
        // validation
        if($request['bank_id'] != 0 && !empty($request['agency']) && !empty($request['account']) && !empty($request['operation'])) :
