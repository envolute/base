	...
	<form id="form-list-'.$APPTAG.'" method="post">
		<table class="table table-striped table-hover table-condensed">
			<thead>
				<tr>
					'.$adminView['head']['info'].'
					<th>'.$$SETOrder(JText::_('FIELD_LABEL_SPORT'), 'T2.name', $APPTAG).'</th>
					<th>'.$$SETOrder(JText::_('FIELD_LABEL_MONTH'), 'T1.month', $APPTAG).'</th>
					<th class="text-center" width="80">'.$$SETOrder(JText::_('FIELD_LABEL_YEAR'), 'T1.year', $APPTAG).'</th>
	...
	foreach($res as $item) {
		...
		$html .= '
			<tr id="'.$APPTAG.'-item-'.$item->id.'" class="'.$rowState.'">
				'.$adminView['list']['info'].'
				<td>'.baseHelper::nameFormat($item->sport).$note.'</td>
