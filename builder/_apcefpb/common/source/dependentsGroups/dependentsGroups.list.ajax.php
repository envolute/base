		...
		foreach($res as $item) {
			...
			$html .= '
				<li class="'.$rowState.'">
					...
					'.baseHelper::nameFormat($item->name).' <small class="text-muted cursor-help font-featured hasTooltip" title="Período de Dependência">('.($item->overtime > 0 ? $item->overtime : 'Ilimitado').')</small>
				</li>
			';
		}
		$html .= '</ul>';
