...
// LIST
	...
	$query = '
		SELECT SQL_CALC_FOUND_ROWS
			...
			'. $db->quoteName('T5.month') .' invoiceMonth,
      '. $db->quoteName('T5.year') .' invoiceYear,
		...
		<table class="table table-striped table-hover table-condensed">
			<thead>
				<tr>
					...
					<th>'.JText::_('FIELD_LABEL_INSTALLMENT').'</th>
					<th>'.JText::_('FIELD_LABEL_INVOICE').'</th>
					<th class="text-center">'.JText::_('FIELD_LABEL_IS_CARD').'</th>
					'.$adminView['head']['actions'].'
				</tr>
			</thead>
			<tbody>
...
if($num_rows) : // verifica se existe
  ...
  foreach($res as $item) {
    ...
    if($item->parent_id != $currID || $fInst == 1) :
  		...
  		$invoice = $item->invoice_id != 0 ? $usergroup.'<div class="small text-live font-featured">'.baseHelper::getMonthName($item->invoiceMonth).' de '.$item->invoiceYear.'</div>' : '';
      ...
      $rowState = $item->state == 0 ? 'danger' : '';
  		$html .= '
  			<tr id="'.$APPTAG.'-item-'.$item->id.'" class="'.$rowState.'">
  				...
  				<td>'.$item->installment.'/'.$item->total.'</td>
  				<td>'.$invoice.'</td>
  				<td class="text-center">'.$isCard.'</td>
					'.$adminView['list']['actions'].'
  			</tr>
  		';
