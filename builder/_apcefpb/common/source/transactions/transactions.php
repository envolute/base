...
jQuery(function() {
	...
	var fixed 				= jQuery('#<?php echo $APPTAG?>-fixed');
	var isCard 				= jQuery('#<?php echo $APPTAG?>-isCard');
	...
	// métodos controladores do formulário
		...
		window.<?php echo $APPTAG?>_formReset = function() {
			...
			// visualiza os campos não editáveis
			jQuery('.<?php echo $APPTAG?>-no-edit').removeClass('element-invisible');

			// App Fields
			// IMPORTANTE:
			// => SE HOUVER UM CAMPO INDICADO NA VARIÁVEL 'parentFieldId', NÃO RESETÁ-LO NA LISTA ABAIXO
			client_id.val('0').trigger("chosen:updated"); // select
			dependent_id.val('0').trigger("chosen:updated"); // select
			invoice_id.val('0').trigger("chosen:updated"); // select
			description.val(''); // select
			fixed.prop('checked', false);
				// mostra campos da movimentação normal
				jQuery('.<?php echo $APPTAG?>-no-fixed').removeClass('hide');
			isCard.prop('checked', false);
			cardLimit.val('');
				// define o limite e a parcela mínima -> '1'
				<?php echo $APPTAG?>_setCard(1);
				total.val(1).trigger("chosen:updated"); // select
			date.val('');
			price.val('');
			doc_number.val('');
			note.val('');
			...
			mainForm.find('.set-success, .set-error, #btn-<?php echo $APPTAG?>-save-new, #btn-<?php echo $APPTAG?>-delete').addClass('hide');
		...
		fixed.on('change', function() {
			if(jQuery(this).is(':checked')) {
				jQuery('.<?php echo $APPTAG?>-no-fixed').addClass('hide');
				isCard.prop('checked', false);
				isCard.closest('label.btn').removeClass('active');
				<?php echo $APPTAG?>_setCard(1);
				invoice_id.val(0).trigger("chosen:updated");
			} else {
				jQuery('.<?php echo $APPTAG?>-no-fixed').removeClass('hide');
			}
		});
		...
		window.<?php echo $APPTAG?>_editInvoice = function() {
			...
			if(itemID != '' && itemID != 0) invoices_loadEditFields(itemID, false, false);
		...
		window.<?php echo $APPTAG?>_setCard = function(value) {
			var e = (typeof value !== "null" && typeof value !== "undefined") ? value : 1;
			total.empty();
			var limit = isCard.is(':checked') ? 3 : 90;
			for(i = 1; i <= limit; i++) {
				total.append('<option value="'+i+'">'+i+'</option>');
			}
			total.val(e).trigger("chosen:updated");
			price.val('');
		};
	...
	// métodos controladores das ações referente ao banco de dados e envio de arquivos
			...
			jQuery.ajax({
				...
				success: function(data) {
					jQuery.map( data, function( item ) {
						...
						// App Fields
						provider_id.val(item.provider_id).trigger("chosen:updated"); // select
						client_id.val(item.client_id).trigger("chosen:updated"); // select
							<?php echo $APPTAG?>_getDependentList(item.client_id, item.dependent_id);
						invoice_id.val(item.invoice_id).trigger("chosen:updated"); // select
						description.val(item.description);
						fixed.prop('checked', (item.fixed == 1 ? true : false));
							// esconde campos da movimentação normal
							if(item.fixed == 1) jQuery('.<?php echo $APPTAG?>-no-fixed').addClass('hide');
							else jQuery('.<?php echo $APPTAG?>-no-fixed').removeClass('hide');
						isCard.prop('checked', (item.isCard == 1 ? true : false));
					...
					jQuery('#btn-<?php echo $APPTAG?>-save-new, #btn-<?php echo $APPTAG?>-delete').removeClass('hide');
							...
							if(res.status > 0) { // se alguma ação for realizada
								...
								if(res.parentField != '' && res.parentFieldVal != '') {
									...
									if(res.parentFieldLabel != '' && res.parentFieldLabel != null) {
										...
										// CUSTOM: set url value and focus
										jQuery('#webSocials-page').val(res.parentFieldUrl);
									} else {
										jQuery(res.parentField).trigger("chosen:updated");
										// CUSTOM: clear url value
										jQuery('#webSocials-page').val('');
									}
								}
		...
		window.<?php echo $APPTAG?>_getDependentList = function(itemID, id) {
			<?php echo $APPTAG?>_formExecute(true, true, true); // inicia o loader
			jQuery.ajax({
				url: "<?php echo JURI::root().'templates/base/source/'.$APPNAME.'/'.$APPNAME ?>.model.php?task=cList&cID="+itemID,
				dataType: 'json',
				type: 'POST',
				cache: false,
				success: function(data){
					...
					jQuery.map( data, function( res, i ) {
						if(res.status != 0) {
							...
							if(typeof id !== "null" && typeof id !== "undefined" && id != 0) dependent_id.val(id);
							dependent_id.trigger("chosen:updated"); // atualiza o select
						} else {
							if(!itemID) mainForm.find('.set-error').removeClass('hide').text(res.msg);
						}
					});
      ...
      if(!confirm('<?php echo JText::_('MSG_INVOICED_CONFIRM'); ?>')) return false;
			...
			jQuery.ajax({
				url: "<?php echo JURI::root().'templates/base/source/'.$APPNAME.'/'.$APPNAME ?>.model.php?aTag=<?php echo $APPTAG?>&rTag=<?php echo $RTAG?>&task=invoice&st="+invID,
				dataType: 'json',
				type: 'POST',
				data:  dados,
				cache: false,
      ...
      if(!confirm('<?php echo JText::_('MSG_REMOVE_INVOICED_CONFIRM'); ?>')) return false;
			...
			jQuery.ajax({
				url: "<?php echo JURI::root().'templates/base/source/'.$APPNAME.'/'.$APPNAME ?>.model.php?aTag=<?php echo $APPTAG?>&rTag=<?php echo $RTAG?>&task=removeInvoice",
				dataType: 'json',
				type: 'POST',
				data:  dados,
				cache: false,
		...
		window.<?php echo $APPTAG?>_getInvoiceFile = function(invID) {
			...
			if(invID == 0) {
        alert('<?php echo JText::_('MSG_INVOICE_NOT_SELECTED'); ?>');
        return false;
      }
			if(seq == "") {
        alert('<?php echo JText::_('MSG_INVOICE_NOT_SEQUENTIAL'); ?>');
        return false;
      }
      if(!confirm('<?php echo JText::_('MSG_INVOICE_FILE_CONFIRM'); ?>')) return false;

			jQuery.ajax({
				url: "<?php echo JURI::root().'templates/base/source/'.$APPNAME.'/'.$APPNAME ?>.model.php?aTag=<?php echo $APPTAG?>&rTag=<?php echo $RTAG?>&task=invoiceFile&st="+invID+"&sq="+seq,
				dataType: 'json',
				cache: false,
				success: function(data) {
					jQuery.map( data, function( res ) {
      ...
      if(!confirm('<?php echo JText::_('MSG_ADDFIXED_CONFIRM'); ?>')) return false;
			jQuery.ajax({
				url: "<?php echo JURI::root().'templates/base/source/'.$APPNAME.'/'.$APPNAME ?>.model.php?aTag=<?php echo $APPTAG?>&rTag=<?php echo $RTAG?>&task=addFixed&st="+groupID,
				dataType: 'json',
				cache: false,
				success: function(data){
					jQuery.map( data, function( res ) {
