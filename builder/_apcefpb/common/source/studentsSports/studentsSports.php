...
jQuery(function() {
	...
	// APP FIELDS
	var student_id 		= jQuery('#<?php echo $APPTAG?>-student_id');
	var sport_id 			= jQuery('#<?php echo $APPTAG?>-sport_id');
	var registry_date	= jQuery('#<?php echo $APPTAG?>-registry_date');
	var coupon_free		= jQuery('#<?php echo $APPTAG?>-coupon_free');
	...
	// métodos controladores do formulário
		...
		cardPopup.on('hidden.bs.modal', function () {
			// limpa a validação quando o formulário é fechado
			jQuery('#<?php echo $APPTAG?>-form-card-iframe').attr("src", "");
		...
		window.<?php echo $APPTAG?>_formReset = function() {
			...
			sport_id.val('0').trigger("chosen:updated"); // select
			registry_date.val('');
			coupon_free.prop('checked', false);
			price.val(''); // select
			note.val('');
		...
		window.<?php echo $APPTAG?>_setPrintCard = function(elem){
			document.getElementById(elem).contentWindow.print();
		}

		// CUSTOM -> edit from select
		window.<?php echo $APPTAG?>_editStudent = function() {
			var itemID = student_id.val();
			if(itemID != '' && itemID != 0) students_loadEditFields(itemID, false, false);
			...
			jQuery.ajax({
				...
				success: function(data) {
					jQuery.map( data, function( item ) {
						...
						student_id.val(item.student_id).trigger("chosen:updated"); // selects
						sport_id.val(item.sport_id).trigger("chosen:updated"); // selects
						registry_date.val(dateFormat(item.registry_date));
						coupon_free.prop('checked', (item.coupon_free == 1 ? true : false));
						price.val(item.price);
						note.val(item.note);
						<?php echo $APPTAG?>_printCard(item.id, 1);
