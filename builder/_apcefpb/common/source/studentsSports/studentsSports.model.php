...
header( 'content-type: application/json; charset=utf-8' );
  ...
  if($task != null) :
    ...
    // app
  	$request['student_id']   = $input->get('student_id', 0, 'int');
  	$request['sport_id']     = $input->get('sport_id', 0, 'int');
  	$request['registry_date'] = $input->get('registry_date', '', 'string');
  	$request['coupon_free']  = $input->get('coupon_free', 0, 'int');
      ...
      if($id) :
        ...
        // COUNT LIST IDS
  		...
  		if($num_rows || $exist) : // verifica se existe
            ...
            // App Fields
    				...
    				'registry_date' => $item->registry_date,
    				'coupon_free' => $item->coupon_free,
            'price'	      => $item->price,
    				'note'        => $item->note
    			);
  			...
  			elseif($task == 'save' && $id) :
  				...
  				$query .=
            $db->quoteName('student_id') 	.'='. $request['student_id'] .','.
            $db->quoteName('sport_id') 	.'='. $request['sport_id'] .','.
  					$db->quoteName('registry_date') .'='. $db->quote($request['registry_date']) .','.
            $db->quoteName('coupon_free') 	.'='. $request['coupon_free'] .','.
            ...
            // UPDATE FIELD
            $element = $elemVal = $elemLabel = $elemPrice = '';
            if(!empty($_SESSION[$RTAG.'FieldUpdated']) && !empty($_SESSION[$RTAG.'TableField'])) :
              $element = $_SESSION[$RTAG.'FieldUpdated'];
              $elemVal = $id;
              $query = 'SELECT * FROM '. $db->quoteName($cfg['mainTable']).' WHERE id='.$id.' AND state = 1';
              $db->setQuery($query);
              $elems = $db->loadObject();
              $elemLabel = $elems->name;
              $elemPrice = $elems->price;
            endif;
              ...
              'uploadError' => $fileMsg,
    					...
    					'parentFieldPrice'	=> baseHelper::priceFormat($elemPrice)
  					);
          ...
          $query = 'SELECT '. $db->quoteName('student_id') .' FROM '. $db->quoteName($cfg['mainTable']) .' WHERE '. $db->quoteName('id') .' IN ('.$ids.')';
          $db->setQuery($query);
          $uList = $db->loadObjectList();
            ...
            if(!empty($_SESSION[$RTAG.'RelTable']) && !empty($_SESSION[$RTAG.'AppNameId'])) :
              ...
              // $query = 'DELETE FROM '. $db->quoteName('#__apcefpb_rel_clients_contacts') .' WHERE '. $db->quoteName('contact_id') .' IN ('.$ids.')';
              // $db->setQuery($query);
    					// $db->execute();
            endif;
        ...
        elseif($task == 'state') :
          ...
          try {
            ...
            if(!empty($_SESSION[$RTAG.'FieldUpdated']) && !empty($_SESSION[$RTAG.'TableField'])) :
              $element = $_SESSION[$RTAG.'FieldUpdated'];
              $elemVal = $ids;
              $query = 'SELECT * FROM '. $db->quoteName($cfg['mainTable']).' WHERE '. $db->quoteName('id') .' = '.$ids;
              $db->setQuery($query);
              $elems = $db->loadObject();
              $elemLabel = $elems->name;
              $elemPrice = $elems->price;
        ...
        // validation
        if($request['student_id'] != 0 && $request['sport_id'] != 0) :

          // Prepare the insert query
    			$query  = 'INSERT INTO '. $db->quoteName($cfg['mainTable']) .'('.
            $db->quoteName('student_id') .','.
            $db->quoteName('sport_id') .','.
    				$db->quoteName('registry_date') .','.
    				$db->quoteName('coupon_free') .','.
            $db->quoteName('price') .','.
    				$db->quoteName('note') .','.
    				$db->quoteName('state') .','.
            ...
            $request['student_id'] .','.
            $request['sport_id'] .','.
    				$db->quote($request['registry_date']) .','.
            $request['coupon_free'] .','.
            ...
            if(!empty($_SESSION[$RTAG.'FieldUpdated']) && !empty($_SESSION[$RTAG.'TableField'])) :
              ...
              'uploadError' => $fileMsg,
    					'parentField'	=> $element,
    					'parentFieldVal'	=> $elemVal,
    					'parentFieldPrice'	=> baseHelper::nameFormat($elemLabel),
    					'parentFieldLabel'	=> baseHelper::priceFormat($elemPrice)
    				);
