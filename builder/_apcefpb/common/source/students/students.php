...
jQuery(function() {
	...
	var emailConfirm2	= jQuery('#<?php echo $APPTAG?>-emailConfirm2');
	...
	// métodos controladores do formulário
		...
		window.<?php echo $APPTAG?>_formReset = function() {
			...
			selectRadio(has_disease, 0); // radio
			disease_desc.val('').prop('disabled', true);
			selectRadio(has_allergy, 0); // radio
			allergy_desc.val('').prop('disabled', true);
			blood_type.val('');
			note.val('');
			name_card.val('');
			<?php echo $APPTAG?>_resetRegistrationForm();
			emailConfirm2.prop('checked', true);
			emailConfirm2.closest('.checkbox').removeClass('hide');
		...
		userRegistration.change(function() {
			if(formId.val() == 0 || formId.val() == '') emailConfirm2.closest('.checkbox').toggleClass('hide');
		});
		...
		window.<?php echo $APPTAG?>_setType = function(e){
			if(e == 1) {
				...
				jQuery('#<?php echo $APPTAG?>-company').show();
			} else {
				setCPF(doc_number);
				doc_number.prev('label').text('CPF');
				jQuery('#<?php echo $APPTAG?>-company').hide();
			}
		};
            ...
            if(item.user_id != 0) {
              if(item.block == 0) {
                ...
                jQuery('#<?php echo $APPTAG?>-block-1').addClass('hidden');
						...
						name.val(item.name);
						birthday.val(dateFormat(item.birthday));
						...
						selectRadio(has_disease, item.has_disease); // radio
						disease_desc.val(item.disease_desc).prop('disabled', (item.has_disease == 0 ? true : false));
						selectRadio(has_allergy, item.has_allergy); // radio
						allergy_desc.val(item.allergy_desc).prop('disabled', (item.has_allergy == 0 ? true : false));
						blood_type.val(item.blood_type);
						note.val(item.note);
						name_card.val(item.name_card);
						emailConfirm.prop('checked', true);
						emailConfirm2.prop('checked', false);
						emailConfirm2.closest('.checkbox').addClass('hide');
