		...
		<table class="table table-striped table-hover table-condensed">
			<thead>
				<tr>
					...
					<th class="text-center" width="80">'.$$SETOrder(JText::_('FIELD_LABEL_DISEASE').'?', 'T1.has_disease', $APPTAG).'</th>
					<th class="text-center" width="80">'.$$SETOrder(JText::_('FIELD_LABEL_ALLERGY').'?', 'T1.has_allergy', $APPTAG).'</th>
					<th class="text-center" width="80">'.$$SETOrder(JText::_('FIELD_LABEL_BLOOD'), 'T1.blood_type', $APPTAG).'</th>
...
if($num_rows) : // verifica se existe
	...
	foreach($res as $item) {
		...
		if(!empty($item->mother_name) || !empty($item->father_name) || !empty($item->blood_type)) :
			...
			$info = ' <span class="base-icon-info-circled text-info cursor-help hasPopover" data-placement="top" data-content="'.$info.'"></span>';
		endif;
		$health = '-';
		if($item->has_disease == 1) :
			$desc = !empty($item->disease_desc) ? ' hasPopover" data-placement="top" data-content="'.$item->disease_desc : '';
		 	$health = ' <span class="base-icon-attention text-danger cursor-help'.$desc.'"></span>';
		endif;
		$allergy = '-';
		...
		$html .= '
			<tr id="'.$APPTAG.'-item-'.$item->id.'" class="'.$rowState.'">
				...
				<td>'.baseHelper::dateFormat($item->birthday).'</td>
