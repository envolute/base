...
// filter params
	...
	if(isset($fUser) && $fUser == 1) $where .= ' AND '.$db->quoteName('T2.username').' IS NULL';
	// DATE
	...
	if(!empty($search)) :
		...
		$where .= ' OR LOWER('.$db->quoteName('T1.disease_desc').') LIKE LOWER("%'.$search.'%")';
		$where .= ' OR LOWER('.$db->quoteName('T1.allergy_desc').') LIKE LOWER("%'.$search.'%")';
