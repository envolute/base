...
if(isset($_SERVER["HTTP_X_REQUESTED_WITH"]) AND strtolower($_SERVER["HTTP_X_REQUESTED_WITH"]) == "xmlhttprequest") :
	...
	if($num_rows) : // verifica se existe
		...
		foreach($res as $item) {
			...
			$hosting = ($item->hosting == 1) ? JText::_('FIELD_LABEL_HOSTING').' - ' : '';
			$price_public = ($item->price_public != '0.00') ? '<small class="text-muted font-featured cursor-help hasTooltip" title="Preço para Não Associado">'.baseHelper::priceFormat($item->price_public, false, '&raquo; R$ ').'</small>' : '';
			...
			$html .= '
				<li class="'.$rowState.'">
					...
					'.$main.baseHelper::nameFormat($item->name).'<br />
					<small class="text-muted font-featured">
						'.$hosting.'Capacidade: '.$item->capacity.'pessoas - Acesso: '.$acesso.' - Preço: '.baseHelper::priceFormat($item->price, false, 'R$ ').$price_public.'
					</small>
				</li>
			';
		}
