...
// LIST
	...
	$query = '
		SELECT SQL_CALC_FOUND_ROWS
			...
			'. $db->quoteName('T1.capacity') .',
			'. $db->quoteName('T1.price') .',
			'. $db->quoteName('T1.price_brazil') .',
			'. $db->quoteName('T1.price_public') .',
			'. $db->quoteName('T1.max_time') .',
		...
		<table class="table table-striped table-hover table-condensed">
			<thead>
				<tr>
					...
					<th>'.JText::_('FIELD_LABEL_CAPACITY').'</th>
					<th>'.$$SETOrder(JText::_('FIELD_LABEL_ACCESS'), 'T1.access', $APPTAG).'</th>
					<th>'.$$SETOrder(JText::_('FIELD_LABEL_PRICE'), 'T1.price', $APPTAG).'</th>
					<th>'.$$SETOrder(JText::_('FIELD_LABEL_MAX_TIME'), 'T1.max_time', $APPTAG).'</th>
					<th>'.$$SETOrder(JText::_('FIELD_LABEL_START_HOUR'), 'T1.start_hour', $APPTAG).'</th>
					<th>'.$$SETOrder(JText::_('FIELD_LABEL_END_HOUR'), 'T1.end_hour', $APPTAG).'</th>
					'.$adminView['head']['actions'].'
				</tr>
			</thead>
			<tbody>
...
if($num_rows) : // verifica se existe
	...
	foreach($res as $item) {
		...
		$hosting = ($item->hosting == 1) ? '<div class="small text-muted font-featured cursor-help hasTooltip" title="'.JText::_('FIELD_LABEL_HOSTING_DESC').'">'.JText::_('FIELD_LABEL_HOSTING').'</div>' : '';
		$price_brazil = ($item->price_brazil != '0.00') ? '<div class="small text-muted font-featured cursor-help hasTooltip" title="'.JText::_('FIELD_LABEL_PRICE_BRAZIL_DESC').'">'.baseHelper::priceFormat($item->price_brazil, false, '&raquo; R$ ').'</div>' : '';
		$price_public = ($item->price_public != '0.00') ? '<div class="small text-muted font-featured cursor-help hasTooltip" title="'.JText::_('FIELD_LABEL_PRICE_PUBLIC_DESC').'">'.baseHelper::priceFormat($item->price_public, false, '&raquo; R$ ').'</div>' : '';
		$rowState = $item->state == 0 ? 'danger' : '';
		$html .= '
			<tr id="'.$APPTAG.'-item-'.$item->id.'" class="'.$rowState.'">
				'.$adminView['list']['info'].'
				<td>'.baseHelper::nameFormat($item->name).$hosting.'</td>
				<td>'.$item->capacity.' pessoas</td>
				<td>'.$access.'</td>
				<td>'.baseHelper::priceFormat($item->price, false, 'R$ ').$price_brazil.$price_public.'</td>
				<td>'.substr($item->max_time, 0, 5).'</td>
				<td>'.substr($item->start_hour, 0, 5).'</td>
				<td>'.substr($item->end_hour, 0, 5).'</td>
				'.$adminView['list']['actions'].'
			</tr>
		';
	}
