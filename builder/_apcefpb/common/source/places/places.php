<?php
/* SISTEMA PARA CADASTRO DE PLANOS DE HOSPEDAGEM
 * AUTOR: IVO JUNIOR
 * EM: 18/02/2016
*/
...
jQuery(function() {
	...
	var capacity			= jQuery('#<?php echo $APPTAG?>-capacity');
	var description		= jQuery('#<?php echo $APPTAG?>-description');
	var price					= jQuery('#<?php echo $APPTAG?>-price');
	var price_brazil	= jQuery('#<?php echo $APPTAG?>-price_brazil');
	var price_public	= jQuery('#<?php echo $APPTAG?>-price_public');
	var max_time			= jQuery('#<?php echo $APPTAG?>-max_time');
	...
	// métodos controladores do formulário
		...
		window.<?php echo $APPTAG?>_formReset = function() {
			...
			selectRadio(access, 0);
			hosting.prop('checked', false); // checkbox -> default: not checked
			name.val('');
			capacity.val('');
			description.val('');
			price.val('');
			price_brazil.val('');
			price_public.val('');
			<?php echo $APPTAG?>_priceDisable(true);
			max_time.val('').trigger("chosen:updated"); // select
			start_hour.val('');
			end_hour.val('');
			note.val('');
		...
		// ENABLE/DISABLE PRICE PUBLIC
		window.<?php echo $APPTAG?>_priceDisable = function(e) {
			price_public.prop('disabled', e);
		};
			...
			jQuery.ajax({
				...
				success: function(data) {
					jQuery.map( data, function( item ) {
						...
						// App Fields
						selectRadio(access, item.access);
						hosting.prop('checked', (item.hosting == 0 ? false : true));
						name.val(item.name);
						capacity.val(item.capacity);
						description.val(item.description);
						price.val(item.price);
						price_brazil.val(item.price_brazil);
						price_public.val(item.price_public);
						<?php echo $APPTAG?>_priceDisable((item.access != 2 ? true : false));
						max_time.val(item.max_time).trigger("chosen:updated"); // select
