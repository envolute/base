...
// filter params
	...
	// HOSTING -> select
	$fHost	= $app->input->get('fHost', 2, 'int');
	$where .= ($fHost == 2) ? '' : ' AND '.$db->quoteName('T1.hosting').' = '.$fHost;
	// price
		...
		<fieldset class="fieldset-embed filter '.((isset($_GET[$APPTAG.'_filter']) || $cfg['showFilter']) ? '' : 'closed').'">
            ...
            <span class="input-group">
              <span class="input-group-addon strong">R$</span>
				...
				<div class="col-sm-2">
					<div class="form-group">
						<select name="fHost" id="fHost" class="form-control input-sm set-filter">
							<option value="2">- '.JText::_('FIELD_LABEL_HOSTING').' -</option>
							<option value="1"'.($fHost == 1 ? ' selected' : '').'>'.JText::_('TEXT_YES').'</option>
							<option value="0"'.($fHost == 0 ? ' selected' : '').'>'.JText::_('TEXT_NO').'</option>
						</select>
					</div>
				</div>
				<div class="col-sm-2">
