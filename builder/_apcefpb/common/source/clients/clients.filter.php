...
// filter params
	...
	if(!empty($search)) :
		...
		$where .= ' OR LOWER('.$db->quoteName('T1.mother_name').') LIKE LOWER("%'.$search.'%")';
		$where .= ' OR LOWER('.$db->quoteName('T1.father_name').') LIKE LOWER("%'.$search.'%")';
		$where .= ' OR LOWER('.$db->quoteName('T1.cx_email').') LIKE LOWER("%'.$search.'%")';
		$where .= ' OR LOWER('.$db->quoteName('T1.cx_matricula').') LIKE LOWER("%'.$search.'%")';
		$where .= ' OR LOWER('.$db->quoteName('T1.cx_lotacao').') LIKE LOWER("%'.$search.'%")';
		$where .= ' OR LOWER('.$db->quoteName('T1.cx_cargo').') LIKE LOWER("%'.$search.'%")';
		$where .= ')';
	endif;
