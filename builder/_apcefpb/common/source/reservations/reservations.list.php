		...
		<table class="table table-striped table-hover table-condensed">
			<thead>
				<tr>
					...
					<th>'.JText::_('FIELD_LABEL_CONFIRMED').'</th>
					'.$adminView['head']['actions'].'
				</tr>
			</thead>
			<tbody>
...
if($num_rows) : // verifica se existe
	...
	foreach($res as $item) {
		...
		$client = !empty($item->client) ? baseHelper::nameFormat($item->client) : '';
		$client = !empty($item->contact) ? '<span class="base-icon-user text-live cursor-help hasTooltip" title="Não é Associado"></span> '.baseHelper::nameFormat($item->contact) : $client;
		$period = '';
		...
		$confirmed = $item->confirmed == 1 ? '<span class="base-icon-ok text-success"> Confirmado</span>' : '';
		$confirmed = $item->confirmed == 1 ? $confirmed : '<a href="#" id="'.$APPTAG.'-confirm-'.$item->id.'" class="base-icon-clock text-live hasTooltip" title="Aguardando confirmação" onclick="'.$APPTAG.'_setConfirm('.$item->id.', 1)">'.$confirmed.'</a>';
		$rowState = $item->state == 0 ? 'danger' : '';
		$html .= '
			<tr id="'.$APPTAG.'-item-'.$item->id.'" class="'.$rowState.'">
				'.$adminView['list']['info'].'
				<td>'.baseHelper::nameFormat($item->place).'</td>
				<td>'.$client.'</td>
