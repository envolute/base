...
jQuery(function() {
	...
	var contact_id 		= jQuery('#<?php echo $APPTAG?>-contact_id');
	var place_id 			= jQuery('#<?php echo $APPTAG?>-place_id');
	var confirmed			= jQuery('#<?php echo $APPTAG?>-confirmed');
	var date 					= jQuery('#<?php echo $APPTAG?>-date');
	var time 					= jQuery('#<?php echo $APPTAG?>-time');
	var amount 				= jQuery('#<?php echo $APPTAG?>-amount');
	var guests 				= jQuery('#<?php echo $APPTAG?>-guests');
	var price 				= jQuery('#<?php echo $APPTAG?>-price');
	var phones 				= jQuery('#<?php echo $APPTAG?>-phones');
	var note 					= jQuery('#<?php echo $APPTAG?>-note');
	var client_alert	= jQuery('#<?php echo $APPTAG?>-client_alert');
	var extra_info		= jQuery('#<?php echo $APPTAG?>-extra_info');

	// PARENT FIELD
	// informe, se houver, o campo que representa a chave estrangeira principal
	var parentFieldId			= client_id;
	...
	// métodos controladores do formulário
		...
		popup.on('hidden.bs.modal', function () {
			...
			if(parentFieldId != null) {
				...
				// CUSTOM
				jQuery('#<?php echo $APPTAG?>-group-contact').removeClass('hide');
			}
			<?php
		...
		window.<?php echo $APPTAG?>_formReset = function() {
			...
			contact_id.val(0).trigger("chosen:updated"); // selects;
			place_id.val(0).trigger("chosen:updated"); // selects;
			// alert & confirm
			<?php echo $APPTAG?>_setClientAlert(0);
			client_alert.prop('checked', true);
			date.val('');
			amount.val(0).trigger("chosen:updated"); // selects;
				// reseta o limite de tempo, nº de convidados e preço
				window.<?php echo $APPTAG?>_resetPlace();
			phones.val('');
			note.val('');
		...
		// CLIENT ACTIONS
		window.<?php echo $APPTAG?>_setClient = function(edit) {
			if(!edit) <?php echo $APPTAG?>_formReset();
			if(client_id.val() != 0) {
				jQuery('#<?php echo $APPTAG?>-group-contact').addClass('hide');
			} else {
				jQuery('#<?php echo $APPTAG?>-group-contact').removeClass('hide');
			}
		};
		...
		// NOT CLIENT ACTIONS
		window.<?php echo $APPTAG?>_setContact = function(edit) {
			if(!edit) <?php echo $APPTAG?>_formReset();
			var cID = contact_id.val();
			setTimeout(function() {
				if(cID != 0) {
					jQuery('#<?php echo $APPTAG?>-group-client').addClass('hide');
				} else {
					jQuery('#<?php echo $APPTAG?>-group-client').removeClass('hide');
				}
				contact_id.val(cID).trigger("chosen:updated"); // selects
			}, 100);
		};

		// SET PLACE -> atribui valores de acordo com o local
		window.<?php echo $APPTAG?>_setPlace = function(id) {
			if(place_id.find('option[value="'+id+'"]').length) {
				var option = place_id.find('option[value="'+id+'"]');
				var contact = contact_id.val();

				if(id == 0) {

					<?php echo $APPTAG?>_resetPlace(); // reseta os valores de acordo com o local
					jQuery('#<?php echo $APPTAG?>-group-time, #<?php echo $APPTAG?>-group-amount').addClass('hide');

				} else {
					var host = option.data('host');
					var maxTime = option.data('maxTime');
					var capacity = option.data('capacity');
					var cost = option.data('price');
					var cost1 = option.data('priceBrazil');
					var cost2 = option.data('pricePublic');

					<?php echo $APPTAG?>_resetPlace(); // reseta os valores de acordo com o local

					// set amount
					if(host == 1) {
						jQuery('#<?php echo $APPTAG?>-group-time').addClass('hide');
						jQuery('#<?php echo $APPTAG?>-group-amount').removeClass('hide');
					} else {
						jQuery('#<?php echo $APPTAG?>-group-time').removeClass('hide');
						jQuery('#<?php echo $APPTAG?>-group-amount').addClass('hide');
					}

					// TIME -> load max time from local limit
					for(i = 1; i <= maxTime; i++) {
						var x = (i < 10) ? '0'+i : i;
						var h = (i > 1) ? ' hs' : ' h';
						time.append('<option value="'+x+':00:00">'+x+h+'</option>').trigger("chosen:updated"); // selects
					}

					// GUESTS -> load guests from local limit
					for(i = 1; i <= capacity; i++) {
						guests.append('<option value="'+i+'">'+i+'</option>').trigger("chosen:updated"); // selects
					}
					...
					if(client_id.val() != 0) {
						...
						if(contact != 0) {
							var grp = contact_id.find('option[value="'+contact+'"]').data('group');
							if(grp == 2) c = cost1;
							else c = cost2;
						}
					}
		...
		// RESET PLACE -> reseta dos valores de acordo com o local
		window.<?php echo $APPTAG?>_resetPlace = function() {
			time.empty().append('<option value="00:00:00">- <?php echo JText::_('TEXT_SELECT'); ?> -</option>').trigger("chosen:updated"); // selects;
			guests.empty().append('<option value="0">- <?php echo JText::_('TEXT_SELECT'); ?> -</option>').trigger("chosen:updated"); // selects;
			price.val('');
		};

		// SET CLIENT ALERT
		window.<?php echo $APPTAG?>_setClientAlert = function(e) {
			if(e == 1) {
				confirmed.prop('checked', true);
				confirmed.prop('disabled', true);
				confirmed.closest('label.btn').addClass('disabled');
				jQuery('#<?php echo $APPTAG?>-group-client_alert').addClass('hide');
			} else {
				confirmed.prop('checked', false);
				confirmed.prop('disabled', false);
				confirmed.closest('label.btn').removeClass('disabled');
				jQuery('#<?php echo $APPTAG?>-group-client_alert').removeClass('hide');
			}
			extra_info.val('');
		}

		// BTN CONFIRM
		confirmed.closest('label.btn').off('click').on('click', function() {
			if(formId.val() != 0) <?php echo $APPTAG?>_setConfirm(formId.val(), confirmed.prop('checked'));
		});

		// CUSTOM -> edit from select
		window.<?php echo $APPTAG?>_editContact = function() {
			var itemID = contact_id.val();
			if(itemID != '' && itemID != 0) contacts_loadEditFields(itemID, false, false);
	...
	// métodos controladores das ações referente ao banco de dados e envio de arquivos
			...
			jQuery.ajax({
				...
				success: function(data) {
					jQuery.map( data, function( item ) {
						...
						// App Fields
						client_id.val(item.client_id).trigger("chosen:updated"); // selects
							if(item.client_id != 0) <?php echo $APPTAG?>_setClient(true);
						contact_id.val(item.contact_id).trigger("chosen:updated"); // selects
							if(item.contact_id != 0) <?php echo $APPTAG?>_setContact(true);
						place_id.val(item.place_id).trigger("chosen:updated"); // selects
							<?php echo $APPTAG?>_setPlace(item.place_id);
						// alert & confirm
						<?php echo $APPTAG?>_setClientAlert(item.confirmed);
						client_alert.prop('checked', false);
						date.val(dateFormat(item.date));
						time.val(item.time).trigger("chosen:updated"); // selects
						amount.val(item.amount).trigger("chosen:updated"); // selects
						guests.val(item.guests).trigger("chosen:updated"); // selects
						price.val(item.price);
						phones.val(item.phones);
						note.val(item.note);
		...
		// CUSTOM -> Set confirm
		// confirma a reserva
		window.<?php echo $APPTAG?>_setConfirm = function(itemID, state) {
			var msg = (itemID) ? '<?php echo JText::_('MSG_RESERVATION_CONFIRM'); ?>' : '<?php echo JText::_('MSG_RESERVATION_LISTCONFIRM'); ?>';
			if(confirm(msg)) {
				var st = e = '';
				var msg = '<?php echo JText::_('MSG_RESERVATION_LIST0CONFIRM'); ?>';
				if(state === 1) msg = '<?php echo JText::_('MSG_RESERVATION_LIST1CONFIRM'); ?>';
				if(typeof state !== "null" && typeof state !== "undefined") st = '&st='+state;
				cod = '&id='+itemID;
				...
				// CUSTOM -> SET CONFIRM
				jQuery.ajax({
					url: "<?php echo JURI::root().'templates/base/source/'.$APPNAME.'/'.$APPNAME ?>.model.php?aTag=<?php echo $APPTAG?>&rTag=<?php echo $RTAG?>&task=confirm"+cod+st,
					dataType: 'json',
					type: 'POST',
					cache: false,
					success: function(data){
						...
						jQuery.map( data, function( res ) {
							if(res.status == 7) {
								e = list.find('#<?php echo $APPTAG?>-confirm-'+res.id);
								if(res.state == 1) {
									e.after('<span class="base-icon-ok text-success"> Confirmado</span>');
									e.remove();
									<?php if($cfg['listFull']) echo 'e.parents("tr").removeClass("warning");'?>
								}
								<?php if(!$cfg['listFull']) echo $APPTAG.'_listReload(false, false, false, '.$APPTAG.'oCHL, '.$APPTAG.'rNID, '.$APPTAG.'rID);'; ?>
							} else {
								if(!itemID) mainForm.find('.set-error').removeClass('hide').text(res.msg);
