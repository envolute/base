...
// filter params
	...
	// PLACE -> select
	$fPlace	= $app->input->get('fPlace', 0, 'int');
	if($fPlace != 0) $where .= ' AND '.$db->quoteName('T1.place_id').' = '.$fPlace;
	// CLIENT BRAZIL -> checkbox
	$fCbr	= $app->input->get('fCbr', 0, 'int');
	if(isset($fCbr) && $fCbr == 1) $where .= ' AND '.$db->quoteName('T1.client_br').' = 1';
	// DATE
	...
	if(!empty($search)) :
		...
		$where .= ' AND LOWER('.$db->quoteName('T1.email').') LIKE LOWER("%'.$search.'%")';
		$where .= ')';
	endif;
...
// FILTER'S DINAMIC FIELDS
	...
	// types -> select
  $flt_place = '';
  $query = 'SELECT * FROM '. $db->quoteName('#__apcefpb_places') .' ORDER BY name';
  $db->setQuery($query);
  $places = $db->loadObjectList();
  foreach ($places as $obj) {
    $flt_place .= '<option value="'.$obj->id.'"'.($obj->id == $fPlace ? ' selected = "selected"' : '').'>'.$obj->name.'</option>';
  }
			...
			<div class="row">
				<div class="col-sm-4">
					<div class="form-group">
						<select name="fPlace" id="fPlace" class="form-control input-sm set-filter">
							<option value="0">- '.JText::_('FIELD_LABEL_PLACE').' -</option>
							'.$flt_place.'
						</select>
					</div>
				</div>
				<div class="col-sm-2">
					<div class="form-group">
						<div class="btn-group btn-group-justified" data-toggle="buttons">
							<label class="btn btn-sm btn-warning btn-active-success set-filter hasTooltip" title="'.JText::_('FIELD_LABEL_CLIENT_BR_DESC').'" />
								<span class="base-icon-cancel btn-icon"></span>
								<input type="checkbox" name="fCbr" value="1"'.($fCbr == 1 ? ' checked' : '').' /> '.JText::_('FIELD_LABEL_CLIENT_BR').'
							</label>
						</div>
					</div>
				</div>
