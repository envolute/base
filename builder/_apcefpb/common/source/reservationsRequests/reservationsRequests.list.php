...
if($num_rows) : // verifica se existe
	...
	foreach($res as $item) {
		...
		$client = $item->client_id != 0 ? '<span class="base-icon-user text-success cursor-help hasTooltip" title="Associado"></span> '  : '';
		$client = $item->client_br == 1 ? '<span class="base-icon-user text-live cursor-help hasTooltip" title="Associado APCEF/'.$item->client_uf.'">'.$item->client_uf.'</span> ' : '<span class="base-icon-user text-danger cursor-help hasTooltip" title="Não é Associado"></span> ';
		$period = '';
		if($item->time != '00:00:00') $period = substr($request['time'], 0, 2).' hs';
		else if($item->amount > 0) $period = $request['amount'].($request['amount'] == 1 ? ' diária' : ' diárias');
		$note = !empty($item->note) ? '<span class="base-icon-info-circled text-live cursor-help hasTooltip" title="'.$item->note.'"></span> ' : '';
		$rowState = $item->state == 0 ? 'danger' : '';
		$html .= '
			<tr id="'.$APPTAG.'-item-'.$item->id.'" class="'.$rowState.'">
				'.$adminView['list']['info'].'
				<td>'.$note.baseHelper::nameFormat($item->place).'</td>
				<td>'.$client.$item->name.'<div class="small text-muted font-featured">'.$item->email.'</div></td>
				<td>'.baseHelper::dateFormat($item->date).'</td>
				<td>'.$period.'</td>
				<td>'.$item->guests.' pessoas</td>
				'.$adminView['list']['actions'].'
			</tr>
		';
	}
