...
jQuery(function() {
	...
	var client_br 		= jQuery('#<?php echo $APPTAG?>-client_br');
	var client_uf 		= jQuery('#<?php echo $APPTAG?>-client_uf');
	...
	// métodos controladores do formulário
		...
		window.<?php echo $APPTAG?>_formReset = function() {
			...
			client_id.val('<?php echo $user->id?>');
			client_br.prop('checked', false);
			client_uf.val('').trigger("chosen:updated"); // selects
			...
			if(place_id.find('option[value="'+id+'"]').length) {
				...
				if(id == 0) {
					...
					if(client_id.val() != '' && client_id.val() != 0) {
						...
						c = client_br.prop('checked') ? cost2 : cost1;
					}
					price.val(c);
					setPrice(price);
				}
			...
			jQuery.ajax({
				...
				success: function(data) {
					jQuery.map( data, function( item ) {
						...
						client_br.prop('checked', (item.client_br == 1 ? true : false));
						client_uf.val(item.client_uf).trigger("chosen:updated"); // selects
						name.val(item.name);
