...
$query = 'SELECT * FROM '. $db->quoteName($cfg['mainTable'].'_plans') .' ORDER BY name';
$db->setQuery($query);
			...
			<div class="row">
				...
				<div class="col-sm-3">
					<div class="form-group">
						...
						<select name="operator" id="<?php echo $APPTAG?>-operator" class="form-control">
							<option value="">- <?php echo JText::_('TEXT_SELECT'); ?> -</option>
							<?php
								for ($i = 0; $i < count($cfg['operators']); $i++) {
									echo '<option value="'.$cfg['operators'][$i].'">'.$cfg['operators'][$i].'</option>';
								}
							?>
						</select>
					</div>
				</div>
				<?php if($_SESSION[$RTAG.'ShowPlanId']): ?>
					<div class="col-sm-5">
						<div class="form-group">
							<label class="iconTip hasTooltip" title="<?php echo JText::_('FIELD_LABEL_PLAN_DESC'); ?>"><?php echo JText::_('FIELD_LABEL_PLAN'); ?></label>
							<div class="input-group">
								<select name="plan_id" id="<?php echo $APPTAG?>-plan_id" class="form-control">
				...
				<?php else: ?>
					<input type="hidden" name="plan_id" id="<?php echo $APPTAG?>-plan_id" value="0" />
				<?php endif;?>
				<div class="col-sm-4">
					<div class="form-group">
