...
// Crud's permissions
  ...
  // ----------------------------------------------------
    ...
    $cfg['groupId']['admin'][]  = 18; // Instrutor
  // ----------------------------------------------------
...
// crud's main table
  $cfg['mainTable']         = '#__apcefpb_phones';

// crud's main table
  $cfg['operators']         = array('CLARO', 'OI', 'TIM', 'VIVO', 'NEXTEL', '[FIXO]');
...
// crud's relation actions
  ...
  if(!$ajaxRequest) :
    ...
    // show 'Plan Id'
    $_SESSION[$RTAG.'ShowPlanId'] = isset(${$APPTAG.'ShowPlanId'}) ? ${$APPTAG.'ShowPlanId'} : 0;
  endif;
