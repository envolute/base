...
jQuery(function() {
	...
	// métodos controladores do formulário
		...
		var firstField = phone_number; // campo que recebe o focus no carregamento
		...
		window.<?php echo $APPTAG?>_formReset = function() {
			...
			plan_id.val(0).trigger("chosen:updated"); // selects;
			phone_number.val('');
			operator.val('').trigger("chosen:updated"); // selects;
			description.val('');
			...
			jQuery.ajax({
				...
				success: function(data) {
					jQuery.map( data, function( item ) {
						...
						operator.val(item.operator).trigger("chosen:updated"); // selects
