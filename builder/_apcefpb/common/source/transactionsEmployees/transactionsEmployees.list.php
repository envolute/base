...
// LIST
	...
	$query = '
		SELECT SQL_CALC_FOUND_ROWS
			...
			'. $db->quoteName('T1.employee_id') .',
			'. $db->quoteName('T2.name') .' provider,
			'. $db->quoteName('T3.name') .' employee,
			'. $db->quoteName('T1.invoice_id') .',
			'. $db->quoteName('T4.month') .' invoiceMonth,
      '. $db->quoteName('T4.year') .' invoiceYear,
		...
		FROM
			...
			JOIN '. $db->quoteName('#__apcefpb_employees') .' T3
			ON T3.id = T1.employee_id
			LEFT JOIN '. $db->quoteName('#__apcefpb_invoices_employees') .' T4
			ON T4.id = T1.invoice_id
		WHERE
			'.$where.$orderList;
	;
	try {
	...
	<form id="form-list-'.$APPTAG.'" method="post">
		<table class="table table-striped table-hover table-condensed">
			<thead>
				<tr>
					'.$adminView['head']['info'].'
					<th>'.JText::_('FIELD_LABEL_EMPLOYEE').'</th>
					<th>'.JText::_('FIELD_LABEL_PROVIDER').'</th>
					<th>'.JText::_('FIELD_LABEL_DESCRIPTION').'</th>
					<th>'.JText::_('FIELD_LABEL_DATE').'</th>
					<th>'.JText::_('FIELD_LABEL_PRICE').'</th>
					<th>'.JText::_('FIELD_LABEL_INVOICE').'</th>
					'.$adminView['head']['actions'].'
				</tr>
			</thead>
			<tbody>
...
if($num_rows) : // verifica se existe
  ...
  foreach($res as $item) {
		...
		if($hasAdmin) :
			$adminView['list']['info'] = '
				<td class="check-row hidden-print">
					...
					<input type="hidden" name="'.$APPTAG.'_employee_id[]" value="'.$item->employee_id.'" />
		...
		$invoice = $item->invoice_id != 0 ? '<div class="small text-live font-featured">'.baseHelper::getMonthName($item->invoiceMonth).' de '.$item->invoiceYear.'</div>' : '';
    ...
    $rowState = $item->state == 0 ? 'danger' : '';
		$html .= '
			<tr id="'.$APPTAG.'-item-'.$item->id.'" class="'.$rowState.'">
				'.$adminView['list']['info'].'
				<td>'.baseHelper::nameFormat($item->employee).'</td>
  ...
  $html .= '
				<tr class="warning">
					...
					<td colspan="4"><strong>'.baseHelper::priceFormat($total, false, 'R$ ').'</strong></td>
				</tr>
  ';
