...
jQuery(function() {
	...
	var employee_id 	= jQuery('#<?php echo $APPTAG?>-employee_id');
	...
	// métodos controladores do formulário
		...
		window.<?php echo $APPTAG?>_formReset = function() {
			...
			employee_id.val('0').trigger("chosen:updated"); // select
		...
		window.<?php echo $APPTAG?>_editInvoice = function() {
			...
			if(itemID != '' && itemID != 0) invoicesEmployees_loadEditFields(itemID, false, false);
			...
			jQuery.ajax({
				...
				success: function(data) {
					jQuery.map( data, function( item ) {
						...
						employee_id.val(item.employee_id).trigger("chosen:updated"); // select
