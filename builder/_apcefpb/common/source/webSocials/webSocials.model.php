        ...
        // COUNT LIST IDS
          ...
          foreach($list as $item) {
            ...
            if(!empty($_SESSION[$RTAG.'RelTable']) && !empty($_SESSION[$RTAG.'AppNameId'])) :
              ...
              $query = 'DELETE FROM '. $db->quoteName('#__apcefpb_rel_clients_socials') .' WHERE '. $db->quoteName('social_id') .' IN ('.$ids.')';
              $db->setQuery($query);
    					$db->execute();
              // CONTACTS -> remove os registros relacionados aos contatos
              $query = 'DELETE FROM '. $db->quoteName('#__apcefpb_rel_contacts_socials') .' WHERE '. $db->quoteName('social_id') .' IN ('.$ids.')';
              $db->setQuery($query);
    					$db->execute();
            endif;
