...
// LIST
	...
	$query = '
		SELECT SQL_CALC_FOUND_ROWS
			...
			'. $db->quoteName('T1.page') .',
			'. $db->quoteName('T1.username') .',
			'. $db->quoteName('T1.state') .',
			'. $db->quoteName('T4.name') .' client,
			'. $db->quoteName('T6.name') .' contact
		FROM
			'. $db->quoteName($cfg['mainTable']) .' T1
			LEFT JOIN '. $db->quoteName('#__apcefpb_web_socials_names') .' T2
			ON T2.id = T1.socialName_id
			LEFT JOIN '. $db->quoteName('#__apcefpb_rel_clients_socials') .' T3
			ON T3.social_id = T1.id
			LEFT JOIN '. $db->quoteName('#__apcefpb_clients') .' T4
			ON T4.id = T3.client_id
			LEFT JOIN '. $db->quoteName('#__apcefpb_rel_contacts_socials') .' T5
			ON T5.social_id = T1.id
			LEFT JOIN '. $db->quoteName('#__apcefpb_contacts') .' T6
			ON T6.id = T5.contact_id
		WHERE
			'.$where.$orderList;
	;
	try {
		...
		<table class="table table-striped table-hover table-condensed">
			<thead>
				<tr>
					...
					<th>'.$$SETOrder(JText::_('FIELD_LABEL_SOCIAL'), 'T2.name', $APPTAG).'</th>
					<th>'.JText::_('FIELD_LABEL_PAGE').'</th>
					<th>'.JText::_('FIELD_LABEL_USERNAME').'</th>
					'.$adminView['head']['actions'].'
				</tr>
			</thead>
			<tbody>
...
if($num_rows) : // verifica se existe
	...
	foreach($res as $item) {
		...
		$name = !empty($item->name) ? baseHelper::nameFormat($item->name) : '-';
