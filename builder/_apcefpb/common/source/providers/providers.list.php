...
// LIST
	...
	$query = '
		SELECT SQL_CALC_FOUND_ROWS
			...
			'. $db->quoteName('T1.cnpj') .',
			'. $db->quoteName('T1.insc_estadual') .',
			'. $db->quoteName('T1.insc_municipal') .',
			'. $db->quoteName('T1.due_date') .',
			'. $db->quoteName('T1.website') .',
		...
		<table class="table table-striped table-hover table-condensed">
			<thead>
				<tr>
					...
					<th>E-mail</th>
					<th>'.$$SETOrder(JText::_('FIELD_LABEL_DUE_DATE'), 'T1.due_date', $APPTAG).'</th>
	...
	foreach($res as $item) {
		...
		if($hasAdmin) :
			...
			$adminView['list']['actions'] = '
				...
				<td class="text-center hidden-print">
					<a href="#" class="btn btn-xs btn-success" onclick="providersContacts_setParent('.$item->id.')" data-toggle="modal" data-target="#modal-providersContacts" data-backdrop="static" data-keyboard="false"><span class="base-icon-user-add hasTooltip" title="'.JText::_('MSG_INSERT_CONTACT').'"></span></a>
		...
		$info = '';
		if(!empty($item->cnpj) || !empty($item->insc_estadual) || !empty($item->insc_municipal)) :
			$info = !empty($item->cnpj) ? '<strong>CNPJ</strong>: '.$item->cnpj : '';
			$info .= !empty($item->insc_estadual) ? '<br /><strong>Insc. Estadual</strong>: '.$item->insc_estadual : '';
			$info .= !empty($item->insc_municipal) ? '<br /><strong>Insc. Municipal</strong>: '.$item->insc_municipal : '';
		...
		$html .= '
			<tr id="'.$APPTAG.'-item-'.$item->id.'" class="'.$rowState.'">
				...
				<td>
					...
					'.$info.'
					<a href="#" class="base-icon-user text-live hasTooltip" onclick="providersContacts_listReload(false, false, false, true, \'provider_id\', '.$item->id.')" data-toggle="modal" data-target="#modal-list-providersContacts" title="'.JText::_('MSG_VIEW_CONTACTS').'"></a>
