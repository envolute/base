...
if(isset($_SERVER["HTTP_X_REQUESTED_WITH"]) AND strtolower($_SERVER["HTTP_X_REQUESTED_WITH"]) == "xmlhttprequest") :
	...
	if($num_rows) : // verifica se existe
		...
		foreach($res as $item) {
			...
			$group = !empty($item->grp) ? '<br /><abbr class="small text-muted cursor-help hasTooltip" title="'.(!empty($item->occupation) ? $item->occupation : '').'">'.baseHelper::nameFormat($item->grp).'</abbr>' : '';
			...
			$html .= '
				<li class="'.$rowState.'">
					...
					'.baseHelper::nameFormat($item->name).$group.'
				</li>
			';
		}
		$html .= '</ul>';
