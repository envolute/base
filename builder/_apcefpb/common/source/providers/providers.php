	...
	// métodos controladores do formulário
		...
		window.<?php echo $APPTAG?>_formReset = function() {
			...
			email_optional.val('');
				...
				jQuery('#setEmailOptional').addClass('base-icon-plus').removeClass('base-icon-right-big').prop('disabled', false); // enable add button
			cnpj.val('');
			insc_estadual.val('');
			insc_municipal.val('');
			due_date.val('');
		...
		jQuery('#setEmailOptional').click(function() {
			...
			jQuery(this).removeClass('base-icon-plus').addClass('base-icon-right-big').prop('disabled', true);
