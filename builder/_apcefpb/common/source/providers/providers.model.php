        ...
        // COUNT LIST IDS
          ...
          $itemBlock  = ($isUser) ? $userInfoBlock : 0;
            ...
            if($request['userRegistration'] == 1 && $request['user_id'] == 0 && !$isUser) :
              ...
              $eBody = JText::sprintf('MSG_ACTIVATION_EMAIL_BODY', baseHelper::nameFormat($request['name']), $request['email'], $bodyData, $username, $sitename);
          ...
          $uList = $db->loadObjectList();
            ...
            if(!empty($_SESSION[$RTAG.'RelTable']) && !empty($_SESSION[$RTAG.'AppNameId'])) :
              ...
              // CONTATOS -> remove o relacionamento com os contatos
              $query = 'DELETE FROM '. $db->quoteName('#__apcefpb_rel_providers_contacts') .' WHERE '. $db->quoteName('provider_id') .' IN ('.$ids.')';
              $db->setQuery($query);
              $db->execute();
              ...
              foreach ($rIDs as $rID) {
                ...
                $query = 'DELETE FROM '. $db->quoteName('#__apcefpb_rel_providers_addresses') .' WHERE '. $db->quoteName('provider_id') .' = '.$rID;
                $db->setQuery($query);
      					$db->execute();
                ...
                $query = '
                SELECT T2.id FROM
                  '. $db->quoteName('#__apcefpb_rel_providers_phones') .' T1
                ...
                // exclui o relacionamento
                $query = 'DELETE FROM '. $db->quoteName('#__apcefpb_rel_providers_phones') .' WHERE '. $db->quoteName('provider_id') .' = '.$rID;
                $db->setQuery($query);
      					$db->execute();
                ...
                $query = '
                SELECT T2.id FROM
                  '. $db->quoteName('#__apcefpb_rel_providers_socials') .' T1
                  JOIN '. $db->quoteName('#__apcefpb_web_socials') .' T2
                  ON '. $db->quoteName('T1.provider_id') .' = '.$rID
                ;
                $db->setQuery($query);
                $relId = $db->loadColumn();
                $dIDs = implode(',', $relId);
                ...
                // exclui o relacionamento
                $query = 'DELETE FROM '. $db->quoteName('#__apcefpb_rel_providers_socials') .' WHERE '. $db->quoteName('provider_id') .' = '.$rID;
                $db->setQuery($query);
                $db->execute();
                ...
                $query = '
                SELECT T2.id FROM
                  '. $db->quoteName('#__apcefpb_rel_providers_banksAccounts') .' T1
                ...
                // exclui o relacionamento
                $query = 'DELETE FROM '. $db->quoteName('#__apcefpb_rel_providers_banksAccounts') .' WHERE '. $db->quoteName('provider_id') .' = '.$rID;
                $db->setQuery($query);
      					$db->execute();
