...
jQuery(function() {
	...
	// APP FIELDS
	var registry_id 	= jQuery('#<?php echo $APPTAG?>-registry_id');
	...
	// métodos controladores do formulário
		...
		window.<?php echo $APPTAG?>_formReset = function() {
			...
			// o valor é passado quando seleciona a matrícula
			if(registry_id.val() == 0) price.val('');
			note.val('');
			...
			jQuery.ajax({
				...
				success: function(data) {
					jQuery.map( data, function( item ) {
						...
						// App Fields
						registry_id.val(item.registry_id).trigger("chosen:updated"); // select
							...
							if(res.status > 0) { // se alguma ação for realizada
								...
								if(res.parentField != '' && res.parentFieldVal != '') {
									...
									if(res.parentFieldLabel != '' && res.parentFieldLabel != null) {
										...
										jQuery(res.parentField).append('<option value="'+res.parentFieldVal+'" data-target-value="'+res.parentFieldPrice+'">'+res.parentFieldLabel+'</option>'); // add valor à lista
						...
						if(res.status == 4) {
							for(i = 0; i < res.ids.length; i++) {
								...
								if((res.state == 2 && e.hasClass('base-icon-ok')) || res.state == 0) {
									...
									if(res.parentField != '' && res.parentFieldVal != '') {
										// CUSTOM: add value to 'data-target'
										jQuery(res.parentField).append('<option value="'+res.parentFieldVal+'" data-target-value="'+res.parentFieldPrice+'">'+res.parentFieldLabel+'</option>');
										jQuery(res.parentField).val(res.parentFieldVal).trigger("chosen:updated").change(); // atualiza o select
									}
								}
							}
