...
// LIST
	...
	$query = '
		SELECT SQL_CALC_FOUND_ROWS
			'. $db->quoteName('T1.id') .',
			'. $db->quoteName('T3.name') .' student,
			'. $db->quoteName('T4.name') .' sport,
			'. $db->quoteName('T5.month') .',
			'. $db->quoteName('T5.year') .',
			'. $db->quoteName('T1.price') .',
			'. $db->quoteName('T1.note') .',
			'. $db->quoteName('T1.state') .'
		FROM
			'. $db->quoteName($cfg['mainTable']) .' T1
			JOIN '. $db->quoteName('#__apcefpb_students_sports') .' T2
			ON T2.id = T1.registry_id
			JOIN '. $db->quoteName('#__apcefpb_students') .' T3
			ON T3.id = T2.student_id
			JOIN '. $db->quoteName('#__apcefpb_sports') .' T4
			ON T4.id = T2.sport_id
			JOIN '. $db->quoteName('#__apcefpb_students_invoices') .' T5
	...
	<form id="form-list-'.$APPTAG.'" method="post">
		<table class="table table-striped table-hover table-condensed">
			<thead>
				<tr>
					'.$adminView['head']['info'].'
					<th>'.$$SETOrder(JText::_('FIELD_LABEL_STUDENT'), 'T3.name', $APPTAG).'</th>
					<th>'.$$SETOrder(JText::_('FIELD_LABEL_SPORT'), 'T4.name', $APPTAG).'</th>
					<th>'.$$SETOrder(JText::_('FIELD_LABEL_MONTH'), 'T5.month', $APPTAG).'</th>
					<th class="text-center" width="80">'.$$SETOrder(JText::_('FIELD_LABEL_YEAR'), 'T5.year', $APPTAG).'</th>
	...
	foreach($res as $item) {
		...
		$html .= '
			<tr id="'.$APPTAG.'-item-'.$item->id.'" class="'.$rowState.'">
				'.$adminView['list']['info'].'
				<td>'.baseHelper::nameFormat($item->student).'</td>
				<td>'.baseHelper::nameFormat($item->sport).'</td>
				<td>'.baseHelper::getMonthName($item->month).'</td>
	...
	$html .= '
		...
		<tfoot>
			<tr class="warning strong">
				<td colspan="5">&nbsp;</td>
				<td>TOTAL</td>
				<td colspan="3">'.baseHelper::priceFormat($total, false, 'R$ ').'</td>
			</tr>
		</tfoot>
	';
