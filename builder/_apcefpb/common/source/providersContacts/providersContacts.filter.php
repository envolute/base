...
// filter params
	...
	// CLIENT
	$providerID	= $app->input->get('cID', 0, 'int');
	if(isset($providerID) && $providerID != 0) $where .= ' AND '.$db->quoteName('T1.provider_id').' = '.$providerID;
...
// FILTER'S DINAMIC FIELDS
  ...
  // contacts -> select
  $flt_contact = '';
  $query = 'SELECT * FROM '. $db->quoteName('#__apcefpb_contacts') .' ORDER BY name';
  $db->setQuery($query);
