	...
	<form id="form-list-'.$APPTAG.'" method="post">
		<table class="table table-striped table-hover table-condensed">
			<thead>
				<tr>
					'.$adminView['head']['info'].'
					<th>'.$$SETOrder(JText::_('FIELD_LABEL_DEPARTMENT'), 'T2.name', $APPTAG).'</th>
...
if($num_rows) : // verifica se existe
	...
	foreach($res as $item) {
		...
		// endif;
		$occupation = !empty($item->occupation) ? '<br /><small class="text-muted font-featured cursor-help hasTooltip" title="'.JText::_('FIELD_LABEL_OCCUPATION').'">'.baseHelper::nameFormat($item->occupation).'</small>' : '';
		$rowState = $item->state == 0 ? 'danger' : '';
		$html .= '
			<tr id="'.$APPTAG.'-item-'.$item->id.'" class="'.$rowState.'">
				'.$adminView['list']['info'].'
				<td>'.baseHelper::nameFormat($item->grp).$occupation.'</td>
				<td>
