...
jQuery(function() {
	...
	var pis						= jQuery('#<?php echo $APPTAG?>-pis');
	var ctps					= jQuery('#<?php echo $APPTAG?>-ctps');
	var blood_type		= jQuery('#<?php echo $APPTAG?>-blood_type');
	var driver				= jQuery('#<?php echo $APPTAG?>-driver');
	...
	var partner_cpf		= jQuery('#<?php echo $APPTAG?>-partner_cpf');
	var partner_birthday = jQuery('#<?php echo $APPTAG?>-partner_birthday');
	...
	// métodos controladores do formulário
		...
		window.<?php echo $APPTAG?>_formReset = function() {
			...
			pis.val('');
			ctps.val('');
			blood_type.val('').trigger("chosen:updated"); // select
			driver.val('').trigger("chosen:updated"); // select
			occupation.val('');
			start_date.val(''); // DATE -> conversão de data
					...
					for(i = 0; i < files.length; i++) {
						if(files[i]['index'] == a) {
							...
							if(files[i]['mimetype'].indexOf('image') == -1) {
								...
								jQuery('#<?php echo $APPTAG?>-btnPrint-img').append('<a href="#" class="base-icon-print btn btn-lg btn-block btn-warning" onclick="<?php echo $APPTAG?>_printCard()"></a>');
							}
            ...
            if(item.user_id != 0) {
              if(item.block == 0) {
                ...
                jQuery('#<?php echo $APPTAG?>-block-1').addClass('hidden');
						...
						pis.val(item.pis);
						ctps.val(item.ctps);
						blood_type.val(item.blood_type).trigger("chosen:updated"); // select
						driver.val(item.driver).trigger("chosen:updated"); // select
						...
						marital_status.val(item.marital_status).trigger("chosen:updated"); // select
							...
							partner_cpf.val(item.partner_cpf);
							partner_birthday.val(dateFormat(item.partner_birthday));
