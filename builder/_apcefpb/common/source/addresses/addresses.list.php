...
// LIST
	...
	$query = '
		...
		FROM
			...
			LEFT JOIN '. $db->quoteName('#__apcefpb_rel_clients_addresses') .' T2
			ON T2.address_id = T1.id
			LEFT JOIN '. $db->quoteName('#__apcefpb_clients') .' T3
			ON T3.id = T2.client_id
			LEFT JOIN '. $db->quoteName('#__apcefpb_rel_contacts_addresses') .' T4
			ON T4.address_id = T1.id
			LEFT JOIN '. $db->quoteName('#__apcefpb_contacts') .' T5
