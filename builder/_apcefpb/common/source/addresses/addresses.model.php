        ...
        // COUNT LIST IDS
          ...
          foreach($list as $item) {
            ...
            if(!empty($_SESSION[$RTAG.'RelTable']) && !empty($_SESSION[$RTAG.'AppNameId'])) :
              ...
              $query = 'DELETE FROM '. $db->quoteName('#__apcefpb_rel_clients_addresses') .' WHERE '. $db->quoteName('address_id') .' IN ('.$ids.')';
              $db->setQuery($query);
    					$db->execute();
              // DEPENDENTS -> remove os registros relacionados aos dependentes
              $query = 'DELETE FROM '. $db->quoteName('#__apcefpb_rel_dependents_addresses') .' WHERE '. $db->quoteName('address_id') .' IN ('.$ids.')';
              $db->setQuery($query);
    					$db->execute();
              // PROVIDERS -> remove os registros relacionados aos prestadores
              $query = 'DELETE FROM '. $db->quoteName('#__apcefpb_rel_providers_addresses') .' WHERE '. $db->quoteName('address_id') .' IN ('.$ids.')';
              $db->setQuery($query);
    					$db->execute();
              // EMPLOYEES -> remove os registros relacionados aos funcionários
              $query = 'DELETE FROM '. $db->quoteName('#__apcefpb_rel_employees_addresses') .' WHERE '. $db->quoteName('address_id') .' IN ('.$ids.')';
              $db->setQuery($query);
    					$db->execute();
              // STUDENTS -> remove os registros relacionados aos alunos das escolinhas
              $query = 'DELETE FROM '. $db->quoteName('#__apcefpb_rel_students_addresses') .' WHERE '. $db->quoteName('address_id') .' IN ('.$ids.')';
              $db->setQuery($query);
    					$db->execute();
              // CONTACTS -> remove os registros relacionados aos contatos
              $query = 'DELETE FROM '. $db->quoteName('#__apcefpb_rel_contacts_addresses') .' WHERE '. $db->quoteName('address_id') .' IN ('.$ids.')';
              $db->setQuery($query);
    					$db->execute();
            endif;
