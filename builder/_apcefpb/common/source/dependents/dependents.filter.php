...
// filter params
	...
	// OVERTIME -> checkbox
	$fEnd	= $app->input->get('fEnd', 0, 'int');
	if(isset($fEnd) && $fEnd == 1) $where .= ' AND ('.$db->quoteName('T1.end_date').' <= NOW() AND '. $db->quoteName('T2.overtime') .' > 0)';
	// ONLY USERS -> checkbox
	$fUser	= $app->input->get('fUser', 0, 'int');
	if(isset($fUser) && $fUser == 1) $where .= ' AND '.$db->quoteName('T4.username').' <> ""';
	// DOCS -> select
...
// ORDER BY
	...
	$orderDef = 'T1.name ASC'; // não utilizar vírgula no inicio ou fim
		...
		<fieldset class="fieldset-embed filter '.((isset($_GET[$APPTAG.'_filter']) || $cfg['showFilter']) ? '' : 'closed').'">
			...
			<div class="row">
              ...
              <span class="input-group-addon strong">'.JText::_('FIELD_LABEL_DATE').'</span>
				...
				<div class="col-sm-2">
					<div class="form-group">
						<select name="fDocs" id="fDocs" class="form-control input-sm set-filter">
							<option value="2">- '.JText::_('FIELD_LABEL_DOCUMENTS').' -</option>
							<option value="1"'.($fDocs == 1 ? ' selected' : '').'>'.JText::_('TEXT_DOCS_ON').'</option>
							<option value="0"'.($fDocs == 0 ? ' selected' : '').'>'.JText::_('TEXT_DOCS_OFF').'</option>
						</select>
					</div>
				</div>
				<div class="col-sm-2">
				...
				<div class="col-sm-2">
					<div class="form-group">
						<div class="btn-group btn-group-justified" data-toggle="buttons">
							<label class="btn btn-sm btn-warning btn-active-success set-filter hasTooltip" title="Listar apenas dependentes cujo o período de vencimento já foi atingido.">
								<span class="base-icon-cancel btn-icon"></span>
								<input type="checkbox" name="fEnd" value="1"'.($fEnd == 1 ? ' checked' : '').' /> Apenas Vencidos
							</label>
						</div>
					</div>
				</div>
