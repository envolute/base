...
if(isset($_SERVER["HTTP_X_REQUESTED_WITH"]) AND strtolower($_SERVER["HTTP_X_REQUESTED_WITH"]) == "xmlhttprequest") :
	...
	$query .= ' ORDER BY '. $db->quoteName('T1.year') .' DESC, '. $db->quoteName('T1.month') .' DESC';
	try {
			...
			$html .= '
				<li class="'.$rowState.'">
					...
					<div class="small text-muted font-featured">
						'.baseHelper::nameFormat($usergroup).' - '.baseHelper::getMonthName($item->month).' de '.$item->year.' - Venc. '.baseHelper::dateFormat($item->due_date).'
					</div>
				</li>
			';
		}
