...
// filter params
	...
	// GROUP -> select
	$fGroup	= $app->input->get('fGroup', 9, 'int');
	if($fGroup != 9) $where .= ' AND '.$db->quoteName('T1.group_id').' = '.$fGroup;
	// MONTH -> select
				...
				<div class="col-sm-2">
					<div class="form-group">
						<select name="fGroup" id="fGroup" class="form-control input-sm set-filter">
							<option value="9">- '.JText::_('FIELD_LABEL_GROUP').' -</option>
							<option value="0"'.($fGroup == '0' ? ' selected = "selected"' : '').'>Associados Caixa</option>
							<option value="1"'.($fGroup == '1' ? ' selected = "selected"' : '').'>Contribuintes</option>
						</select>
					</div>
				</div>
				<div class="col-sm-2">
