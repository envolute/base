		...
		<table class="table table-striped table-hover table-condensed">
			<thead>
				<tr>
					...
					<th>'.$$SETOrder(JText::_('FIELD_LABEL_YEAR'), 'T1.year', $APPTAG).'</th>
...
if($num_rows) : // verifica se existe
	...
	foreach($res as $item) {
		...
		$usergroup = $item->group_id == 0 ? 'Associados Caixa' : 'Contribuintes';
		...
		$html .= '
			<tr id="'.$APPTAG.'-item-'.$item->id.'" class="'.$rowState.'">
				'.$adminView['list']['info'].'
				<td>'.baseHelper::nameFormat($usergroup).$note.'</td>
