...
class PlgContentPagebreak extends JPlugin
{
	...
	public function onContentPrepare($context, &$row, &$params, $page = 0)
	{
		...
		if ($n > 1)
		{
			...
			if ($style === 'pages')
			{
				...
				// Page counter.
				$row->text .= '<div class="pagenavcounter mb-4">';
	...
	protected function _createToc(&$row, &$matches, &$page)
	{
		...
		// TOC header.
		$row->toc = '<div class="article-index">';
		...
		if ($this->params->get('article_index') == 1)
		{
			...
			$row->toc .= '<h5>' . $headingtext . '</h5>';
		}

		// TOC first Page link.
		$class = ($limitstart === 0 && $showall === 0) ? 'toclink active text-live' : 'toclink';
		$row->toc .= '<ul class="list-unstyled pb-4 mb-4 b-bottom">
		<li class="' . $class . '">
			<a href="'
		...
		foreach ($matches as $bot)
		{
			...
			$class     = ($limitstart === $i - 1) ? 'toclink active text-live' : 'toclink';
	...
	protected function _createNavigation(&$row, $page, $n)
	{
		...
		if ($page < $n - 1)
		{
			...
			// Next >>
			$next = '
				<a href="' . $link_next . '" class="btn btn-sm btn-primary" role="button">
					' . JText::_('JNEXT') . ' <span class="base-icon-right-open"></span>
				</a>
			';
		}
		else
		{
			$next = '
				<span class="btn btn-sm btn-primary disabled" role="button" aria-disabled="true">
					' . JText::_('JNEXT') . ' <span class="base-icon-right-open"></span>
				</span>
			';
		}
		...
		if ($page > 0)
		{
			...
			// << Prev
			$prev = '
				<a href="' . $link_prev . '" class="btn btn-sm btn-primary" role="button">
					<span class="base-icon-left-open"></span> ' . JText::_('JPREVIOUS') . '
				</a>
			';
		}
		else
		...
		$row->text .= '<div class="text-center py-5">' . $prev . $next . '</div>';
	}
}
