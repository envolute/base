...
jQuery(function() {
	...
	var parentFieldId		= student_id; // 'null', caso não exista...
	...
	// métodos controladores do formulário
		...
		window.<?php echo $APPTAG?>_formReset = function() {
			...
			sport_id.selectUpdate('0'); // select
			registry_date.val('');
			checkOption(coupon_free, 0); // radio
			price.val(''); // select
			note.val('');
			...
			jQuery.ajax({
				...
				success: function(data) {
					...
					jQuery.map( data, function( item ) {
						...
						// App Fields
						student_id.selectUpdate(item.student_id); // selects
						sport_id.selectUpdate(item.sport_id); // selects
						registry_date.val(dateFormat(item.registry_date));
						checkOption(coupon_free, item.coupon_free); // checkbox
