...
if($num_rows) : // verifica se existe
	...
	foreach($res as $item) {
					...
					<table class="table table-striped table-hover table-sm">
						<thead>
							<tr>
								...
								<th width="50" class="d-none d-lg-table-cell text-center">'.JText::_('FIELD_LABEL_START_DATE').'</th>
								<th width="50" class="d-none d-lg-table-cell text-center">'.JText::_('FIELD_LABEL_END_DATE').'</th>
								'.$adminView['head']['actions'].'
							</tr>
						</thead>
						<tbody>
		...
		if($cfg['hasUpload']) :
			...
			if(!empty($img)) {
				$img = '<img src="'.baseHelper::thumbnail('images/apps/'.$APPPATH.'/'.$img['filename'], 32, 32).'" class="d-none d-md-inline img-fluid rounded float-left mr-2" />';
			}
		...
		$url = !empty($item->url) ? '<div class="small text-muted text-truncate">'.$item->url.'</div>' : '';
		...
		$html .= '
			<tr id="'.$APPTAG.'-item-'.$item->id.'" class="'.$rowState.'">
				'.$adminView['list']['info'].'
				<td>'.baseHelper::nameFormat($item->group_name).'</td>
				<td>'.$img.baseHelper::nameFormat($item->name).$url.'</td>
				<td class="d-none d-lg-table-cell">'.baseHelper::dateFormat($item->start_date, 'd/m/Y', true, '-').'</td>
				<td class="d-none d-lg-table-cell">'.baseHelper::dateFormat($item->end_date, 'd/m/Y', true, '-').'</td>
				'.$adminView['list']['actions'].'
			</tr>
		';
	}
