		...
		foreach($res as $item) {
			...
			$html .= '
				<li class="'.$rowState.'">
					...
					'.$item->phone_number.'
					<div class="text-sm text-muted mt-1"><span class="badge badge-primary">'.$item->provider.'</span> '.baseHelper::nameFormat($item->plan).' <span class="float-right">R$'.baseHelper::priceFormat($item->price).'</span></div>
				</li>
			';
		}
		$html .= '</ul>';
