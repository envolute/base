...
if(isset($_SERVER["HTTP_X_REQUESTED_WITH"]) AND strtolower($_SERVER["HTTP_X_REQUESTED_WITH"]) == "xmlhttprequest") :
	...
	if($num_rows) : // verifica se existe
		...
		foreach($res as $item) {
			...
			$btnEdit = $canEdit ? '<a href="#" class="base-icon-pencil text-live ml-1 hasTooltip" title="'.JText::_('TEXT_EDIT').'" onclick="'.$APPTAG.'_loadEditFields('.$item->id.', false, false)"></a> ' : '';
			$btnDelete = $canDelete ? '<a href="#" class="base-icon-trash text-danger ml-1 hasTooltip" title="'.JText::_('TEXT_DELETE').'" onclick="'.$APPTAG.'_del('.$item->id.', false)"></a>' : '';
