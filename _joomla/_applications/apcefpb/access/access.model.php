...
if(isset($_SERVER["HTTP_X_REQUESTED_WITH"]) AND strtolower($_SERVER["HTTP_X_REQUESTED_WITH"]) == "xmlhttprequest") :
	...
	if($task != null) :
		...
		$request['client_desc']			= $input->get('client_desc', '', 'string');
		$request['presence']			= $input->get('presence', 0, 'int');
		$request['cExam']				= $input->get('cExam', 0, 'int');
		$request['cForbidden']			= $input->get('cForbidden', 0, 'int');
		$request['cReason']				= $input->get('cReason', '', 'string');
		$dependent						= $input->get('dependent', array(), 'array');
		$request['dependent']			= implode(';', $dependent);
		$exam							= $input->get('exam', array(), 'array');
		$exam							= str_replace(';', '.', $exam); // formata
		$request['exam']				= implode(';', $exam);
		$forbidden						= $input->get('forbidden', array(), 'array');
		$forbidden						= str_replace(';', '.', $forbidden); // formata
		$request['forbidden']			= implode(';', $forbidden);
		$reason							= $input->get('reason', array(), 'array');
		$reason							= str_replace(';', '.', $reason); // formata
		$request['reason']				= implode(';', $reason);
		$guestName						= $input->get('guestName', array(), 'array');
		$guestName						= str_replace(';', '.', $guestName); // formata
		$request['guestName']			= implode(';', $guestName);
		$guestAge						= $input->get('guestAge', array(), 'array');
		$guestAge						= str_replace(';', '.', $guestAge); // formata
		$request['guestAge']			= implode(';', $guestAge);
		$guestNote						= $input->get('guestNote', array(), 'array');
		$guestNote						= str_replace(';', '.', $guestNote); // formata
		$request['guestNote']			= implode(';', $guestNote);
		$guestTax						= $input->get('guestTax', array(), 'array');
		$guestTax						= str_replace(';', '.', $guestTax); // formata
		$request['guestTax']			= implode(';', $guestTax);
		$request['tax_price']			= $input->get('tax_price');
		$request['accessDate']			= $input->get('accessDate', '', 'string');

		// SAVE CONDITION
		// Condição para inserção e atualização dos registros
		$save_condition = (!empty($request['client_id']) || !empty($request['guestName']));
		...
		if($id || (!empty($ids) && $ids != 0)) :  //UPDATE OR DELETE
			...
			if($exist) : // verifica se existe
				...
				if($task == 'get') :
					...
					$data[] = array(
						...
						'client_desc'		=> $item->client_desc,
						'presence'			=> $item->presence,
						'cExam'				=> $item->cExam,
						'cForbidden'		=> $item->cForbidden,
						...
						'guestAge'			=> $item->guestAge,
						'guestNote'			=> $item->guestNote,
						'guestTax'			=> $item->guestTax,
				...
				if($save_condition) :
					...
					$query  = '
						INSERT INTO '. $db->quoteName($cfg['mainTable']) .'('.
							...
							$db->quoteName('client_desc') .','.
							$db->quoteName('presence') .','.
							$db->quoteName('cExam') .','.
							$db->quoteName('cForbidden') .','.
							$db->quoteName('cReason') .','.
							$db->quoteName('dependent') .','.
							$db->quoteName('exam') .','.
							$db->quoteName('forbidden') .','.
							$db->quoteName('reason') .','.
							$db->quoteName('guestName') .','.
							$db->quoteName('guestAge') .','.
							$db->quoteName('guestNote') .','.
							$db->quoteName('guestTax') .','.
							$db->quoteName('tax_price') .','.
							$db->quoteName('accessDate') .','.
							$db->quoteName('state') .','.
							$db->quoteName('created_by')
						.') VALUES ('.
							$request['client_id'] .','.
							$db->quote($request['client_desc']) .','.
							$request['presence'] .','.
							$request['cExam'] .','.
							$request['cForbidden'] .','.
							$db->quote($request['cReason']) .','.
							$db->quote($request['dependent']) .','.
							$db->quote($request['exam']) .','.
							$db->quote($request['forbidden']) .','.
							$db->quote($request['reason']) .','.
							$db->quote($request['guestName']) .','.
							$db->quote($request['guestAge']) .','.
							$db->quote($request['guestNote']) .','.
							$db->quote($request['guestTax']) .','.
							$db->quote($request['tax_price']) .','.
							$db->quote($request['accessDate']) .','.
							$request['state'] .','.
							$user->id
						.')
					';
			...
			elseif($task == 'cList' && $client != 0) :
				...
				if(!empty($dt)) :
					...
					$d = explode(' ', $dt);
					// client verify in current date
					$query = '
						SELECT count(*)
						FROM '. $db->quoteName($cfg['mainTable']) .'
						WHERE
							'. $db->quoteName('client_id') .' = '.$client.'
							AND '. $db->quoteName('accessDate') .' BETWEEN '.$db->quote($d[0].' 00:00:00').' AND '.$db->quote($d[0].' 23:59:59')
					;
					$db->setQuery($query);
					$exist = $db->loadResult();
				endif;
