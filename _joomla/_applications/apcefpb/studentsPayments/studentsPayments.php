...
jQuery(function() {
	...
	var parentFieldId		= registry_id; // 'null', caso não exista...
	var parentFieldGroup	= elementExist(parentFieldId) ? parentFieldId.closest('[class*="form-group"]') : null;
	...
	// métodos controladores do formulário
		...
		window.<?php echo $APPTAG?>_editInvoice = function() {
			var itemID = invoice_id.val();
			if(itemID != '' && itemID != 0) studentsInvoices_loadEditFields(itemID, false, false);
			...
			jQuery.ajax({
				...
				success: function(data) {
					...
					jQuery.map( data, function( item ) {
						...
						// App Fields
						registry_id.selectUpdate(item.registry_id);
						invoice_id.selectUpdate(item.invoice_id);
