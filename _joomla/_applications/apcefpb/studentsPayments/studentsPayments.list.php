...
// LIST
	...
	$query = '
		SELECT SQL_CALC_FOUND_ROWS
			...
			'. $db->quoteName('T5.due_date') .'
		FROM
			'. $db->quoteName($cfg['mainTable']) .' T1
			JOIN '. $db->quoteName('#__'.$cfg['project'].'_students_sports') .' T2
			ON T2.id = T1.registry_id
			JOIN '. $db->quoteName('#__'.$cfg['project'].'_students') .' T3
			ON T3.id = T2.student_id
			JOIN '. $db->quoteName('#__'.$cfg['project'].'_sports') .' T4
			ON T4.id = T2.sport_id
			JOIN '. $db->quoteName('#__'.$cfg['project'].'_students_invoices') .' T5
	...
	foreach($res as $item) {
				...
				<form id="form-list-'.$APPTAG.'" method="post" class="pt-3">
					<table class="table table-striped table-hover table-sm">
						<thead>
							<tr>
								'.$adminView['head']['info'].'
								<th>'.JText::_('FIELD_LABEL_STUDENT').'</th>
								<th>'.JText::_('FIELD_LABEL_SPORT').'</th>
								<th width="120">'.JText::_('FIELD_LABEL_DUE_DATE').'</th>
								<th width="120">'.JText::_('FIELD_LABEL_PRICE').'</th>
		...
		$html .= '
			<tr id="'.$APPTAG.'-item-'.$item->id.'" class="'.$rowState.'">
				...
				<td>'.baseHelper::dateFormat($item->due_date).'</td>
