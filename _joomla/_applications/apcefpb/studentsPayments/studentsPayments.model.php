...
if(isset($_SERVER["HTTP_X_REQUESTED_WITH"]) AND strtolower($_SERVER["HTTP_X_REQUESTED_WITH"]) == "xmlhttprequest") :
	...
	if($task != null) :
		...
		// app
		$request['registry_id']			= $input->get('registry_id', 0, 'int');
		...
		$save_condition = ($request['registry_id'] > 0 && $request['invoice_id'] > 0);
			...
			if($exist) : // verifica se existe
				...
				elseif($task == 'save' && $id) :
					...
					if($save_condition) {
						...
						$query .=
							$db->quoteName('registry_id')	.'='. $request['registry_id'] .','.
