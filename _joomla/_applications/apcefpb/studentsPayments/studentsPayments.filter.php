...
// filter params
	...
	if(!empty($dateMin) || !empty($dateMax)) $where .= ' AND '.$db->quoteName('T5.due_date').' BETWEEN '.$db->quote($dtmin).' AND '.$db->quote($dtmax);
...
// FILTER'S DINAMIC FIELDS
	...
	foreach ($invoices as $obj) {
		$flt_invoice .= '<option value="'.$obj->id.'"'.($obj->id == $fInv ? ' selected = "selected"' : '').'>'.baseHelper::dateFormat($obj->due_date).' - '.baseHelper::nameFormat($obj->name).'</option>';
	}
