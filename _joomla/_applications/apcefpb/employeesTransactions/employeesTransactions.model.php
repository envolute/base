...
if(isset($_SERVER["HTTP_X_REQUESTED_WITH"]) AND strtolower($_SERVER["HTTP_X_REQUESTED_WITH"]) == "xmlhttprequest") :
	...
	if($task != null) :
		...
		// app
		$request['employee_id']			= $input->get('employee_id', 0, 'int');
		...
		$save_condition = ($request['employee_id'] > 0 && $request['provider_id'] > 0);
			...
			if($exist) : // verifica se existe
				...
				elseif($task == 'save' && $id) :
					...
					if($save_condition) {
						...
						$query .=
							$db->quoteName('employee_id')	.'='. $request['employee_id'] .','.
				...
				if($save_condition) :
					...
					try {
						...
						if(!empty($_SESSION[$RTAG.'FieldUpdated']) && !empty($_SESSION[$RTAG.'TableField'])) :
							...
							// CUSTOM
							// $query = 'SELECT '. (preg_match('/^[a-zA-Z_][a-zA-Z0-9_]*$/', $_SESSION[$RTAG.'TableField']) ? $db->quoteName($_SESSION[$RTAG.'TableField']) : $_SESSION[$RTAG.'TableField']) .' FROM '. $db->quoteName($cfg['mainTable']).' WHERE '. $db->quoteName('id') .' = '.$id.' AND state = 1';
							$query = 'SELECT CONCAT(DATE_FORMAT('.$db->quoteName('price').', "%d/%m/%Y"), " - ", IF('.$db->quoteName('custom_desc').' <> "", '.$db->quoteName('custom_desc').', '.$db->quoteName('description').')) FROM '. $db->quoteName($cfg['mainTable']).' WHERE id='.$id.' AND state = 1';
							$db->setQuery($query);
							$elemLabel = $db->loadResult();
						endif;
