...
if(isset($_SERVER["HTTP_X_REQUESTED_WITH"]) AND strtolower($_SERVER["HTTP_X_REQUESTED_WITH"]) == "xmlhttprequest") :
	...
	$query	.= ' ORDER BY '. $db->quoteName('T1.created_date') .' DESC';
	try {
		$db->setQuery($query);
		$db->execute();
		...
		foreach($res as $item) {
			...
			$html .= '
				<li class="'.$rowState.'">
					...
					'.baseHelper::dateFormat($item->created_date, 'd/m/Y').' - '.$item->sport.' ('.baseHelper::priceFormat($item->price).')
				</li>
			';
		}
		$html .= '</ul>';
