...
// LIST
	...
	$query = '
		...
		FROM
			...
			JOIN '. $db->quoteName('#__'.$cfg['project'].'_employees') .' T3
			ON T3.id = T1.employee_id
			LEFT JOIN '. $db->quoteName('#__'.$cfg['project'].'_employees_invoices') .' T4
	...
	$html .= '
		...
		<tfoot>
			<tr class="warning strong">
				<td colspan="5">&nbsp;</td>
				<td>TOTAL</td>
				<td colspan="5">'.baseHelper::priceFormat($total, false, 'R$ ').'</td>
			</tr>
		</tfoot>
	';
