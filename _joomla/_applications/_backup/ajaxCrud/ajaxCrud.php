...
jQuery(function() {
	...
	var check					= jQuery('#<?php echo $APPTAG?>-check');
	...
	// métodos controladores do formulário
		...
		window.<?php echo $APPTAG?>_formReset = function() {
			...
			name.val('');
			date.val('');
			type.val('0').trigger("chosen:updated"); // select
			price.val('');
			description.val('');
			check.prop('checked', false); // checkbox -> default: not checked
		...
		<?php if($cfg['hasUpload']) :?>
			...
			window.<?php echo $APPTAG?>_setNewFile = function() {
				...
				fileField += '		<button type="button" class="base-icon-search btn btn-default file-action"> <?php echo JText::_('TEXT_FILE_SELECT'); ?></button>';
				fileField += '	</span>';
				fileField += '	<input type="file" name="<?php echo $cfg['fileField']?>['+<?php echo $APPTAG?>IndexFile+']" id="<?php echo $APPTAG?>-<?php echo $cfg['fileField']?>'+<?php echo $APPTAG?>IndexFile+'" class="form-control" hidden />';
				fileField += '</div>';
			...
			jQuery.ajax({
				...
				success: function(data) {
					jQuery.map( data, function( item ) {
						...
						type.val(item.type).trigger("chosen:updated"); // selects
						price.val(item.price);
						description.val(item.description);
						check.prop('checked', (item.check == 0 ? false : true));
