...
// filter params
	...
	// MAIN -> select
	$fMain	= $app->input->get('fMain', 2, 'int');
	if($fMain != 2) $where .= ' AND '.$db->quoteName('T1.main').' = '.$fMain;
	// WHATSAPP -> select
	$fWapp	= $app->input->get('fWapp', 2, 'int');
	if($fWapp != 2) $where .= ' AND '.$db->quoteName('T1.whatsapp').' = '.$fWapp;
	...
	$searchFields = array(
		...
		'T1.operator'			=> 'FIELD_LABEL_OPERATOR',
		'T1.description'		=> ''
	);
	$i = 0;
	...
	<form id="filter-'.$APPTAG.'" class="hidden-print collapse '.((isset($_GET[$APPTAG.'_filter']) || $cfg['openFilter']) ? 'show' : '').'" method="get">
		<fieldset class="fieldset-embed fieldset-sm pt-3 pb-0">
			...
			<div class="row">
				...
				<div class="col-sm-4 col-md-2">
					<div class="form-group">
						<label class="label-xs text-muted">Whatsapp</label>
						<select name="fWapp" id="fWapp" class="form-control form-control-sm set-filter">
							<option value="2">- '.JText::_('TEXT_ALL').' -</option>
							<option value="1"'.($fWapp == 1 ? ' selected' : '').'>'.JText::_('TEXT_YES').'</option>
							<option value="0"'.($fWapp == 0 ? ' selected' : '').'>'.JText::_('TEXT_NO').'</option>
						</select>
					</div>
				</div>
				<div class="col-sm-4 col-md-2">
					<div class="form-group">
						<label class="label-xs text-muted">'.JText::_('TEXT_STATE').'</label>
