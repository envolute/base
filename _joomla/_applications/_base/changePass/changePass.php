...
jQuery(function() {
	...
	// métodos controladores do formulário
		...
		window.<?php echo $APPNAME?>_formReset = function() {
			...
			mainForm.find('fieldset, #btn-<?php echo $APPNAME?>-save, #btn-<?php echo $APPNAME?>-cancel').prop('hidden', false);
			mainForm.find('#btn-<?php echo $APPNAME?>-close').prop('hidden', true);
	...
	// métodos controladores das ações referente ao banco de dados e envio de arquivos
		...
		window.<?php echo $APPNAME?>_save = function() {
			...
			if(mainForm.valid()) {
				...
				jQuery.ajax({
					url: "<?php echo $_CORE.'apps/changePass/changePass.model.php?'.$langDef?>",
					dataType: 'json',
					type: 'POST',
					method: "POST",
					data:  dados,
					...
					success: function(data){
						...
						jQuery.map( data, function( res ) {
							if(res.status == 1) { // changed
								// esconde o form
								mainForm.find('fieldset, #btn-<?php echo $APPNAME?>-save, #btn-<?php echo $APPNAME?>-cancel').prop('hidden', true);
								mainForm.find('#btn-<?php echo $APPNAME?>-close').prop('hidden', false);
								// MENSAGENS: mostra a mensagem de sucesso/erro
								mainForm.find('.set-success').prop('hidden', false);
							} else {
								// caso ocorra um erro na ação, mostra a mensagem de erro
								mainForm.find('.set-error').prop('hidden', false).text((res.status == 2 ? '<?php echo JText::_('MSG_PASS_NOT_EQUAL'); ?>' : res.msg));
	...
	<div class="modal-dialog modal-sm" role="document" style="max-width:220px">
      ...
      <form name="form-<?php echo $APPNAME?>" id="form-<?php echo $APPNAME?>" method="post" enctype="multipart/form-data">
      	...
      	<div class="modal-body">
					<div class="base-icon-ok-circled2 set-success alert alert-success m-0" hidden><?php echo JText::_('MSG_SAVED');?></div>
					<div class="base-icon-cancel-circled set-error alert alert-danger mb-2" hidden><?php echo JText::_('MSG_ERROR');?></div>
      		<fieldset>
      			<div class="row">
      				<div class="col-sm-12">
      					<div class="form-group">
	      						...
	      						<input type="password" name="newpass" id="<?php echo $APPNAME?>-newpass" class="form-control input-required" placeholder="<?php echo JText::_('FIELD_LABEL_NEWPASS'); ?>" />
										<span class="input-group-addon">
											<span class="base-icon-help-circled cursor-help hasTooltip" data-animation="false" title="<?php echo JText::_('FIELD_LABEL_NEWPASS'); ?>"></span>
										</span>
									</div>
      					</div>
      				</div>
      				<div class="col-sm-12">
      					<div class="form-group m-0">
									<div class="input-group">
										...
										<span class="input-group-addon">
											<span class="base-icon-help-circled cursor-help hasTooltip" data-animation="false" title="<?php echo JText::_('FIELD_LABEL_REPASS'); ?>"></span>
										</span>
									</div>
      					</div>
      				</div>
      	...
      	<div class="modal-footer">
      		...
      		<div class="col p-0 text-right">
      			...
      			<button name="btn-<?php echo $APPNAME?>-close" id="btn-<?php echo $APPNAME?>-close" class="base-icon-cancel btn btn-danger btn-sm" hidden data-dismiss="modal"> <?php echo JText::_('TEXT_CLOSE'); ?></button>
      		</div>
      	</div>
