...
// filter params
	...
	// WHATSAPP -> checkbox
	$fwapp	= $app->input->get('fwapp', 0, 'int');
	if(isset($fwapp) && $fwapp == 1) $where .= ' AND '.$db->quoteName('T1.whatsapp').' = 1';

	// search
	$search	= $app->input->get('fSearch', '', 'string');
	if(!empty($search)) :
		$where .= ' AND (';
		$where .= 'LOWER('.$db->quoteName('T1.phone_number').') LIKE LOWER("%'.$search.'%")';
		$where .= ' OR LOWER('.$db->quoteName('T1.operator').') LIKE LOWER("%'.$search.'%")';
		...
		<fieldset class="fieldset-embed fieldset-sm pt-3 pb-0">
			...
			<div class="row">
				<div class="col-sm-2">
					<div class="form-group">
						<div class="btn-group w-full" data-toggle="buttons">
							<label class="col btn btn-sm btn-warning btn-active-success set-filter">
				...
				<div class="col-sm-2">
					<div class="form-group">
						<div class="btn-group w-full" data-toggle="buttons">
							<label class="col btn btn-sm btn-default btn-active-success set-filter hasTooltip" title="'.JText::_('TEXT_ONLY_HAS_WHATSAPP').'">
								<span class="base-icon-whatsapp btn-icon"></span>
								<input type="checkbox" name="fwapp" value="1"'.($fwapp == 1 ? ' checked' : '').' />
							</label>
						</div>
					</div>
				</div>
