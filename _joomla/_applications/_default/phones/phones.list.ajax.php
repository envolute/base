...
if(isset($_SERVER["HTTP_X_REQUESTED_WITH"]) AND strtolower($_SERVER["HTTP_X_REQUESTED_WITH"]) == "xmlhttprequest") :
	...
	$query .= ' ORDER BY '. $db->quoteName('main') .' DESC, '. $db->quoteName('phone_number') .' ASC';
	try {
	...
	if($num_rows) : // verifica se existe
		...
		foreach($res as $item) {
			...
			$info = !empty($item->description) ? '<br /><small class="text-muted font-featured">'.baseHelper::nameFormat($item->description).'</small>' : '';
			...
			$html .= '
				<li class="'.$rowState.'">
					...
					'.$item->phone_number.$main.$wapp.$operator.$info.'
				</li>
			';
		}
		$html .= '</ul>';
