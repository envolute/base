        ...
        // COUNT LIST IDS
          ...
          $uList = $db->loadObjectList();
            ...
            if(!empty($_SESSION[$RTAG.'RelTable']) && !empty($_SESSION[$RTAG.'AppNameId'])) :
              ...
              $query = 'DELETE FROM '. $db->quoteName('#__envolute_rel_clients_contacts') .' WHERE '. $db->quoteName('contact_id') .' IN ('.$ids.')';
              $db->setQuery($query);
    					$db->execute();
              // PROJECTS -> remove os registros relacionados aos projetos
              $query = 'DELETE FROM '. $db->quoteName('#__envolute_rel_projects_contacts') .' WHERE '. $db->quoteName('contact_id') .' IN ('.$ids.')';
              $db->setQuery($query);
    					$db->execute();
              // TASKS -> remove os registros relacionados às tarefas
              $query = 'DELETE FROM '. $db->quoteName('#__envolute_rel_tasks_contacts') .' WHERE '. $db->quoteName('contact_id') .' IN ('.$ids.')';
              $db->setQuery($query);
    					$db->execute();
              ...
              foreach ($rIDs as $rID) {
                ...
                $query = '
                SELECT T2.id FROM
                  '. $db->quoteName('#__envolute_rel_contacts_addresses') .' T1
                  JOIN '. $db->quoteName('#__envolute_addresses') .' T2
                ...
                if(!empty($dIDs)) :
                  // exclui os endereços
                  $query = 'DELETE FROM '. $db->quoteName('#__envolute_addresses') .' WHERE '. $db->quoteName('id') .' IN ('.$dIDs.')';
                  $db->setQuery($query);
        					$db->execute();
                endif;
                // exclui o relacionamento
                $query = 'DELETE FROM '. $db->quoteName('#__envolute_rel_contacts_addresses') .' WHERE '. $db->quoteName('contact_id') .' = '.$rID;
                $db->setQuery($query);
      					$db->execute();
                ...
                $query = '
                SELECT T2.id FROM
                  '. $db->quoteName('#__envolute_rel_contacts_phones') .' T1
                  JOIN '. $db->quoteName('#__envolute_phones') .' T2
                ...
                if(!empty($dIDs)) :
                  // exclui os endereços
                  $query = 'DELETE FROM '. $db->quoteName('#__envolute_phones') .' WHERE '. $db->quoteName('id') .' IN ('.$dIDs.')';
                  $db->setQuery($query);
        					$db->execute();
                endif;
                // exclui o relacionamento
                $query = 'DELETE FROM '. $db->quoteName('#__envolute_rel_contacts_phones') .' WHERE '. $db->quoteName('contact_id') .' = '.$rID;
                $db->setQuery($query);
      					$db->execute();
                ...
                $query = '
                SELECT T2.id FROM
                  '. $db->quoteName('#__envolute_rel_contacts_socials') .' T1
                  JOIN '. $db->quoteName('#__envolute_web_socials') .' T2
                  ON '. $db->quoteName('T1.contact_id') .' = '.$rID
                ;
                $db->setQuery($query);
                $relId = $db->loadColumn();
                $dIDs = implode(',', $relId);
                if(!empty($dIDs)) :
                  // exclui os endereços
                  $query = 'DELETE FROM '. $db->quoteName('#__envolute_web_socials') .' WHERE '. $db->quoteName('id') .' IN ('.$dIDs.')';
                  $db->setQuery($query);
                  $db->execute();
                endif;
                // exclui o relacionamento
                $query = 'DELETE FROM '. $db->quoteName('#__envolute_rel_contacts_socials') .' WHERE '. $db->quoteName('contact_id') .' = '.$rID;
                $db->setQuery($query);
                $db->execute();
                ...
                $query = '
                SELECT T2.id FROM
                  '. $db->quoteName('#__envolute_rel_contacts_banksAccounts') .' T1
                  JOIN '. $db->quoteName('#__envolute_banks_accounts') .' T2
                ...
                if(!empty($dIDs)) :
                  // exclui os endereços
                  $query = 'DELETE FROM '. $db->quoteName('#__envolute_banks_accounts') .' WHERE '. $db->quoteName('id') .' IN ('.$dIDs.')';
                  $db->setQuery($query);
        					$db->execute();
                endif;
                // exclui o relacionamento
                $query = 'DELETE FROM '. $db->quoteName('#__envolute_rel_contacts_banksAccounts') .' WHERE '. $db->quoteName('contact_id') .' = '.$rID;
                $db->setQuery($query);
      					$db->execute();
