...
jQuery(function() {
	...
	var	assing_id			= jQuery('#<?php echo $APPTAG?>-assing_id');
	...
	var cnpj_contact_name		= jQuery('#<?php echo $APPTAG?>-cnpj_contact_name');
	var cnpj_contact_cpf		= jQuery('#<?php echo $APPTAG?>-cnpj_contact_cpf');
	...
	var email_optional= jQuery('#<?php echo $APPTAG?>-email_optional');
	...
	var comment				= jQuery('#<?php echo $APPTAG?>-comment');
	var userRegistration = jQuery('#<?php echo $APPTAG?>-userRegistration');
	...
	// métodos controladores do formulário
		...
		window.<?php echo $APPTAG?>_formReset = function() {
			...
			user_id.val('0').trigger("chosen:updated"); // selects
				...
				jQuery('#registrationFormGroup').removeClass('hide');
				// hide user registrated message
				jQuery('#contactIsUser').addClass('hide');
			group_id.val('1').trigger("chosen:updated"); // select
			selectRadio(type, 0); // radio
			// set CNPJ or CPF
			<?php echo $APPTAG?>_setType(0);
			doc_number.val('');
			cnpj_contact_name.val('');
			cnpj_contact_cpf.val('');
			...
			cmail.val('');
			email_optional.val('');
				// 'email_optional' hidden by default
				jQuery('#<?php echo $APPTAG?>-group-email_optional').addClass('hide');
				jQuery('#<?php echo $APPTAG?>-setEmailOptional').addClass('base-icon-plus').removeClass('base-icon-down-big').prop('disabled', false); // enable add button
			birthday.val('');
			occupation.val('');
			description.val('');
			selectRadio(portfolio, 0); // radio
			comment.val('');
		...
		window.<?php echo $APPTAG?>_resetRegistrationForm = function(){
			// reset button
			userRegistration.prop('checked', false);
			userRegistration.parents('label.btn.active').removeClass('active');
			// form closed by default
			jQuery('#registrationFieldsGroup').addClass('closed').hide(); // close fieldset
			// reset fields
		...
		window.<?php echo $APPTAG?>_setType = function(e){
			if(e == 1) {
				setCNPJ(doc_number);
				doc_number.prev('label').text('CNPJ');
				jQuery('.<?php echo $APPTAG?>-group-cnpj').removeClass('hide');
				jQuery('.<?php echo $APPTAG?>-group-cpf').val('').addClass('hide');
				// reset fields
				jQuery('.<?php echo $APPTAG?>-group-cpf').find('input[type="text"]').val('');
				selectRadio(gender, 0); // radio
			} else {
				setCPF(doc_number);
				doc_number.prev('label').text('CPF');
				jQuery('.<?php echo $APPTAG?>-group-cnpj').val('').addClass('hide');
				jQuery('.<?php echo $APPTAG?>-group-cpf').removeClass('hide');
				// reset fields
				jQuery('.<?php echo $APPTAG?>-group-cnpj').find('input[type="text"]').val('');
			}
		}

		// CUSTOM -> Add email optional field
		jQuery('#<?php echo $APPTAG?>-setEmailOptional').click(function() {
			jQuery(this).tooltip('hide');
			jQuery('#<?php echo $APPTAG?>-group-email_optional').removeClass('hide');
			jQuery(this).removeClass('base-icon-plus').addClass('base-icon-down-big').prop('disabled', true);
			email_optional.focus();
		});

		// CUSTOM -> ASSIGN USER
		assing_id.change(function() {
			var option = jQuery(this).find('option:selected');
			user_id.val(jQuery(this).val());
			name.val(option.data('name'));
			email.val(option.data('email'));
			cmail.val(option.data('email'));
		});
		// CUSTOM -> CREATE
		jQuery('#<?php echo $APPTAG?>-user-create').click(function() {
			user_id.val(0);
			assing_id.val('0').trigger("chosen:updated"); // selects
		});

		// CUSTOM -> view addresses list
		window.<?php echo $APPTAG?>_viewAddresses = function() {
			var itemID = formId.val();
			contactsaddresses_listReload(false, false, false, false, false, itemID);
		};
		// CUSTOM -> view phones list
		window.<?php echo $APPTAG?>_viewPhones = function() {
			var itemID = formId.val();
			contactsphones_listReload(false, false, false, false, false, itemID);
		};
		// CUSTOM -> view web socials list
		window.<?php echo $APPTAG?>_viewSocials = function() {
			var itemID = formId.val();
			contactswebSocials_listReload(false, false, false, false, false, itemID);
		};
		// CUSTOM -> view banks accounts list
		window.<?php echo $APPTAG?>_viewBanks = function() {
			var itemID = formId.val();
			contactsbanksAccounts_listReload(false, false, false, false, false, itemID);
		};
	...
	// métodos controladores das ações referente ao banco de dados e envio de arquivos
		...
		window.<?php echo $APPTAG?>_loadEditFields = function(appID, reload, formDisable) {
            ...
            if(item.user_id != 0) {
              if(item.block == 0) {
                jQuery('#<?php echo $APPTAG?>-block-0').addClass('hidden');
              } else {
                jQuery('#<?php echo $APPTAG?>-block-1').addClass('hidden');
              }
            }
						group_id.val(item.group_id).trigger("chosen:updated"); // selects
						selectRadio(type, item.type); // radio
						doc_number.val(item.doc_number);
						cnpj_contact_name.val(item.cnpj_contact_name);
						cnpj_contact_cpf.val(item.cnpj_contact_cpf);
						selectRadio(gender, item.gender); // radio
						name.val(item.name);
						nickname.val(item.nickname);
						email.val(item.email);
						cmail.val(item.email);
						email_optional.val(item.email_optional);
							// show or hide 'email_optional' field
							if(item.email_optional == '') {
								jQuery('#<?php echo $APPTAG?>-group-email_optional').addClass('hide');
								jQuery('#<?php echo $APPTAG?>-setEmailOptional').addClass('base-icon-plus').removeClass('base-icon-down-big').prop('disabled', false); // enable add button
							} else {
									jQuery('#<?php echo $APPTAG?>-setEmailOptional').trigger('click');
							}
						birthday.val(dateFormat(item.birthday)); // DATE -> conversão de data
						occupation.val(item.occupation);
						description.val(item.description);
						selectRadio(portfolio, item.portfolio); // radio
						...
						setFormDefinitions();
						// set CNPJ or CPF
						// Important: after setFormDefinitions();
						<?php echo $APPTAG?>_setType(item.type);
					});
		...
		window.<?php echo $APPTAG?>_userBlock = function(lock) {
      var msg = (lock) ? '<?php echo JText::_('TEXT_USER_LOCK_CONFIRM'); ?>' : '<?php echo JText::_('TEXT_USER_UNLOCK_CONFIRM'); ?>';
      if(confirm(msg)) {
        <?php echo $APPTAG?>_formExecute(true, true, true); // inicia o loader
  			jQuery.ajax({
  				url: "<?php echo $URL_APP_FILE ?>.model.php?aTag=<?php echo $APPTAG?>&rTag=<?php echo $RTAG?>&task=userBlock&lock="+lock+"&pid="+jQuery('#<?php echo $APPTAG?>-user_id').val(),
  				dataType: 'json',
  				type: 'POST',
  				cache: false,
  				success: function(data){
  					...
  					jQuery.map( data, function( res ) {
              if(res.status == 1) {
  							jQuery('#<?php echo $APPTAG?>-block-0, #<?php echo $APPTAG?>-block-1').toggleClass('hidden');
  						} else {
  							if(!itemID) mainForm.find('.set-error').removeClass('hide').text(res.msg);
  						}
  					});
		...
		window.<?php echo $APPTAG?>_userExist = function(itemID) {
			jQuery.ajax({
				url: "<?php echo $_CORE?>users/checkUser.php?id="+itemID,
				type: 'POST',
				cache: false,
				success: function(data) {
					if(data == 'true') {
						user_id.val(itemID);
						jQuery('#registrationFormGroup').addClass('hide');
						jQuery('#contactIsUser').removeClass('hide');
					} else {
						user_id.val(0);
						jQuery('#registrationFormGroup').removeClass('hide');
						jQuery('#contactIsUser').addClass('hide');
...
jQuery(window).load(function() {
	...
	window.<?php echo $APPTAG?>_validator = mainForm_<?php echo $APPTAG?>.validate({
		rules: {
			email: {
				remote: {
					url: '<?php echo $_CORE?>users/checkEmail.php',
					type: 'post',
	        data: {
	          cmail: function() {
				...
				required: function(element){
					return jQuery('#<?php echo $APPTAG?>-userRegistration').is(':checked');
        }
			},
			username : {
				remote: {
					url: '<?php echo $_CORE?>users/checkUsername.php',
					type: 'post'
				}
			},
			password : {
