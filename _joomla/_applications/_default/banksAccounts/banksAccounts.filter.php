...
// filter params
	...
	// BANK
	$bankID	= $app->input->get('bID', 0, 'int');
	...
	if(!empty($search)) :
		$where .= ' AND (';
		$where .= 'LOWER('.$db->quoteName('T1.agency').') LIKE LOWER("%'.$search.'%")';
		$where .= ' OR LOWER('.$db->quoteName('T1.account').') LIKE LOWER("%'.$search.'%")';
		$where .= ' OR LOWER('.$db->quoteName('T1.operation').') LIKE LOWER("%'.$search.'%")';
...
// FILTER'S DINAMIC FIELDS
	...
	// banks -> select
	$flt_bank = '';
	$query = 'SELECT * FROM '. $db->quoteName('#__envolute_banks') .' WHERE '. $db->quoteName('state') .' = 1 ORDER BY name';
	$db->setQuery($query);
			...
			<div class="row">
				<div class="col-sm-4">
					<div class="form-group">
						<select name="bID" id="bID" class="form-control input-sm set-filter">
							<option value="">- '.JText::_('FIELD_LABEL_BANK').' -</option>
							'.$flt_bank.'
						</select>
					</div>
				</div>
