...
jQuery(function() {
	...
	// APP FIELDS
	var category_id		= jQuery('#<?php echo $APPTAG?>-category_id');
	...
	var paid					= jQuery('#<?php echo $APPTAG?>-paid');
	var period				= mainForm.find('input[name=period]:radio');
	var due_date			= jQuery('#<?php echo $APPTAG?>-due_date');
	var currency			= jQuery('#<?php echo $APPTAG?>-currency');
	var price					= jQuery('#<?php echo $APPTAG?>-price');
	var start_date		= jQuery('#<?php echo $APPTAG?>-start_date');
	var user					= jQuery('#<?php echo $APPTAG?>-user');
	...
	// métodos controladores do formulário
		...
		var firstField = category_id; // campo que recebe o focus no carregamento
		...
		window.<?php echo $APPTAG?>_formReset = function() {
			...
			paid.prop('checked', false);
				// hide 'service payment' info
				jQuery('#<?php echo $APPTAG?>-paid-btnGroup').addClass('hide');
				jQuery('#<?php echo $APPTAG?>-paid-fieldsGroup').addClass('closed').hide(); // close fieldset
			selectRadio(period, 0);
			due_date.val('');
			currency.val('BRL').trigger("chosen:updated"); // select
			price.val('');
			start_date.val('');
			user.val('');
			password.val('');
		...
		// SET SERVICE PAYMENT INFO
		window.<?php echo $APPTAG?>_setPaymentInfo = function(show) {
			if(show) jQuery('#<?php echo $APPTAG?>-paid-btnGroup').removeClass('hide');
			else jQuery('#<?php echo $APPTAG?>-paid-btnGroup').addClass('hide');
		};
			...
			jQuery.ajax({
				...
				success: function(data) {
					jQuery.map( data, function( item ) {
						...
						// App Fields
						category_id.val(item.category_id).trigger("chosen:updated"); // selects
						selectRadio(type, item.type); // radio
							// show/hide 'paid' btn
							if(item.type == 1) jQuery('#<?php echo $APPTAG?>-paid-btnGroup').removeClass('hide');
							else jQuery('#<?php echo $APPTAG?>-paid-btnGroup').addClass('hide');
						url.val(item.url);
						description.val(item.description);
						paid.prop("checked", (item.paid == 1 ? true : false));
							// form closed by default
							if(item.paid == 1) jQuery('#<?php echo $APPTAG?>-paid-fieldsGroup').removeClass('closed').show(); // close fieldset
							else jQuery('#<?php echo $APPTAG?>-paid-fieldsGroup').addClass('closed').hide();
						selectRadio(period, item.period); // radio
						due_date.val(item.due_date);
						currency.val((item.currency != '' ? item.currency : 'BRL')).trigger("chosen:updated"); // select
