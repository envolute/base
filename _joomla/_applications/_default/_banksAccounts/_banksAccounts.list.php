	...
	foreach($res as $item) {
				...
				<form id="form-list-'.$APPTAG.'" method="post" class="pt-3">
					<table class="table table-striped table-hover table-sm">
						<thead>
							<tr>
								'.$adminView['head']['info'].'
								<th>'.baseAppHelper::linkOrder(JText::_('FIELD_LABEL_BANK'), 'T2.name', $APPTAG).'</th>
								<th class="d-none d-lg-table-cell">'.baseAppHelper::linkOrder(JText::_('FIELD_LABEL_AGENCY'), 'T1.agency', $APPTAG).'</th>
								<th class="d-none d-lg-table-cell">'.JText::_('FIELD_LABEL_ACCOUNT').'</th>
								<th class="d-none d-lg-table-cell">'.JText::_('FIELD_LABEL_OPERATION').'</th>
		...
		$html .= '
			<tr id="'.$APPTAG.'-item-'.$item->id.'" class="'.$rowState.'">
				'.$adminView['list']['info'].'
				<td>
					'.$note.baseHelper::nameFormat($item->bank).'
					<div class="d-lg-none text-sm text-muted mt-1">
				...
				<td class="d-none d-md-table-cell">'.$item->agency.'</td>
				<td class="d-none d-md-table-cell">'.$item->account.'</td>
				<td class="d-none d-md-table-cell">'.$item->operation.'</td>
