...
// LIST
	...
	$query = '
		...
		FROM
			...
			LEFT JOIN '. $db->quoteName('#__envolute_rel_clients_addresses') .' T2
			ON T2.address_id = T1.id
			LEFT JOIN '. $db->quoteName('#__envolute_clients') .' T3
			ON T3.id = T2.client_id
			LEFT JOIN '. $db->quoteName('#__envolute_rel_contacts_addresses') .' T4
		...
		<table class="table table-striped table-hover table-condensed">
			<thead>
				<tr>
					...
					<th>'.baseAppHelper::linkOrder(JText::_('FIELD_LABEL_ADDRESS_DISTRICT'), 'T1.address_district', $APPTAG).'</th>
					<th>'.baseAppHelper::linkOrder(JText::_('FIELD_LABEL_ADDRESS_CITY'), 'T1.address_city', $APPTAG).'</th>
					<th>'.baseAppHelper::linkOrder(JText::_('FIELD_LABEL_ADDRESS_STATE'), 'T1.address_state', $APPTAG).'</th>
					<th>'.baseAppHelper::linkOrder(JText::_('FIELD_LABEL_ADDRESS_COUNTRY'), 'T1.address_country', $APPTAG).'</th>
					'.$adminView['head']['actions'].'
				</tr>
			</thead>
			<tbody>
