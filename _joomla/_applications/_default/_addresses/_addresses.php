...
jQuery(function() {
	...
	var map					= jQuery('#<?php echo $APPTAG?>-map');
			...
			jQuery.ajax({
				...
				success: function(data) {
					...
					jQuery.map( data, function( item ) {
						...
						var showMap = (!isEmpty(item.latitude) || !isEmpty(item.longitude) || !isEmpty(item.url_map)) ? 1 : 0;
						checkOption(map, showMap); // checkbox
