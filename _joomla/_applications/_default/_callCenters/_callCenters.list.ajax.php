...
if(isset($_SERVER["HTTP_X_REQUESTED_WITH"]) AND strtolower($_SERVER["HTTP_X_REQUESTED_WITH"]) == "xmlhttprequest") :
	...
	$query	.= ' ORDER BY '. $db->quoteName('id') .' DESC,  '. $db->quoteName('id');
	try {
		$db->setQuery($query);
		$db->execute();
	...
	if($num_rows) : // verifica se existe
		...
		foreach($res as $item) {
			...
			$sTitle = $item->showTitle == 0 ? ' <span class="base-icon-eye-off text-danger cursor-help hasTooltip" title="'.JText::_('TEXT_NOT_VISIBLE').'"></span> ' : '';
			// Phones
			...
			// Extra info
			$info = !empty($item->extra_info) ? '<div class="pt-1">'.$item->extra_info.'</div>' : '';
			...
			$html .= '
				<li class="'.$rowState.'">
					...
					<h6 class="font-weight-bold mb-2">'.$lock.$item->title.$sTitle.'</h6>'.$phones.$emails.$chats.$links.$info.'
				</li>
			';
		}
		$html .= '</ul>';
