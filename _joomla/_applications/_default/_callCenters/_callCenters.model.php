...
if(isset($_SERVER["HTTP_X_REQUESTED_WITH"]) AND strtolower($_SERVER["HTTP_X_REQUESTED_WITH"]) == "xmlhttprequest") :
	...
	if($task != null) :
		...
		$request['showTitle']			= $input->get('showTitle', 0, 'int');
		...
		$email							= $input->get('email', array(), 'array');
		$email							= str_replace(';', '.', $email); // formata
		$request['email']				= implode(';', $email);
		...
		$save_condition = !empty($request['title']);
			...
			if($exist) : // verifica se existe
				...
				elseif($task == 'save' && $save_condition && $id) :
					...
					$query .=
						...
						$db->quoteName('showTitle')			.'='. $request['showTitle'] .','.
