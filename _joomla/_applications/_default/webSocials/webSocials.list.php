...
// LIST
	...
	$query = '
		...
		FROM
			...
			LEFT JOIN '. $db->quoteName('#__envolute_web_socials_names') .' T2
			ON T2.id = T1.socialName_id
			LEFT JOIN '. $db->quoteName('#__envolute_rel_clients_socials') .' T3
			ON T3.social_id = T1.id
			LEFT JOIN '. $db->quoteName('#__envolute_clients') .' T4
			ON T4.id = T3.client_id
			LEFT JOIN '. $db->quoteName('#__envolute_rel_contacts_socials') .' T5
			ON T5.social_id = T1.id
			LEFT JOIN '. $db->quoteName('#__envolute_contacts') .' T6
		...
		<table class="table table-striped table-hover table-condensed">
			<thead>
				<tr>
					...
					<th>'.baseAppHelper::linkOrder(JText::_('FIELD_LABEL_SOCIAL'), 'T2.name', $APPTAG).'</th>
