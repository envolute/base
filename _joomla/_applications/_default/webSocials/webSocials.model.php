...
header( 'content-type: application/json; charset=utf-8' );
  ...
  if($task != null) :
    ...
    // app
  	$request['socialName_id']    = $input->get('socialName_id', 0, 'int');
  	$request['page']          = $input->get('page', '', 'string');
      ...
      if($id) :
        ...
        // COUNT LIST IDS
          ...
          foreach($list as $item) {
              ...
              // Default Fields
          		...
          		'next'	      => $next,
              // App Fields
      				'socialName_id'	  => $item->socialName_id,
      				'page'	        => $item->page,
  			...
  			elseif($task == 'save' && $id) :
  				...
  				$query .=
            $db->quoteName('socialName_id') 	.'='. $request['socialName_id'] .','.
            $db->quoteName('page') 	.'='. $db->quote($request['page']) .','.
            ...
            if(!empty($_SESSION[$RTAG.'RelTable']) && !empty($_SESSION[$RTAG.'AppNameId'])) :
              ...
              $query = 'DELETE FROM '. $db->quoteName('#__envolute_rel_clients_socials') .' WHERE '. $db->quoteName('social_id') .' IN ('.$ids.')';
              $db->setQuery($query);
    					$db->execute();
              // CONTACTS -> remove os registros relacionados aos contatos
              $query = 'DELETE FROM '. $db->quoteName('#__envolute_rel_contacts_socials') .' WHERE '. $db->quoteName('social_id') .' IN ('.$ids.')';
              $db->setQuery($query);
    					$db->execute();
            endif;
        ...
        // validation
        if(!empty($request['socialName_id']) && $request['socialName_id'] != 0 && !empty($request['page'])) :

          // Prepare the insert query
    			$query  = '
          INSERT INTO '. $db->quoteName($cfg['mainTable']) .'('.
            $db->quoteName('socialName_id') .','.
            $db->quoteName('page') .','.
            $db->quoteName('username') .','.
    				$db->quoteName('state') .','.
    				$db->quoteName('created_by')
    			.') VALUES ('.
            $request['socialName_id'] .','.
            $db->quote($request['page']) .','.
