...
// filter params
	...
	// CLIENT
	$client = '';
	$fUser = $app->input->get('fUser', array(), 'array');
	for($i = 0; $i < count($fUser); $i++) {
		$client .= ($i == 0) ? ' AND (' : ' OR ';
		$client .= 'FIND_IN_SET ('.$fUser[$i].', T1.created_by)';
		$client .= ($i == (count($fUser) - 1)) ? ')' : '';
	}
	$where .= $client;
	...
	$searchFields = array(
		'T1.title'				=> 'FIELD_LABEL_TITLE',
		'T1.description'		=> 'FIELD_LABEL_DESCRIPTION',
		'T1.request_id'			=> 'FIELD_LABEL_TASK_ID'
	);
	$i = 0;
...
// ORDER BY
	...
	$orderDef = 'T1.created_date'; // não utilizar vírgula no inicio ou fim
	if(!isset($_SESSION[$APPTAG.'oF'])) : // DEFAULT ORDER
		$_SESSION[$APPTAG.'oF'] = 'T1.orderer';
...
foreach ($client as $obj) {
	...
	$team = ($obj->type == 1) ? '*' : '';
	$flt_user .= '<option value="'.$obj->user_id.'"'.($obj->user_id == $fUser ? ' selected = "selected"' : '').'>'.$team.baseHelper::nameFormat($name).'</option>';
}
			...
			<div class="row">
				<div class="col-sm-6 col-md-3">
					<div class="form-group">
						<label class="label-xs text-muted">'.JText::_('FIELD_LABEL_CREATED_BY').'</label>
						<select name="fUser[]" id="fUser" class="form-control form-control-sm set-filter" multiple>
							'.$flt_user.'
						</select>
					</div>
				</div>
