...
// filter params
	...
	// TEAM MEMBER -> select
	$fTeam	= $app->input->get('tID', 0, 'int');
	if($fTeam > 0) $where .= ' AND '.$db->quoteName('T1.team_id').' = '.$fTeam;
	// CLIENT
...
// FILTER'S DINAMIC FIELDS
	...
	// teams -> select
	$flt_team = '';
	$query = 'SELECT * FROM '. $db->quoteName('#__'.$cfg['project'].'_teams') .' WHERE type = 2 ORDER BY name';
	$db->setQuery($query);
	$teams = $db->loadObjectList();
	foreach ($teams as $obj) {
		$flt_team .= '<option value="'.$obj->id.'"'.($obj->id == $fTeam ? ' selected = "selected"' : '').'>'.baseHelper::nameFormat($obj->name).'</option>';
	}
