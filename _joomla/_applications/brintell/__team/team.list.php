...
if($hasAdmin) :
	...
	$adminView['head']['actions'] = '
		...
		<th class="text-center d-print-none">'.JText::_('TEXT_ACTIONS').'</th>
	';
endif;
		...
		<table class="table table-striped table-hover table-sm">
			<thead>
				<tr>
					...
					<th>'.JText::_('FIELD_LABEL_GROUP').'</th>
					<th>'.JText::_('FIELD_LABEL_ROLE').'</th>
...
if($num_rows) : // verifica se existe
	...
	foreach($res as $item) {
		...
		if($cfg['hasUpload']) :
			...
			if(!empty($img)) $img = '<img src="'.baseHelper::thumbnail('images/apps/'.$APPPATH.'/'.$img['filename'], 44, 44).'" class="d-none d-md-inline img-fluid rounded float-left mr-2" />';
		...
		$urlViewData = JURI::root().'apps/team/view?vID='.$item->id;
		...
		$html .= '
			<tr id="'.$APPTAG.'-item-'.$item->id.'" class="'.$rowState.'">
				...
				<td>'.$item->type.'</td>
				<td class="d-none d-lg-table-cell text-center">'.baseHelper::nameFormat($item->role).'</td>
				<td>'.$status.'</td>
