...
// list view
$urlToList = JURI::root().'apps/'.$APPPATH.'/portfolio';
...
if($vID != 0) :
	...
	$query	= '
		...
		WHERE
			...
			'. $db->quoteName('T1.portfolio') .' = 1 AND
	...
	if(!empty($item->name)) : // verifica se existe
		...
		$agree	= $item->portfolio == 1 ? '<span class="badge badge-success float-right">'.JText::_('FIELD_LABEL_AGREEMENT').'</span>' : '';
		// DESCRIPTION
		...
		$query	= '
			...
			FROM '.$db->quoteName('#__'.$cfg['project'].'_locations') .' T1
				JOIN '. $db->quoteName('#__'.$cfg['project'].'_rel_clients_locations') .' T2
				ON '.$db->quoteName('T2.location_id') .' = T1.id
			WHERE '.$db->quoteName('T2.client_id') .' = '. $vID .' AND '.$db->quoteName('T1.isPublic') .' = 1
		...
		if($num_rows) : // verifica se existe
			$locations .= '<div class="portfolio-locations">';
		...
		$query	= '
			...
			FROM '.$db->quoteName('#__'.$cfg['project'].'_callCenters') .' T1
				JOIN '. $db->quoteName('#__'.$cfg['project'].'_rel_clients_callCenters') .' T2
		...
		if(!empty($website) || !empty($callCenters)) : // verifica se existe
			$attend .= '
				<div class="portfolio-callCenters">
					<hr class="hr-tag" />
		...
		if(!empty($item->service_desc)) : // verifica se existe
			$terms .= '
			<div class="portfolio-locations p-3 bg-light b-all">
		...
		$client .= '
			...
			<div id="portfolio-header" class="row">
				<div class="col">
					<div class="clearfix">
						'.$logo.'
						<h1 class="mt-0">
		...
		$client = '<p class="base-icon-info-circled alert alert-info m-0"> '.JText::_('MSG_ITEM_NOT_AVAILABLE').'</p>';
	endif;
