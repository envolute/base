...
// LIST
	...
	$query = '
		...
		FROM
			...
			LEFT OUTER JOIN '. $db->quoteName($cfg['mainTable'].'_roles') .' T2
			ON T2.id = T1.role_id
...
if($num_rows) : // verifica se existe
	...
	foreach($res as $item) {
					...
					<table class="table table-striped table-hover table-sm">
						<thead>
							<tr>
								...
								<th class="d-none d-lg-table-cell">'.baseAppHelper::linkOrder(JText::_('FIELD_LABEL_TYPE'), 'T2.name', $APPTAG).'</th>
		...
		if(!empty($item->role)) $info .= '<div class="small text-muted">'.baseHelper::nameFormat($item->role).'</div>';
		else if(!empty($item->occupation)) $info .= '<div class="small text-muted">'.baseHelper::nameFormat($item->occupation).'</div>';
		...
		$urlViewData = JURI::root().'apps/staff/view?vID='.$item->user_id;
