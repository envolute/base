...
if(isset($_SERVER["HTTP_X_REQUESTED_WITH"]) AND strtolower($_SERVER["HTTP_X_REQUESTED_WITH"]) == "xmlhttprequest") :
	...
	if($task != null) :
		...
		$save_condition = ($request['issue_id'] > 0 && !empty($request['comment']));
			...
			if($task == 'save') :
				...
				if($save_condition) :
					...
					try {
						...
						// get request data
						$query = 'SELECT * FROM '. $db->quoteName('#__'.$cfg['project'].'_issues') .' WHERE '. $db->quoteName('id') .' = '. $request['issue_id'];
						$db->setQuery($query);
						$item = $db->loadObject();
						...
						if(!empty($emails)) {
							...
							for ($i = 0; $i < count($users); $i++) {
								...
								$url = $_ROOT.'/apps/issues/view?vID='.$item->id;
