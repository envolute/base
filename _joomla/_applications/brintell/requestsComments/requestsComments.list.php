	...
	<form id="form-list-'.$APPTAG.'" method="post" class="pt-3">
		<table class="table table-striped table-hover table-sm">
			<thead>
				<tr>
					'.$adminView['head']['info'].'
					<th width="60" class="text-right">'.JText::_('FIELD_LABEL_TASK_ID').'</th>
					<th>'.baseAppHelper::linkOrder(JText::_('FIELD_LABEL_COMMENT'), 'T1.comment', $APPTAG).'</th>
	...
	foreach($res as $item) {
		...
		$html .= '
			<tr id="'.$APPTAG.'-item-'.$item->id.'" class="'.$rowState.'">
				'.$adminView['list']['info'].'
				<td class="text-right">#'.$item->request_id.'</td>
				<td><div class="text-sm">'.baseHelper::nameFormat($item->comment).'</div>'.$listFiles.'</td>
