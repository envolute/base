...
jQuery(function() {
	...
	var parentFieldId		= type_id; // 'null', caso não exista...
	...
	// métodos controladores do formulário
		...
		window.<?php echo $APPTAG?>_formReset = function() {
			...
			type_id.selectUpdate(0); // selects;
			name.val('');
			email.val('');
			cmail.val('');
			cpf.val('');
			...
			address_city.val('');
			address_state.selectUpdate('PB', ''); // select;
			description.val('');
			...
			jQuery.ajax({
				...
				success: function(data) {
					...
					jQuery.map( data, function( item ) {
						...
						// App Fields
						type_id.selectUpdate(item.type_id); // selects;
						name.val(item.name);
						...
						address_state.selectUpdate(item.address_state, ''); // select;
