...
// GET DATA
$query = 'SELECT *, DAY(birthday) day FROM '.$db->quoteName($cfg['mainTable']).' WHERE MONTH(birthday) = MONTH(NOW()) AND DAY(birthday) = DAY(NOW()) ORDER BY name';
try {
	$db->setQuery($query);
	$db->execute();
...
if($num_rows) : // verifica se existe
	...
	foreach($res as $item) {
		...
		$html .= '
			<li class="'.$rowState.'">
				<span class="base-icon-birthday text-live float-right"></span>
	...
	if($noReg) $html = '<p class="base-icon-info-circled alert alert-info m-0"> '.JText::_('MSG_NO_DAY_BIRTHDAY').'</p>';
endif;
