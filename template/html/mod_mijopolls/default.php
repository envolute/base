...
<?php if ($params->get('show_poll_title')) : ?>
  <h4 class="font-featured text-live"><?php echo $poll->title; ?></h4>
<?php endif; ?>
...
<div id="polldiv_<?php echo $poll->id;?>">
  ...
  <form action="<?php echo JRoute::_('index.php');?>" method="post" name="poll_vote_<?php echo $poll->id;?>" id="poll_vote_<?php echo $poll->id;?>">
    <div class="form-check">
      <?php for ($i = 0, $n = count($options); $i < $n; $i ++) { ?>
      	<label for="mod_voteid<?php echo $options[$i]->id;?>" class="form-check-label <?php echo $tabclass_arr[$tabcnt].$params->get('moduleclass_sfx'); ?>" style="display:block;">
      		<input type="radio" class="form-check-input" name="voteid" id="mod_voteid<?php echo $options[$i]->id;?>" value="<?php echo $options[$i]->id;?>" alt="<?php echo $options[$i]->id;?>" <?php echo $disabled; ?> />
        ...
        $tabcnt = 1 - $tabcnt;
  		...
  		if($params->get('show_msg')) :
  			echo '<div id="mod_poll_messages_'.$poll->id.'" class="m-1">'.JText::_($msg);
  	...
  	<div style="padding:2px;" id="poll_buttons_<?php echo $poll->id;?>" >
  		<input type="submit" id="submit_vote_<?php echo $poll->id; ?>" name="task_button" class="btn btn-primary" value="<?php echo JText::_('MOD_MIJOPOLLS_VOTE'); ?>" <?php echo $disabled; ?> />
  	</div>
  	<div id="poll_loading_<?php echo $poll->id;?>" style="display:none;">
			<?php echo JText::_('MOD_MIJOPOLLS_PROCESSING'); ?>
  	</div>
  ...
  //If user has voted
  		...
  		foreach ($results as $row) :
  			...
  			<div>
  				...
  				<div style="padding:1px; border:1px solid #<?php echo $params->get('poll_bars_border_color'); ?>;">
  ...
  <?php  endforeach;
  			...
  			if($params->get('show_msg')) :
  				echo '<div class="alert alert-warning p-1 my-xs" id="mod_poll_messages_'.$poll->id.'">'.JText::_($msg);
